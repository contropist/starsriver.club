<?php
/********************************************************************
 * Copyright (c) 2020 All Right Reserved By [StarsRiver]            *
 *                                                                  *
 * Author  Zhangyu                                                  *
 * Email   starsriver@yahoo.com                                     *
 ********************************************************************/

/**
 *      [Discuz!] (C)2001-2099 Comsenz Inc.
 *      This is NOT a freeware, use is subject to license terms
 *
 *      $Id: lang_template.php 36334 2017-01-03 01:32:35Z nemohou $
 *
 *      This file is automatically generate
 */

$lang = array (
  'activity_jointime' => '申请时间',
  'activity_member_unit' => '人',
  'activity_new_join' => '已加入',
  'activity_payment' => '预算',
  'activity_self' => '自费',
  'leaveword' => '留言',
  'reason_leaveword' => '你为何要放弃治疗 (*•̀ㅂ•́)و',
  'payment_unit' => '元',
  'activity_allow_join' => '允许参加',
  'activity_applylist' => '活动报名者',
  'activity_applylist_manage' => '活动报名者管理',
  'activity_cant_audit' => '尚未审核',
  'activity_do_replenish' => '等待完善',
  'activity_join_members' => '申请者',
  'activity_ps' => '附言',
  'activity_refuse' => '拒绝',
  'checkall' => '全选',
  'close' => '关闭',
  'back' => '返回',
  'confirm' => '批准',
  'extension_project' => '扩展项目',
  'homepage' => '首页',
  'no_informations' => '无任何信息',
  'send_notification' => '发通知',
  'send_pm' => '私信',
  'status' => '状态',
  'to_improve' => '需完善',
  'views' => '查看',
  'activity_already' => '已报名',
  'activity_content' => '内容',
  'activity_space' => '地点',
  'activity_start_between' => '$activity[starttimefrom] 至 $activity[starttimeto] 商定',
  'activity_starttime' => '时间',
  'activity_totime' => '截止日期',
  'female' => '♀',
  'gender' => '性别',
  'male' => '♂',
  'unlimited' => '秀吉',
  'admin_pm' => '通知作者',
  'admin_reason' => '操作原因',
  'confirms' => '确定',
  'quickclear' => '快速清理',
  'quickclear_avatar' => '清除头像',
  'quickclear_crime_avatar_nums' => '用户 $crimeauthor 已被清除头像 $crimenum_avatar 次',
  'quickclear_crime_customstatus_nums' => '用户 $crimeauthor 已被清除自定义头衔 $crimenum_customstatus 次',
  'quickclear_crime_sightml_nums' => '用户 $crimeauthor 已被清除签名 $crimenum_sightml 次',
  'quickclear_customstatus' => '清除自定义头衔',
  'quickclear_sightml' => '清除签名',
  'submit' => '提交',
  'e_attach_del' => '删除',
  'e_attach_mediacode' => '添加附件媒体播放代码',
  'e_attach_url' => '添加附件地址',
  'highest_right' => '最高权限',
  'readperm' => '阅读权限',
  'dlperm' => '下载权限',
  'update' => '更新',
  'upload_msg' => '$attach[filenametitle] {LF}上传日期: $attach[dateline] {LF}文件大小: $attach[attachsize]',
  'delete' => '删除',
  'follow_quickreply' => '回复',
  'follow_relay' => '转播',
  'poston' => '发表于',
  'forumlist' => '版块列表',
  'forumlist_allforum' => '所有版块',
  'forumlist_myfav' => '我的收藏',
  'forumlist_recent' => '最近浏览',
  'search' => '搜索',
  'description' => '描述',
  'save_to_album' => '存画廊',
  'post_meanwhile_relay' => '转发',
  'reply' => '回复',
  'view_all_replies' => '查看全部',
  'all' => '全部',
  'announcement' => '公告',
  'author' => '作者',
  'month' => '月',
  'open' => '展开',
  'year' => '年',
  'attachment' => '附件',
  'buy_all_attch' => '购买所有',
  'download' => '下载',
  'free_buy' => '免费获取',
  'pay_attachment' => '购买附件',
  'pay_author_income' => '作者可得',
  'pay_balance' => '购买后余额',
  'price' => '售价',
  'status_download' => '你所在的用户组可以免费下载',
  'status_insufficient' => '抱歉，你的 {$_G[setting][extcredits][$_G[setting][creditstransextra][1]][title]} 不足，无法下载',
  'attachment_buy_not' => '目前没有用户购买此附件',
  'pay_view' => '购买记录',
  'time' => '时间',
  'username' => '用户名',
  'user' => '用户',
  'collect' => '收藏',
  'collection' => '专辑',
  'collection_create' => '创建专辑',
  'collection_key' => '专辑信息',
  'collection_cover' => '专辑封面',
  'collection_cover_sug' => '推荐比例4：3',
  'collection_desc' => '简介',
  'collection_desc_exceed' => '"简介不能超过 "+desclimit+" 字节"',
  'collection_edit' => '编辑专辑',
  'collection_keywords' => '专辑标签',
  'collection_keywords_desc' => '用逗号隔开，最多可填写 5 个。',
  'collection_remain_tips' => '还可以创建 $reamincreatenum 个专辑',
  'collection_title' => '新专辑名',
  'collection_title_exceed' => '"标题不能超过 "+titlelimit+" 字节"',
  'collection_sortby_coomentnum' => '最多评论',
  'collection_sortby_follownum' => '最多订阅',
  'collection_sortby_threadnum' => '最多文章',
  'collection_sortby_update' => '最早创建',
  'collection_backcollection' => '返回专辑',
  'collection_comment_submit' => '发表评论',
  'collection_commentlist' => '评论列表',
  'collection_torate' => '点评',
  'collection_followlist' => '订阅用户列表',
  'collection_norate' => '暂无评分',
  'collection_norate_toolow' => '评分人数太少',
  'collection_ratecollection' => '请为本专辑评分：',
  'collection_rated' => '你已经为专辑评分：',
  'collection_totalrates' => '共{$_G[collection][ratenum]}人参与',
  'collection_comment_specthread' => '针对《$thread[subject]》的评论：',
  'collection_comment_thread' => '淘帖评论',
  'no_content' => '暂时没有内容',
  'collection_invite' => '邀请参与',
  'collection_max_invite' => '每个专辑最多可以邀请 <span id="invitenum">$maxteamworkers</span> 人维护',
  'collection_selectfriend' => '选择好友',
  'collection_username' => '用户名:',
  'invite' => '邀请',
  'collection_commentnum' => '评论',
  'collection_createtime' => '创建',
  'collection_follow' => '订阅',
  'collection_index_keywords' => '关键词',
  'collection_lastthread' => '最新文章',
  'collection_lastupdate' => '更新于',
  'collection_nocollection' => '还没有专辑，快来<a href="forum.php?mod=collection&action=edit" onclick="showWindow(\'showMsgBox\', this.href, \'get\', 0)">创建</a>吧',
  'collection_threadnum' => '文章',
  'collection_all' => '所有专辑',
  'collection_my' => '我的专辑',
  'collection_nav_related' => '相关专辑',
  'collection_recommended' => '推荐专辑',
  'collection_recommend' => '推荐文章',
  'collection_recommend_submit' => '推荐文章',
  'collection_recommend_thread_url' => '文章地址',
  'collection_addbtn' => '添加',
  'collection_addreason' => '淘帖理由',
  'collection_fill_entire' => '请将标题填写完整',
  'collection_reason_exceed' => '"淘帖理由不能超过 "+reasonlimit+" 字节"',
  'collection_select' => '选择专辑',
  'collection_select_nocollection' => '你还没有创建专辑。',
  'collection_select_remain' => '现在还可以创建 <a id="reamincreatenum">$reamincreatenum</a> 个专辑。',
  'collection_view_mine' => '查看专辑',
  'collection_mycreate' => '我创建的',
  'collection_myteam' => '我参与的',
  'collection_mysubscribe' => '我订阅的',
  'anonymous' => '匿名',
  'attach_img' => '图片附件',
  'collection_allcomment' => '全部',
  'collection_followers' => '最近订阅者',
  'collection_allfollowers' => '所有订阅者',
  'collection_cloud_link' => '搜索相关文章',
  'collection_cloud_search' => '专辑还没有内容，你可以点击 <a href="forum.php?mod=collection&action=view&ctid={$ctid}&op=related">搜索相关文章</a> 找到相关内容。',
  'collection_creator' => '专辑创建人：',
  'collection_delete_confirm' => '确定要 <strong>删除这个专辑</strong> 吗?',
  'collection_delete_confirm2' => '确定要让该用户退出管理组吗',
  'collection_exit_team' => '退出维护',
  'collection_exit_team_confirm' => '真的要退出维护专辑?',
  'collection_invite_team' => '邀请维护',
  'collection_inviteallfriend' => '邀请所有好友',
  'collection_newcomment' => '最新评论',
  'collection_newfollowers' => '最新订阅',
  'collection_recommended_reason' => '推荐理由',
  'collection_teamworkers' => '共同维护人：',
  'collection_thread' => '淘帖文章',
  'collection_unfollow' => '已订阅',
  'collection_creators' => '他还有这些淘帖',
  'collection_admins' => '管理人',
  'edit' => '编辑',
  'lastpost' => '最后发表',
  'replies' => '回复/查看',
  'reward_solved' => '卍&nbsp;解',
  'rewarding' => '探索中',
  'thread_activity' => '活动',
  'thread_debate' => '辩论',
  'thread_digest' => '蜜制',
  'thread_poll' => '投票',
  'thread_reward' => '悬赏',
  'thread_trade' => '交易',
  'thread_hot' => '热门',
  'level' => '级',
  'comment_1' => '差',
  'comment_2' => '一般',
  'comment_3' => '还行',
  'comment_4' => '好',
  'comment_5' => '很好',
  'comment_6' => '非常好',
  'comment_give_ip' => '放弃此观点',
  'comment_message' => '记得在这里写下标题，不能超过26个字呢....   (*´Д｀*)    ',
  'comments' => '评论',
  'more' => '更多<i class="ft-chevron-right"></i>',
  'more_c' => '更多',
  'publish' => '发布',
  'detail' => '详情',
  'guest' => '游客',
  'debate_bestdebater' => '最佳辩手',
  'debate_draw' => '平局',
  'debate_list_nonexistence' => '如果不在列表中，请自行填写',
  'debate_opponent' => '蓝方',
  'debate_poll' => '票',
  'debate_recommend_list' => '推荐名单',
  'debate_square' => '红方',
  'debate_umpirecomment' => '裁判评论',
  'debate_umpirepoint' => '裁判观点',
  'debate_winner' => '获胜',
  'announcements' => '公告',
  'forum_category_modedby' => '分区版主',
  'forum_lastpost' => '最后发表',
  'forum_moderators' => '版主',
  'forum_myfav' => '我的关注',
  'forum_posts' => '帖数',
  'forum_subforums' => '子版块',
  'forum_threads' => '文章',
  'forum_todayposts' => '今日',
  'hotthreads_forum' => '$_G[setting][navs][2][navname]热点',
  'index_posts' => '回复',
  'index_today' => '今日',
  'index_yesterday' => '昨日',
  'my_posts' => '我的文章',
  'never' => '从未',
  'on' => '于',
  'onlinemember' => '当前在线',
  'private_forum' => '私密版块',
  'show_newthreads' => '最新回复',
  'spread' => '收起/展开',
  'total' => '总计',
  'url_link' => '链接到外部地址',
  'welcome_new_members' => '欢迎新会员',
  'attachcredits' => '下载积分',
  'attachment_buy' => '购买',
  'clicktodownload' => '点击文件名下载附件',
  'guestviewthumb' => '登录/注册后可看大图',
  'discuzcode_copyclipboard' => '复制代码',
  'downloads' => '下载数',
  'post_hide' => '本文章隐藏的内容',
  'post_code' => '代码',
  'post_quote' => '引用',
  'post_hide_credits' => '这里面的内容需要你拥有超过 $creditsrequire 积分时才能看，不然可能会亮瞎眼 (,,•́ . •̀,,)',
  'post_hide_credits_hidden' => '这里面的内容需要你拥有超过 $creditsrequire 积分时才能看，不然可能会亮瞎眼 (,,•́ . •̀,,)',
  'post_hide_reply_hidden' => '<a href="forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]" onclick="showWindow(\'reply\', this.href)">回复是作者的第一生产力 (ฅ´ω`ฅ) ！</a>',
  'set_cover' => '设为封面',
  'upload' => '上传',
  'attachment_allow_exts' => '支持类型',
  'attachment_insert_all_attach' => '插入全部附件',
  'attachment_list' => '上传附件',
  'attachment_select_attach_image' => '全选',
  'attachment_size' => '文件容量上限',
  'cancel' => '取消',
  'common' => '普通',
  'common_upload' => '普通上传',
  'create_new_album' => '创建新画廊',
  'credits_policy' => '积分说明',
  'e_attach_insert' => '点击附件文件名添加到回复内容中',
  'e_img_albumlist' => '画廊图片',
  'e_img_attach' => '上传图片',
  'e_img_height' => '高',
  'e_img_insertphoto' => '点击图片添加到回复内容中',
  'e_img_inserturl' => '请输入图片地址',
  'e_img_width' => '宽',
  'e_img_www' => '网络图片',
  'filename' => '文件名',
  'input_album_name' => '请输入画廊名称',
  'lower_than' => '小于',
  'normal_upload' => '普通上传',
  'post_credits_postattach' => '每上传一个附件你的',
  'post_price_attachincome_comment' => '附件出售上限： {$_G[setting][maxincperthread]} {$_G[setting][extcredits][$_G[setting][creditstransextra][1]][unit]}&nbsp;&nbsp;&nbsp;&nbsp;',
  'post_select_usergroup_readacces' => '选中及以上组用户才可以访问阅读',
  'save_selected_pic' => '将选中的图片保存到画廊',
  'select_album' => '选择画廊',
  'size_no_limit' => '大小不限制',
  'uch_selectfromalbum' => '从我的画廊中选择图片',
  'upload_after_selected' => '选择完文件后请点击“上传”按钮',
  'uploading' => '上传中，请稍候，你可以<a onclick="hideMenu()">暂时关闭这个小窗口</a>，上传完成后你会收到通知',
  'forum_activeusers' => '正在浏览此版块的会员',
  'forum_archive' => '存档',
  'forum_domain' => '本版域名',
  'forum_favorite' => '收藏本版',
  'forum_modedby' => '版主',
  'forum_moderate_unhandled' => '<a href="home.php?mod=space&uid=$_G[uid]&do=thread&view=me&type=thread&from=&filter=aduit">你有 $threadmodcount 个文章正等待审核中，点击查看</a>',
  'forum_recommend' => '推荐文章',
  'forum_recyclebin' => '回收站',
  'forum_viewall' => '全部',
  'index_threads' => '文章',
  'forum_live_newreply_refresh' => '有新的发言了，点击刷新',
  'forum_live_fastreply_notice' => '回复一下吧喵~(ฅ´ω`ฅ)~',
  'forum_live_post' => '发表',
  'forum_live_nocontent_error' => '抱歉，你尚未输入内容',
  'forum_live_nolength_error' => '\'你的回复长度不符合要求。\n\n当前长度: \' + mb_strlen(theform.message.value) + \' 字节\n系统限制: \' + postminchars + \' 到 \' + postmaxchars + \' 字节\'',
  'modcp' => '控制面板',
  'post_newthread' => '发表文章',
  'post_reply' => '回复',
  'post_newthreadactivity' => '发起活动',
  'post_newthreaddebate' => '发起辩论',
  'post_newthreadpoll' => '发起投票',
  'post_newthreadreward' => '发布悬赏',
  'post_newthreadtrade' => '出售商品',
  'recommended_groups' => '推荐{$_G[setting][navs][3][navname]}',
  'return_index' => '返&nbsp;回',
  'rss_subscribe_this' => '订阅',
  'send_posts' => '发新帖',
  'their' => '所属',
  'threads_all' => '全部文章',
  'viewd_threads' => '浏览过的文章',
  'viewed_forums' => '浏览过的版块',
  'click_to_show_reason' => '查看原因',
  'connect_fill_profile_to_post' => '先 <a href="member.php?mod=connect">完善帐号</a> 或 <a href="member.php?mod=connect&ac=bind">绑定帐号</a> 后才可以回复&nbsp; (,,• ₃ •,,)   ',
  'login_to_post' => '回复之前你需要先有个身份&nbsp;&nbsp;๑乛◡乛๑&nbsp;&nbsp;',
  'no_permission_to_post' => '你现在没有权限回复&nbsp;Σ( ° △ °|||)',
  'post_advancemode' => '食用高端编辑器',
  'post_relay' => '转播给粉丝',
  'post_sync_method' => '将此文章同步到',
  'quick_post' => '快速发送',
  'select_thread_catgory' => '请选择分类',
  'no_thread_catgory' => '快速回复',
  'favorite_forums' => '收藏的版块',
  'forum_nav' => '版块导航',
  'all_reward' => '全部悬赏',
  'closed_thread' => '冻结的文章',
  'digest_posts' => '蜜制',
  'forum_group' => '版块/社团',
  'forum_nothreads' => '这里什么都没有&nbsp;  (￣▽￣")',
  'forum_thread' => '版块文章',
  'from' => '来自',
  'fromgroup' => '来自社团',
  'selectmygroup' => '选择我的社团',
  'have_newreplies' => '有新回复',
  'heats' => '热度',
  'last_1_days' => '一天',
  'last_2_days' => '两天',
  'list_default_sort' => '默认排序',
  'list_one_month' => '一个月',
  'list_one_week' => '一周',
  'list_post_time' => '回复时间',
  'list_three_month' => '三个月',
  'new_window' => '新窗',
  'new_window_thread' => '在新窗口中打开文章',
  'order_heats' => '热门',
  'order_recommends' => '推荐',
  'orderby' => '排序',
  'pics' => '张图片',
  'post_mobile' => '手机回复',
  'posts_deducted' => '回复被减分',
  'rate_credit_add' => '回复被加分',
  'replycredit' => '回复奖励',
  'rushreply' => '抢楼',
  'rush' => '抢',
  'screening' => '筛选',
  'search_any_date' => '时间',
  'show' => '查看',
  'show_rewarded_only' => '只看已解决的',
  'show_rewarding_only' => '只看进行中的',
  'showupgrade' => '查看更新',
  'target_blank' => '新窗口打开',
  'thread_moved' => '移动',
  'thread_recommend' => '评价指数',
  'thread_stick' => '置顶',
  'thread_type1' => '本版置顶文章',
  'thread_type2' => '分类置顶文章',
  'thread_type3' => '全局置顶文章',
  'thread_type4' => '多版置顶文章',
  'title' => '标题',
  'view_thread' => '浏览文章',
  'view_thread_imagemode' => '图片模式',
  'forum_password_require' => '想要进入？ 交出♂神秘代码♂ &nbsp; ๑乛◡乛๑',
  'guide' => '导读',
  'guide_attend' => '人参与',
  'guide_digest' => '最新蜜制',
  'guide_forum_select' => '版块',
  'guide_hot' => '最新热门',
  'guide_index' => '导读首页',
  'guide_my' => '我的',
  'guide_new' => '最新回复',
  'guide_newthread' => '最新发表',
  'guide_sofa' => '抢前排',
  'guide_nothreads' => '这里空空如也',
  'keyword' => '关键字',
  'posts_type' => '文章类型',
  'forum_moderate' => '审核',
  'logout' => '退出',
  'mod_error_invalid' => '抱歉，你无此权限',
  'mod_notice' => '论坛管理员在“控制面板”中权限和超级版主基本相同，如果需要更多功能，请进入 <a href="webcontroller.php?mod=forum" target="_blank"><u>管理中心</u></a> ',
  'mod_notice_title' => '内部留言',
  'mod_option_error' => '系统错误',
  'mod_option_forum_edit' => '版规编辑',
  'mod_option_forum_recommend' => '推荐文章',
  'mod_option_member_access' => '用户权限',
  'mod_option_member_ban' => '禁止用户',
  'mod_option_member_edit' => '编辑用户',
  'mod_option_member_ipban' => '禁止 IP',
  'mod_option_return' => '返回论坛',
  'mod_option_subject' => '文章管理',
  'modcp_logs' => '管理日志',
  'modcp_report' => '管理举报',
  'action' => '操作',
  'delete_check' => '全部删除',
  'delete_all' => '全删',
  'displayorder' => '顺序',
  'endtime' => '结束时间',
  'link' => '链接',
  'mod_announce_add' => '添加公告',
  'mod_announce_edit' => '编辑公告',
  'mod_announce_list' => '公告列表',
  'mod_announce_type' => '公告类型',
  'mod_announce_type_text' => '文字类型',
  'mod_announce_type_url' => '网址链接',
  'mod_message_announce_add' => '公告添加完毕，请继续操作',
  'mod_message_announce_del' => '选定公告删除完毕，请继续操作',
  'mod_message_announce_edit' => '公告设置更新完毕，请继续操作',
  'return' => '返回',
  'starttime' => '起始时间',
  'text' => '文字',
  'disabled' => '禁用',
  'discuzcode' => '编辑器代码',
  'enabled' => '可用',
  'forum_not_allow' => '抱歉，你所在的用户组不允许修改版规！',
  'forum_rules' => '版规',
  'memcp_profile_preview' => '预览',
  'mod_forum_recommend_expiration' => '时间期限',
  'mod_forum_recommend_list' => '更新列表',
  'mod_message_forum_nopermission' => '抱歉，你无此权限！',
  'mod_message_forum_update' => '版规更新成功，请继续操作',
  'mod_message_forum_updaterecommend' => '推荐文章更新成功，请继续操作',
  'more_settings' => '更多设置',
  'post_html' => 'HTML 代码',
  'post_imgcode' => '[img] 代码',
  'recommend_moderator' => '推荐人',
  'search_nomatch' => '抱歉，没有找到匹配结果',
  'subject' => '标题',
  'forum' => '版块',
  'member' => '会员',
  'mod_access_ban_download' => '禁止下载附件',
  'mod_access_ban_getimage' => '禁止查看图片',
  'mod_access_ban_postreply' => '禁止回复',
  'mod_access_ban_postthread' => '禁止发表文章',
  'mod_access_ban_upload' => '禁止上传附件',
  'mod_access_ban_uploadimage' => '禁止上传图片',
  'mod_access_ban_viewthread' => '禁止查看文章',
  'mod_access_change' => '权限变更',
  'mod_access_download' => '下载附件',
  'mod_access_getimage' => '查看图片',
  'mod_access_optime' => '操作时间',
  'mod_access_postreply' => '回复',
  'mod_access_postthread' => '发表文章',
  'mod_access_recover' => '恢复默认',
  'mod_access_specialuser' => '特殊用户',
  'mod_access_upload' => '上传附件',
  'mod_access_uploadimage' => '上传图片',
  'mod_access_viewthread' => '浏览文章',
  'mod_message_access_admin_invalid' => '管理员设置此用户某些权限为强制允许，你不能变更管理员的这些设置',
  'mod_message_access_nonexistence' => '当前没有特殊权限用户',
  'mod_message_access_updatepermission' => '用户权限更新成功, 请继续操作',
  'mod_message_access_user_invalid' => '抱歉，你没有权限操作管理人员或特殊用户！',
  'mod_message_access_user_nonexistence' => '此用户不存在或被冻结',
  'mod_moderate_selectforum' => '版块选择',
  'mod_notice_access' => '此处可以限制某个用户在某版块的权限。看帖是基本权限，一旦禁止, 其他权限会同时进行禁止。',
  'moderator' => '版主',
  'admin' => '管理员',
  'days' => '天',
  'later' => '后',
  'expire' => '有效期',
  'unexpire' => '失效',
  'request' => '请求',
  'modcp_home_adminnote_add' => '添加留言',
  'modcp_home_adminnote_nonexistence' => '当前没有人留言',
  'modcp_home_adminnote_to' => '留言给',
  'modcp_home_message_list' => '留言列表',
  'supermod' => '超级版主',
  'modcp_logs_action' => '操作',
  'modcp_logs_list_1' => '日志列表',
  'modcp_logs_other' => '其他',
  'modcp_logs_perpage' => '每页显示条数',
  'modcp_logs_search' => '查找',
  'add_new' => '新增',
  'avatar_del' => '删除头像',
  'bio' => '自我介绍',
  'changeto' => '变更为',
  'crime_action' => '操作处罚',
  'crime_avatar' => '清除头像',
  'crime_banpost' => '屏蔽回复',
  'crime_banspeak' => '禁言',
  'crime_banstatus' => '锁定用户',
  'crime_banvisit' => '禁止访问',
  'crime_customstatus' => '清除自定义头衔',
  'crime_dateline' => '操作时间',
  'crime_delpost' => '删除',
  'crime_operator' => '风纪委员',
  'crime_reason' => '理由',
  'crime_record' => '违规记录',
  'crime_sightml' => '清除签名',
  'crime_warnpost' => '警告回复',
  'crime_nolog' => '(ง •̀_•́)ง  没有不良纪录 (*•̀ㅂ•́)و',
  'expiry' => '期限',
  'ip_location' => '地理位置',
  'mod_member_ban' => '禁止用户',
  'mod_member_edit' => '编辑用户',
  'mod_message_goto_admincp' => '<u>请点击这里进入管理后台继续操作</u>',
  'mod_message_member_nonexistence' => '该用户不存在或被冻结，请重新输入',
  'mod_message_member_nopermission' => '控制面板无权操作该用户',
  'mod_message_member_search' => '请首先输入用户名或者 UID 搜索用户，然后进行下一步。搜索 UID 比搜索用户名速度更快且准确',
  'modcp_ban_ip' => '已禁止的 IP 列表',
  'modcp_ip_message' => '* 表示整个 IP 段，如 192.168.*.* 表示禁止所有以 192.168 开头的 IP',
  'modcp_members_ban_days_comment' => '期限设置仅对禁言和禁止访问的操作有效',
  'modcp_members_ip_addadmin' => '操作者',
  'modcp_members_ip_error_1' => '请输入标准 IP 地址',
  'modcp_members_ip_error_2' => '新增 IP 地址失败，该 IP 地址范围包含你目前的 IP',
  'modcp_members_ip_error_3' => '新增 IP 地址失败，该 IP 地址已经在禁止范围内',
  'modcp_members_ip_error_4' => '你只能编辑和删改自己添加的 IP 地址',
  'modcp_members_ip_succed' => 'IP 地址更新成功，请继续操作',
  'modcp_members_status_banpost' => '禁言',
  'modcp_members_status_banpost2' => '不能参与评论',
  'modcp_members_status_banvisit' => '禁止访问',
  'modcp_members_status_banvisit2' => '不能查看任何内容',
  'modcp_members_status_normal' => '正常',
  'modcp_members_status_normal2' => '态度良好，可以正常活动',
  'no_ban_ip' => '当前没有已禁止的 IP',
  'online_ip' => 'IP 地址',
  'optional' => '可选',
  'reason' => '理由',
  'signature' => '个人签名',
  'valid_before' => '之前',
  'ignore' => '忽略',
  'invalidate' => '否决',
  'mod_message_moderate_nopermission' => '抱歉，你没有管理任何版块的权限，无法执行此操作',
  'mod_moderate_ignorereply_num' => '已忽略回复',
  'mod_moderate_ignorethread_num' => '已忽略文章',
  'mod_moderate_member_already' => '已否决的用户',
  'mod_moderate_member_info' => '审核信息',
  'mod_moderate_member_mod_admin' => '上次审核者',
  'mod_moderate_member_mod_dateline' => '上次审核时间',
  'mod_moderate_member_never' => '待审核的用户',
  'mod_moderate_member_profile' => '个人资料',
  'mod_moderate_member_range' => '用户范围',
  'mod_moderate_member_register_dateline' => '注册时间',
  'mod_moderate_member_register_ip' => '注册 IP',
  'mod_moderate_member_register_reason' => '注册理由',
  'mod_moderate_member_submit_dateline' => '上次提交',
  'mod_moderate_member_submit_times' => '次注册',
  'mod_moderate_reply_num' => '未审核回复',
  'mod_moderate_select' => '当前已选定 <span id="modlayercount">0</span> 个',
  'mod_moderate_thread_num' => '未审核文章',
  'mod_moderate_thread_range' => '回复范围',
  'mod_notice_moderate' => '审核结果: $modpost[\'validate\'] 个回复审核通过，$modpost[\'delete\'] 篇回复删除，$modpost[\'ignore\'] 篇回复进入忽略列表等待审核',
  'mod_option_moduser' => '用户',
  'mod_option_subject_mod' => '审核',
  'mod_option_subject_modreplies' => '回复',
  'mod_option_subject_modthreads' => '文章',
  'mod_select_invalid' => '请先选择操作对象！',
  'pass' => '通过',
  'table_branch' => '分表：',
  'validate' => '通过',
  'mod_moderate_member' => '审核用户',
  'mod_moderate_member_sendemail' => '发 Email 通知被审核用户',
  'mod_moderate_nonexistence' => '抱歉，你没有选择任何内容，请返回',
  'mod_moderate_reason' => '理由 (可选)',
  'mod_moderate_reply' => '审核回复',
  'mod_moderate_thread' => '审核文章',
  'mod_option_selectforum' => '版块选择',
  'mod_option_subject_delete' => '文章管理',
  'mod_option_subject_forum' => '版块文章',
  'mod_option_subject_recyclebin' => '文章回收站',
  'mod_option_subject_recyclebinpost' => '回复回收站',
  'modcp_forum' => '当前版块',
  'modcp_posts_author' => '回复人',
  'modcp_posts_dateline_range' => '时间范围',
  'modcp_posts_error_1' => '搜索条件不足！你至少应当在 关键字，作者或者 IP 当中设置一个搜索的条件',
  'modcp_posts_error_2' => '时间范围错误！版主只能删除近 1 周的文章，超级版主可以删除 2 周内的文章，请重新选择开始时间',
  'modcp_posts_error_3' => '你输入的关键字不合法！每个关键字至少由 2 个汉字或者 4 个英文字符组成',
  'modcp_posts_error_4' => '抱歉，你无权使用批量删帖功能',
  'post' => '回复内容',
  'modcp_posts_ip' => '回复IP',
  'modcp_posts_keyword' => '内容关键字',
  'modcp_posts_member_credits' => '不更新用户积分',
  'modcp_posts_search' => '共搜索出结果 <strong>$total</strong> 条',
  'modcp_posts_thread' => '文章',
  'modcp_posts_threadfirst' => '文章首帖',
  'modcp_posts_threadreply' => '文章回复帖',
  'modcp_posts_to' => '至',
  'modcp_posts_type' => '文章类型',
  'modcp_posts_week_1' => '你只能操作最近 1 周的文章',
  'modcp_posts_week_2' => '你只能操作最近 2 周的文章',
  'modcp_select_forum' => '请选择版块',
  'posttable_branch' => '文章分表:',
  'mod_option_selectthreadclass' => '文章分类',
  'modcp_dateline_range' => '发表时间范围',
  'modcp_forum_select_msg' => '请选择版块进行管理',
  'modcp_no_reply_range' => '多少天内无回复',
  'modcp_reply_range' => '回复次数范围',
  'modcp_restore' => '恢复',
  'modcp_select_threadclass' => '请选择文章分类',
  'modcp_subject_keyword' => '标题关键字',
  'modcp_thread_msg' => '没有找到相关文章',
  'modcp_threadstick_1' => '置顶I',
  'modcp_threadstick_2' => '置顶II',
  'modcp_threadstick_3' => '置顶III',
  'modcp_views_range' => '点击次数范围',
  'none' => '无',
  'dateline' => '发布时间',
  'ishtmlon' => 'HTML帖',
  'no' => '否',
  'yes' => '是',
  'modcp_report_content' => '内容',
  'modcp_report_note' => '给举报者留个言吧(*•̀ㅂ•́)و',
  'modcp_report_nothing' => '没有新的举报或没有选择版块',
  'modcp_report_perpage' => '每页显示举报数',
  'modcp_report_post' => '举报回复',
  'modcp_report_reason' => '举报理由',
  'modcp_report_reporter' => '举报者',
  'modcp_report_resolve' => '处理选中',
  'modcp_report_reward' => '奖惩',
  'modcp_report_time' => '举报时间',
  'modcp_report_waiting' => '等待处理的举报',
  'modcp_thread_search_msg' => '共搜索出结果 <strong>$total</strong> 条',
  'pay' => '购买文章',
  'pay_nobuyers' => '目前没有用户购买此文章',
  'draftbox' => '草稿箱',
  'edit_save' => '保存',
  'edit_thread' => '编辑',
  'edit_trade' => '编辑商品',
  'faq' => '帮助',
  'join_thread' => '回复',
  'post_credits_rule' => '积分规则',
  'post_message2' => '继续添加商品',
  'post_message3' => '查看所有草稿',
  'reply_quote' => '引用',
  'replycredit_revenue' => '税后支付',
  'save_draft' => '保存草稿',
  'trade_add_post' => '添加商品',
  'activity_city' => '所在城市',
  'activity_need_member' => '需要人数',
  'activity_starttime_endtime' => '时间范围',
  'activiy_sort' => '活动名',
  'consumption_credit' => '消耗积分',
  'input_invalid' => '填写无效',
  'optional_data' => '必填资料项',
  'other_data' => '扩展资料项',
  'post_activity_message' => '每行代表一个项目，最多',
  'post_click_message_1' => '点击更新来更换你的图片',
  'post_click_message_2' => '添加一张好看的图片，让活动更吸引人',
  'post_closing' => '报名截止',
  'post_error_message_1' => '抱请输入活动时间',
  'post_error_message_2' => '请输入活动地点',
  'post_error_message_3' => '请输入活动名',
  'post_event_time' => '活动时间',
  'post_option' => '项',
  'post_topic_image' => '活动图片',
  'user_consumption_credit' => '消耗',
  'attachment_limit' => '你今日还能上传',
  'attachment_or' => '或者',
  'attachment_outoflimit' => '今日你已无法上传更多的附件',
  'attachment_sizelimit' => '总大小 <strong>$allowuploadsize</strong> 以内的文件',
  'attachment_unit' => '<strong>$allowuploadnum</strong> 个文件',
  'debate_opponent_point' => '蓝方观点',
  'debate_square_point' => '红方观点',
  'debate_umpire' => '裁判',
  'post_debate_message_1' => '请填写红方观点',
  'post_debate_message_2' => '请填写蓝方观点',
  'auto_keyword' => '自动获取',
  'choosetag' => '搜索标签',
  'addtag' => '更新标签',
  'credits' => '积分',
  'min_limit' => '下限',
  'post_price_charge_comment' => '文章最多能销售 {$_G[setting][maxchargespan]} 个小时',
  'post_price_comment' => '最高 {$_G[group][maxprice]} {$_G[setting][extcredits][$_G[setting][creditstransextra][1]][unit]}',
  'post_price_free_chargehours' => '，本文章还能销售 $freechargehours 个小时',
  'post_price_income_comment' => '文章出售最高收入上限为 {$_G[setting][maxincperthread]} {$_G[setting][extcredits][$_G[setting][creditstransextra][1]][unit]}&nbsp;&nbsp;&nbsp;&nbsp;',
  'post_rushreply_credit' => '大于此设置才能参与抢楼，可不填',
  'post_tag' => '标签',
  'post_timer' => '定时发布',
  'posttag' => '文章标签',
  'posttag_comment' => '用逗号或空格隔开多个标签，最多可填写 5 个',
  'recent_use_tag' => '最近使用过的标签',
  'replycredit_empty' => '(留空或填 0 为不奖励)',
  'replycredit_however' => '本文章尚有',
  'replycredit_member' => '每人最多可获得',
  'replycredit_once' => '每次回复奖励:',
  'replycredit_rate' => '中奖率',
  'replycredit_reward' => '奖励',
  'replycredit_time' => '次',
  'replycredit_total' => '回复奖励总额:',
  'rushreply_change' => '设为抢楼文章',
  'rushreply_end' => '截止楼层',
  'rushreply_rewardfloor' => '奖励楼层',
  'rushreply_rewardfloor_comment' => '楼层用英文逗号隔开,*号可匹配任意数如 *88',
  'rushreply_time' => '抢楼时间',
  'total_credits' => '总积分',
  'you_have' => '你有',
  'code' => '代码模式',
  'e_attach' => '附件',
  'e_attach_title' => '添加附件',
  'e_audio' => '音乐',
  'e_audio_title' => '添加音乐',
  'e_autotypeset' => '自动排版',
  'e_center' => '居中',
  'e_code_title' => '添加代码文字',
  'e_editor_loading' => '请稍后 ...',
  'e_flash' => 'Flash',
  'e_flash_title' => '添加 Flash',
  'e_beginning' => '动画',
  'e_beginning_title' => '添加起始动画',
  'e_floatleft' => '左浮动',
  'e_floatright' => '右浮动',
  'e_font' => '字体',
  'e_fontbgcolor' => '设置文字背景色',
  'e_fontname' => '设置字体',
  'e_fontsize' => '设置文字大小',
  'e_free' => '添加免费信息',
  'e_hide' => '添加隐藏内容',
  'e_hr_title' => '分隔线',
  'e_image_title' => '添加图片',
  'e_italic' => '文字斜体',
  'e_left' => '居左',
  'e_orderedlist' => '排序的列表',
  'e_quote_title' => '引用',
  'e_removeformat' => '清理格式',
  'e_right' => '居右',
  'e_size' => '大小',
  'e_smilies' => '表情',
  'e_smilies_title' => '添加表情',
  'e_table' => '添加表格',
  'e_underline' => '文字加下划线',
  'e_unlink' => '移除链接',
  'e_unorderedlist' => '未排序列表',
  'e_video' => '视频',
  'e_video_title' => '添加视频',
  'missed_data' => '你有上次未提交成功的数据',
  'approve' => '需审核',
  'attach_delete' => '删除',
  'attach_unused' => '个未使用的附件',
  'attach_use' => '使用',
  'attach_view' => '查看',
  'debate_neutral' => '中立',
  'debate_viewpoint' => '选择立场',
  'draft' => '草稿',
  'img_unused' => '个未使用的图片',
  'modify' => '修改',
  'post_message1' => '添加商品信息，完成后可在本帖中继续添加多个商品',
  'addfeed' => '发送动态',
  'auditstatuson' => '通过审核',
  'del_thread_warning' => '删除后无法撤销',
  'disable' => '禁用',
  'hiddenreplies' => '回复仅作者可见',
  'post_additional_options' => '附加选项',
  'post_anonymous' => '使用匿名回复',
  'post_delpost' => '删除内容',
  'post_descviewdefault' => '回复倒序排列',
  'post_digest_thread' => '蜜制回复',
  'post_imgurl' => '解析图片链接',
  'post_noticeauthor' => '接收回复通知',
  'post_parseurl' => '链接识别',
  'post_show_sig' => '使用个人签名',
  'post_stick_thread' => '文章置顶',
  'reward_price_back' => '，返还悬赏费用，不退还手续费',
  'smilies' => '表情',
  'nav_forum' => '进入版块',
  'nav_forum_frequently' => '常用版块',
  'post_forum_navigation' => '论坛导航',
  'threadtype_option' => '分类信息',
  'poll_after_result' => '投票后结果可见',
  'poll_close' => '关闭投票',
  'poll_finish' => '已结束',
  'poll_rate' => '获票率',
  'post_poll_add' => '继续添加',
  'post_poll_allowmultiple' => '最多可选',
  'post_poll_comment' => '最多可填写 {$_G[setting][maxpolloptions]} 个选项',
  'post_poll_comment_s' => '每行填写 1 个选项',
  'post_poll_expiration' => '记票天数',
  'post_poll_options' => '选项',
  'post_poll_overt' => '公开投票参与人',
  'post_single_frame_mode' => '单框模式',
  'post_reward_error_message' => '抱歉，请输入悬赏价格',
  'post_reward_message' => '天后如果你仍未设置最佳答案，管理员有权代为你选择',
  'post_reward_resolved' => '已解决的悬赏',
  'reward_cant_fall' => '不能降低悬赏积分',
  'reward_price' => '悬赏',
  'reward_price_bound' => '售价超出范围',
  'reward_price_max' => '最高',
  'reward_price_min' => '最低',
  'reward_price_overflow' => '售价不能高于 32767',
  'reward_tax_add' => '税后追加',
  'reward_tax_after' => '税后支付',
  'five_days' => '5天',
  'half_year' => '半年',
  'maxlength' => '最大长度',
  'maxnum' => '最大值',
  'minnum' => '最小值',
  'one_month' => '1个月',
  'one_year' => '1年',
  'please_select' => '请选择',
  'seven_days' => '7天',
  'threadtype_description' => '回复说明',
  'threadtype_expiration' => '信息有效期',
  'three_days' => '3天',
  'three_months' => '3个月',
  'unchangeable' => '不可修改',
  'post_current_credit' => '售价积分',
  'post_current_price' => '售价(人民币)',
  'post_goods_error_message_1' => '请输入商品名称',
  'post_goods_error_message_2' => '请输入商品数量',
  'post_goods_error_message_3' => '请输入商品价格',
  'post_original_credit' => '原积分',
  'post_original' => '原',
  'post_original_price' => '原价',
  'post_trade_locus' => '所在地点',
  'post_trade_name' => '商品',
  'post_trade_number' => '数量',
  'post_trade_paymethod' => '交易方式',
  'post_trade_paymethod_offline' => '担保交易',
  'post_trade_paymethod_online' => '在线交易',
  'post_trade_picture' => '商品图片',
  'post_trade_price' => '商品价格',
  'post_trade_tenpay_seller' => '财付通账户',
  'post_trade_transport' => '物流方式',
  'post_trade_transport_buyer' => '不包邮',
  'post_trade_transport_express' => '快递',
  'post_trade_transport_mail' => '平邮',
  'post_trade_transport_offline' => '面议',
  'post_trade_transport_seller' => '包邮',
  'post_trade_transport_virtual' => '虚拟物品',
  'trade_new' => '全新',
  'trade_old' => '二手',
  'trade_quality' => '商品品质',
  'trade_no_intro' => '这人是不是傻 ( ∙̆ .̯ ∙̆ ) ，卖东西居然不写简介！快去提醒一下他',
  'trade_talk' => '联系卖家',
  'trade_type_transport_physical' => '货到付款',
  'postappend' => '补充',
  'admin_operation_explain' => '操作说明',
  'admin_rate' => '投币扣除自身相应积分',
  'click_here' => '点击这里',
  'push_succeed' => '推送成功',
  'gratuitie' => '投币',
  'rate' => '投币',
  'rate_raterange' => '投币区间',
  'rate_thread' => '给本文章加分',
  'rate_todayleft' => '今日剩余',
  'thread_removerate' => '撤销投币',
  'user_operation_explain' => '可选投币理由',
  'user_operation_tape' => '或者在这里发自内心哟｡:.ﾟヽ(｡◕‿◕｡)ﾉﾟ.:｡+ﾟ',
  'rate_view' => '查看投币纪录',
  'stats' => '站点统计',
  'stats_main' => '基本概况',
  'stats_main_admins' => '管理成员',
  'stats_main_board_activity' => '论坛活跃指数',
  'stats_main_forums_count' => '版块数',
  'stats_main_hot_forum' => '最热门的版块',
  'stats_main_member' => '会员统计',
  'stats_main_members' => '注册会员',
  'stats_main_members_count' => '今日新增会员数',
  'stats_main_new' => '新会员',
  'stats_main_nmpd' => '平均每日注册会员数',
  'stats_main_nonposters' => '未回复会员',
  'stats_main_nppd' => '平均每日新增回复数',
  'stats_main_posters' => '回复会员',
  'stats_main_posters_percent' => '回复会员占总数',
  'stats_main_posts_avg' => '平均每人回复数',
  'stats_main_posts_count' => '回复数',
  'stats_main_rpt' => '平均每个文章被回复次数',
  'stats_main_threads_count' => '文章数',
  'stats_main_todays_newposts' => '今日新增回复数',
  'stats_main_topposter' => '今日论坛之星',
  'stats_main_total_posted' => '回复数',
  'stats_update' => '统计数据已被缓存，上次于 $lastupdate 被更新，下次将于 $nextupdate 进行更新',
  'lastvisit' => '上次访问',
  'member_list' => '会员列表',
  'posts' => '回复',
  'threads' => '文章',
  'regdate' => '注册日期',
  'uid' => 'UID',
  'stats_agent' => '客户软件',
  'stats_browser' => '浏览器',
  'stats_date' => '日期',
  'stats_day_posts' => '每日新增回复记录',
  'stats_forum_stat_log' => '版块统计日志',
  'stats_forums_forumname' => '版块名称',
  'stats_forums_month' => '月份',
  'stats_forums_stat' => '版块统计',
  'stats_forums_sub' => '子版',
  'stats_history' => '历史记录',
  'stats_hour' => '时段流量',
  'stats_modworks' => '管理统计',
  'stats_modworks_all' => '全体管理人员',
  'stats_modworks_data' => '的管理统计数据',
  'stats_modworks_details' => '查看详细管理统计',
  'stats_modworks_export' => '导出',
  'stats_modworks_in' => '在',
  'stats_modworks_timerange' => '时间范围',
  'stats_modworks_to' => '至',
  'stats_modworks_total' => '合计',
  'stats_month_posts' => '每月新增回复记录',
  'stats_os' => '操作系统',
  'stats_posthist' => '回复量记录',
  'stats_views' => '流量统计',
  'stats_week' => '星期流量',
  'admin_status' => '管理级别',
  'admin_usergroup_title' => '组头衔',
  'hours' => '小时',
  'onlinetime_thismonth' => '本月在线',
  'onlinetime_total' => '总计在线',
  'stats_modworks_thismonth' => '本月管理',
  'stats_posts_thismonth' => '最近 30 天回复',
  'stats_team' => '管理团队',
  'stats_team_admins' => '管理员和超级版主',
  'stats_team_offdays' => '离开天数',
  'stats_trade_rank' => '交易排行',
  'trace_number_sort' => '交易数排行',
  'trace_sell_number' => '售出数量',
  'trade_price_sort' => '交易额排行',
  'trade_seller' => '卖家',
  'trade_totalprice' => '总金额',
  'none_tag' => '没有相关标签',
  'tag_search' => '搜索最多显示 50 个，点击结果链接可直接插入标签',
  'admin_bump' => '提升文章',
  'admin_close' => '冻结文章',
  'admin_delthread_confirm' => '删除选中文章',
  'admin_delthread_nopermission' => '你没有删除此文章权限',
  'admin_digest_add' => '蜜制',
  'admin_digest_remove' => '解除',
  'admin_down' => '下沉文章',
  'admin_move' => '移动文章',
  'admin_move_hold' => '保留转向',
  'admin_open' => '解冻文章',
  'admin_select_piece' => '选择了 $modpostsnum 篇回复',
  'admin_copy_target' => '复制文章到目标板块',
  'admin_move_target' => '移动文章',
  'admin_target' => '目标版块',
  'admin_targettype' => '目标分类',
  'admin_type_msg' => '当前版块无分类设置，请联系管理员到后台设置文章分类',
  'admin_unrecommend' => '解除',
  'bump' => '提升',
  'crimerecord' => '违规登记',
  'forum_recommend_image' => '图片',
  'forum_recommend_noimage' => '不显示',
  'forum_recommend_reducetitle' => '标题',
  'recommend' => '推荐',
  'thread_highlight' => '高亮',
  'topicadmin_crime_delpost_nums' => '用户 $crimeauthor 的发言已因违规被删除 $crimenum 次',
  'types' => '分类',
  'admin_change_types' => '切换文章分类',
  'admin_banpost' => '屏蔽',
  'admin_delposts' => '删除选定回复',
  'admin_merge' => '合并文章',
  'admin_merge_span' => '文章ID',
  'admin_merge_tid' => '填写要合并的文章 ID',
  'admin_select_one_piece' => '选择了 1 篇文章',
  'admin_split_comment' => '填写楼号',
  'admin_split_comment2' => '用 "," 间隔',
  'admin_split_newsubject' => '新标题',
  'admin_split_action' => '分割楼层(部分评论和文章将会分离)',
  'admin_stamp_none' => '无图章',
  'admin_stamp_select' => '选择文章图章',
  'admin_stamplist_current' => '当前图标',
  'admin_stamplist_none' => '无图标',
  'admin_stamplist_select' => '选择文章图标',
  'admin_stickreply' => '置顶到文章',
  'admin_threadsplit_restore' => '你确认要将此文章移除存档区吗？',
  'admin_unbanpost' => '解除',
  'admin_unstickreply' => '解除置顶',
  'admin_live' => '直播',
  'admin_live_cancle' => '取消直播',
  'admin_live_tips' => '同一版块内只能设置一个直播帖，设置直播后会覆盖原有直播帖',
  'pay_buyers' => '已购买人数',
  'topicadmin_crime_banpost_nums' => '用户 $crimeauthor 回复已被屏蔽 $crimenum 次',
  'topicadmin_delet_comment' => '删除选定评论',
  'topicadmin_select_comment' => '选择了 1 个评论',
  'topicadmin_warn_add' => '警告',
  'topicadmin_warn_delete' => '解除',
  'topicadmin_warn_nums' => '用户 $warningauthor 已被警告 $authorwarnings 次',
  'topicadmin_warn_prompt' => '{$_G[setting][warningexpiration]} 天内累计被警告 {$_G[setting][warninglimit]} 次，将被自动禁止回复 {$_G[setting][warningexpiration]} 天',
  'topicadmin_opt' => '操作',
  'topicadmin_useropt' => '用户操作',
  'topicadmin_banuser' => '禁言',
  'topicadmin_userdelpost' => '清空用户全部发言',
  'admin_ban_this_ip' => '禁封IP',
  'admin_user_this_ip' => '查看IP',
  'admin_bump_down' => '提升/下沉',
  'admin_openclose' => '冻结/解冻',
  'admin_select' => '选中',
  'maximize' => '最大化',
  'minimize' => '最小化',
  'piece' => '篇',
  'topicadmin_recommend_forum' => '推荐到版块',
  'post_trade_sticklist' => '推荐商品',
  'post_trade_stick' => '荐',
  'post_trade_transport_physical' => '货到付款',
  'post_trade_transport' => '配送方式',
  'post_trade_transport_customer' => '买家承担邮费',
  'trade_intro' => '商品详情',
  'trade_comments' => '评论',
  'trade_additional' => '附加',
  'trade_buy_confirm' => '确认购买',
  'trade_buy_crediterror' => '<span class="heart-pink">你的积分不足</span>',
  'trade_buyercontact' => '收货地址',
  'trade_buyermobile' => '收货人手机',
  'trade_buyername' => '收货人姓名',
  'trade_buyerphone' => '收货人电话',
  'trade_buyerzip' => '收货人邮编',
  'trade_confirm_buy' => '确认购买信息',
  'trade_confirm_goods' => '1.确认商品',
  'trade_credits_total' => '支付总额',
  'trade_guest_alarm' => '你目前为游客，购买后你无法在本版块查看交易状态，请到支付网站查询',
  'trade_nums' => '购买数量',
  'trade_pay_alipay' => '在线交易',
  'trade_pay_offline' => '担保交易',
  'trade_paymethod' => '交易方式',
  'trade_price' => '售价',
  'trade_recommended_goods' => '推荐的商品',
  'trade_seller_remark' => '备注信息',
  'trade_seller_remark_comment' => '200 字以内',
  'trade_units' => '元',
  'save' => '保存',
  'trade_displayorder' => '柜台商品管理',
  'trade_list' => '商品列表',
  'trade_hour' => '小时',
  'trade_remaindays' => '剩余时间',
  'trade_forever' => '永久',
  'trade_show_order' => '显示顺序',
  'trade_timeout' => '成交结束',
  'trade_update_stick' => '推荐',
  'trade_update_stickmax' => '你最多可推荐的商品数:',
  'attach_nopermission' => '你所在的用户组无法下载或查看附件',
  'eccredit_buyerinfo' => '买家信用',
  'eccredit_sellerinfo' => '卖家信用',
  'on_line' => '在线',
  'pack_up' => '收起',
  'post_trade_buynumber' => '已售',
  'post_trade_removed' => '这个商品已下架 (´,,•ω•,,‘)',
  'post_trade_support_tenpay' => '此商品支持财付通，你可以先验货后付款',
  'post_trade_transport_none' => '无运费',
  'sold_out' => '已售完',
  'taobao' => '阿里旺旺',
  'trade' => '商品',
  'trade_bargain' => '联系卖家',
  'trade_costprice' => '原价',
  'trade_locus' => '地点',
  'trade_other_goods' => '柜台其它商品',
  'trade_rate' => '信用评价',
  'trade_seller_other_goods' => '$trade[seller] 的其它商品',
  'trade_seller_real_name' => '卖家实名',
  'trade_transport' => '运费',
  'trade_type_buy' => '商品',
  'trade_type_viewthread' => '商品类型',
  'trade_viewtrade' => '查看商品',
  'user_threads' => '查看文章',
  'eccredit1' => '评价',
  'eccredit_post_already' => '对方已评',
  'eccredit_post_between' => '双方已评',
  'eccredit_post_waiting' => '等待对方评价',
  'tenpay_trade_order_status' => '查看并确认财付通交易单状态',
  'trade_baseprice' => '商品价格',
  'trade_buyer' => '买家',
  'trade_message' => '留言',
  'trade_online_tenpay' => '使用财付通在线支付',
  'trade_online_tradeurl' => '在线交易单',
  'trade_order' => '交易单',
  'trade_order_no' => '交易单号',
  'trade_order_status' => '查看并确认支付宝交易单状态',
  'trade_password' => '登录密码',
  'trade_payment' => '交易金额',
  'trade_payment_comment' => '(点击下方的“更新交易单”按钮重新计算交易金额)',
  'trade_submit_order' => '更新交易单',
  'trade_transportfee' => '运费',
  'upload_selectfile' => '浏览',
  'poll_select_option' => '选择相应的投票项：(列出了有投票的选项,可多选,不选表示给所有投票用户加标签)',
  'set_tag_log' => '添加标签记录',
  'set_tag_to_activity_users' => '给活动参与者贴标签',
  'set_tag_to_poll_users' => '给参与投票的会员贴标签',
  'set_tag_to_reply_users' => '给参与回复的会员贴标签',
  'follow' => '关注TA',
  'have_ignored' => '已忽略',
  'i_want' => '我要',
  'last_thread' => '上一文章',
  'moderating' => '审核中',
  'modmenu_archive' => '取消存档',
  'modmenu_banpost' => '屏蔽',
  'modmenu_banthread' => '屏蔽',
  'modmenu_copy' => '复制',
  'modmenu_deletepost' => '删除',
  'modmenu_deletethread' => '删除文章',
  'modmenu_digestpost' => '蜜制',
  'modmenu_grouprecommend' => '推到版块',
  'modmenu_live' => '直播',
  'modmenu_highlight' => '高亮',
  'modmenu_icon' => '图标',
  'modmenu_merge' => '合并',
  'modmenu_move' => '移动',
  'modmenu_pusharticle' => '生成文章',
  'modmenu_pushplus' => '文章连载',
  'modmenu_recommend' => '推荐',
  'modmenu_removereward' => '移除悬赏',
  'modmenu_repair' => '修复',
  'modmenu_restore' => '撤销付费',
  'modmenu_split' => '分割',
  'modmenu_stamp' => '图章',
  'modmenu_stickpost' => '置顶',
  'modmenu_stickthread' => '置顶',
  'modmenu_switch_off' => '冻结',
  'modmenu_switch_on' => '解冻',
  'modmenu_type' => '分类',
  'modmenu_updown' => '升降',
  'modmenu_warn' => '警告',
  'next_thread' => '下一文章',
  'nofollow' => '不关注',
  'return_forumdisplay' => '返回列表',
  'rushreply_view' => '查看中奖楼层',
  'share_url_copy' => '复制链接',
  'share_url_copy_comment' => '你的朋友访问此链接后，你将获得相应的积分奖励',
  'thread_author' => '作者',
  'thread_printable' => '打印',
  'thread_replycredit_tips1' => '回复本文章可获得 {$_G[forum_thread][replycredit_rule][extcredits]} {$_G[setting][extcredits][$_G[forum_thread][replycredit_rule][extcreditstype]][unit]}{$_G[setting][extcredits][$_G[forum_thread][replycredit_rule][extcreditstype]][title]}奖励!',
  'thread_replycredit_tips2' => '每人限 {$_G[forum_thread][replycredit_rule][membertimes]} 次',
  'thread_replycredit_tips3' => '(中奖概率 {$_G[forum_thread][replycredit_rule][random]}%)',
  'thread_rushreply' => '本文章为抢楼帖，欢迎抢楼!',
  'thread_rushreply_check_back' => '返回抢楼帖',
  'thread_rushreply_end' => '截止楼层',
  'thread_rushreply_floor' => '奖励楼层',
  'thread_rushreply_limit' => '本文章为抢楼帖，{$rushresult[creditlimit_title]}大于{$rushresult[creditlimit]}可以抢楼',
  'thread_rushreply_over' => '抢楼结束：',
  'thread_rushreply_rewardnum' => '个楼层已中奖',
  'thread_rushreply_noreward' => '暂时还没有楼层中奖',
  'thread_rushreply_start' => ' 抢楼开始：',
  'thread_rushreply_statnum' => '统计参与人数',
  'usertag' => '用户标签',
  'activity_about_member' => '剩余名额',
  'activity_enter_imgurl' => '请输入图片地址',
  'activity_imgurl_error' => '图片地址错误',
  'activity_join' => '我要参加',
  'activity_join_audit' => '你已经参加了此活动',
  'activity_join_cancel' => '取消报名',
  'activity_join_group' => '马上加入{$_G[setting][navs][3][navname]}',
  'activity_need_credit' => '注意：除实际消费以外，参加此活动将扣除你',
  'activity_new_signup' => '待审核',
  'activity_no_member' => '你还不是本 {$_G[setting][navs][3][navname]} 的成员不能参与此活动',
  'activity_pay_myself' => '自费',
  'activity_paytype' => '消费方式',
  'activity_type' => '活动',
  'activity_wait' => '你的加入申请已发出，请等待发起者审批',
  'activity_closed' => '活动已经结束了 ╮(๑•́ ₃•̀๑)╭',
  'activity_cantjoin' => '活动报名截至期已过 (..•˘_˘•..)',
  'activity_would_payment' => '上交',
  'activiy_guest_more' => '你的留言超过 200 个字符的限制',
  'complete_data' => '完善资料',
  'manage' => '管理',
  'next_page' => '下一页',
  'next_page_extra' => '下一页 &raquo;',
  'pm_archive' => '导出',
  'debate_all_point' => '全部观点',
  'debate_comment_dateline' => '评判时间',
  'debate_join' => '参辩',
  'debate_support' => '支持',
  'debate_umpire_end' => '结束此次辩论',
  'debate_umpirepoint_edit' => '编辑裁判观点',
  'debater' => '辩手',
  'login_to_reply' => '回复之前你需要先有个身份&nbsp;&nbsp;๑乛◡乛๑&nbsp;&nbsp;',
  'post_fastreply_gotolast' => '回复后跳转到最后一页',
  'post_newreply' => '回复',
  'post_reply_relay' => '回复并转播',
  'debate_view_neutral' => '只看中立',
  'debate_view_opponent' => '只看蓝方',
  'debate_view_square' => '只看红方',
  'post_add_aboutcounter' => '添加柜台介绍',
  'expiration_unlimit' => '永 久',
  'thread_moderations' => '文章操作记录',
  'thread_moderations_action' => '操作',
  'thread_moderations_cron' => '任务系统',
  'thread_moderations_username' => '操作者',
  'to' => '到',
  'ban_member' => '禁止',
  'clear' => '清理',
  'collection_fromctid' => '你是从专辑 <a href="forum.php?mod=collection&action=view&ctid=$post[sourcecollection][ctid]" target="_blank">$post[sourcecollection][name]</a> 访问到本文章的，欢迎对专辑打分：',
  'collection_rate' => '投币',
  'collection_related' => '本文章被以下专辑推荐:',
  'debate_filter' => '按立场筛选',
  'follower' => '粉丝',
  'from_mobile' => '来自手机',
  'ignore_replynotice' => '取消回复通知',
  'member_avatar_banned' => '头像被屏蔽',
  'member_deleted' => '该用户已被删除',
  'member_homepage' => '查看个人网站',
  'member_signature_banned' => '签名被屏蔽',
  'member_viewpro' => '查看详细资料',
  'modmenu_blockrecommend' => '推送',
  'offline' => '当前离线',
  'online' => '当前在线',
  'people_score' => '人投币',
  'post_ascview' => '正序浏览',
  'post_copied' => '回复地址复制成功',
  'post_descview' => '倒序浏览',
  'post_from_mobile' => '该帖通过手机客户端发布',
  'prosit' => '恭喜',
  'published' => '发表',
  'receive_replynotice' => '接收回复通知',
  'related_thread' => '相关回复',
  'removerate' => '撤销投币',
  'rushreply_hit' => '中奖楼层',
  'rushreply_hit_title' => '查看中奖楼层',
  'thread_favorite' => '喜欢',
  'thread_magic' => '使用道具',
  'thread_mod' => '回复模式',
  'thread_mod_by' => '本文章由 $lastmod[modusername] 于 $lastmod[moddateline] $lastmod[modaction]',
  'thread_mod_recommend_by' => '本文章由 $lastmod[modusername] 于 $lastmod[moddateline] $lastmod[modaction]到 $lastmod[reason]',
  'thread_realy' => '转播',
  'thread_redirect_postno' => '电梯直达',
  'thread_redirect_postno_tips' => '跳转到指定楼层',
  'thread_share' => '分享',
  'thread_collection' => '收集',
  'thread_show_all' => '查看全部',
  'thread_show_author' => '只看Ta',
  'viewthread_left_addfriend' => '加好友',
  'viewthread_left_poke' => '打招呼',
  'viewthread_left_sendpm' => '发消息',
  'viewthread_left_tospace' => '串个门',
  'viewthread_share_to' => '分享到',
  'viewthread_share_to_qzone' => 'QQ空间',
  'viewthread_share_to_t' => '腾讯微博',
  'viewthread_share_to_pengyou' => '腾讯朋友',
  'warn_get' => '受到警告',
  'admin_message_banned' => '(..•˘_˘•..) 都说了不准发这些不可描述之物',
  'admin_message_single_banned' => '这种不可描述之物，凡人是看不见的&nbsp; (..•˘_˘•..)',
  'attach_nopermission_login' => '你需要 <a href="member.php?mod=logging&action=login" onclick="showWindow(\'login\', this.href);return false;">登录</a> 才可以下载或查看，没有帐号？<a href="member.php?mod={$_G[setting][regname]}" title="注册帐号">{$_G[setting][reglinkname]}</a>',
  'guesttipsinthread_text' => '马上注册，结交更多好友，享用更多功能，让你轻松玩转社区。',
  'attach_nopermission_connect_fill_profile' => '你需要 <a href="member.php?mod=connect">完善帐号</a> 或 <a href="member.php?mod=connect&ac=bind">绑定帐号</a> 后才可以下载或查看',
  'attach_nopermission_notice' => '这些成果可是作者的心血哦',
  'has_expired' => '该信息已经过期',
  'have' => '已有',
  'image_big' => '大图',
  'image_list_openning' => '组图打开中，请稍候......',
  'image_small' => '小图',
  'message_banned' => '这种不可描述之物，凡人是看不见的 (๑•́ ₃•̀๑٥)',
  'message_ishidden_hiddenreplies' => '这位劫友表示：看了这些东西你也会倒霉的 ꒰๑´•.̫ • `๑꒱~咩~',
  'message_single_banned' => '这种不可描述之物，凡人是看不见的 (๑•́ ₃•̀๑٥)',
  'message_password_exists' => '震惊！某不知名UP主竟然在自己的文章上装了密码锁！',
  'message_password_exists_author' => '文章已上锁，如果你没有发布你的密码，这篇文章可没人可以看得上哦 ( •̀∀•́ ) ',
  'more_images' => '更多图片',
  'pack' => '收起',
  'pay_threads' => '付费文章, 价格',
  'published_in_floor' => '查看楼层',
  'rate_total' => '总投币',
  'replies_recommended' => '回复推荐',
  'pay_comment' => '要砸 <strong>{$thread[price]} {$_G[setting][extcredits][$_G[setting][creditstransextra][1]][unit]}{$_G[setting][extcredits][$_G[setting][creditstransextra][1]][title]}</strong> 才能浏览(๑•́ ₃•̀๑٥)',
  'pay_free_time' => '截止到 <strong>$thread[endtime]</strong>，将不再收费',
  'people_buy' => '人购买',
  'poll_count_down' => '距结束还有',
  'poll_end' => '投票已经结束',
  'poll_hour' => '小时',
  'poll_minute' => '分钟',
  'poll_more_than' => '最多可选 $maxchoices 项',
  'poll_msg_allowvotepolled' => '你已经投过票，谢谢你的参与',
  'poll_msg_allowvotethread' => '该投票已经冻结或者过期，不能投票',
  'poll_msg_allwvoteusergroup' => '你所在的用户组没有投票权限',
  'poll_msg_overt' => '这是一场公开投票。你的投票结果将会公示，且在个人动态中可见',
  'poll_multiple' => '多选',
  'poll_single' => '单选',
  'poll_view_voters' => '查看投票',
  'poll_voterscount' => '共 $voterscount 票',
  'poll_voters' => '参与投票的会员',
  'poll_selectitem' => '投票选项：',
  'comment_num' => '评论数',
  'home_view_num' => '查看数',
  'latest_comments' => '最新评论',
  'portal' => '门户',
  'posted_by' => '发布者',
  'thread_print' => '打印本页',
  'welcometo' => '欢迎光临',
  'reward_answer' => '我来回答',
  'reward_bestanswer' => '最佳答案',
  'reward_set_bestanswer' => '采纳',
  'reward_outdate' => '回答已失效',
  'reward_outdate_reason' => '可能是答主犯了错被抓进小黑屋了(･ัω･ั)',
  'view_full_content' => '查看完整内容',
  'my_trade_stat' => '交易记录',
  'post_trade_totalnumber' => '商品数',
  'trade_nogoods' => '本柜台无商品',
  'viewthread_trade_message1' => '你可以在本文章中继续',
  'viewthread_trade_message2' => '添加多个商品',
  'viewthread_trade_message3' => '并可以针对帖内所有商品添加统一的',
  'viewthread_trade_message4' => '柜台介绍',
  'warn_view_log' => '$warnuser 警告记录',
  'warn_view_prompt' => '$warnuser 已被累计警告 $warnnum 次，{$_G[setting][warningexpiration]} 天内累计被警告 {$_G[setting][warninglimit]} 次，将被自动禁止回复 {$_G[setting][warningexpiration]} 天',
  'main_nav' => '主导航',
  'quick_nav' => '快捷导航',
  'post_deleted' => '无效楼层，该帖已经被删除',
  'have_posted' => '已发表',
  'guide_draft' => '草稿',
  'pending' => '待审核',
  'preview' => '预览',
  'expire_dateline' => '过期时间',
  'darkroom' => '小黑屋',
  'darkroom_no_users' => '还没有人入住小黑屋，大家都是模范会员~',
  'replystick' => '置顶回复',
  'search_forum' => '搜索版块',
  'latest_images' => '最新图片',
  'new_thread' => '新贴',
  'my_order_collection' => '我的订阅',
  'recommend_collection' => '专辑推荐',
  'rank' => '排名',
  'previous_rank' => '上次排名',
  'latest' => '最新',
  'showdisplayorder' => '显示置顶',
  'hidedisplayorder' => '隐藏置顶帖',
  'havemore_special' => '还有',
  'over' => '结束',
  'right_special' => '】',
  'like' => '喜欢',
  'onloading' => '加载中',
  'youneedpay' => '你需要支付',
  'onlyintoforum' => '才能进入此版块',
  'confirmyourpay' => '确认支付',
  'rushreply_thread' => '抢楼文章',
  'thread_pricepay' => '文章售价',
  'stopfloor' => '回复限制',
  'replylimit' => '每个用户回复次数上限',
  'basic_attr' => '文章属性',
  'text_feature' => '内容限制',
  'content_to_pic' => '内容生成图片',
  'manage_operation' => '管理文章',
  'page_separate' => '分页',
  'directory' => '目录',
  'replypassword' => '回复密码',
  'replybackground' => '回复背景',
  'pictypefile' => '图片文件',
  'backgroundcolor' => '背景色',
  'collapse_the_left' => '收起左侧',
  'open_the_left' => '开启左侧',
  'other_reply_hide' => '你很好奇吗？ 点这里看看 &nbsp;o(*≧▽≦)ツ',
  'keyboard_tip' => '提示：支持键盘翻页&lt;-左 右-&gt;',
  'lightclose' => '关灯',
  'lightopen' => '开灯',
  'waitpicloading' => '组图打开中，请稍候',
  'text_summary' => '正文摘要',
  'view_bigpic' => '只看大图',
  'read_mode' => '阅读模式',
  'fav_thread' => '收藏本文章',
  'rate_position' => '投币表立场',
  'follow_spread' => '转播求扩散',
  'share_digest' => '分享推蜜制',
  'thread_collect' => '收藏',
  'maketoponce' => '顶一下',
  'makebottomonce' => '踩一下',
  'support_reply' => '支持',
  'against_reply' => '反对',
  'pleaseinputpw' => '，请输入密码',
  'number_of_participants' => '参与人数',
  'hide_preview' => '隐藏预览',
  'collapse_preview' => '收起预览',
  'hot_thread' => '热文',
  'hidden' => '隐藏',
  'hiderecover_tips' => '点击恢复文章隐藏状态',
  'hiderecover' => '恢复隐藏',
  'content_actions' => '更多操作',
);

?>