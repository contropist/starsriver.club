	...
	open : function(pop, type, url) {
		...
		if(typeof pop == 'string') {
			...
			if(type == 'alert') {
				...
				pop = '<div class="tip"><dt>'+ pop +'</dt><dd><input class="redirect button2" type="button" value="确定" href="'+ url +'"><a onclick="popup.close();">取消</a></dd></div>'
			}
	...
	locationerror : function(error) {
		...
		switch(error.code) {
			...
			case error.POSITION_UNAVAILABLE:
				geo.errmsg = '无法检测到你的当前位置';
			    break;
		    case error.PERMISSION_DENIED:
		        geo.errmsg = '请允许能够正常访问你的当前位置';
		        break;
