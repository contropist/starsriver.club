...
class plugin_wechat {
	...
	function common() {
		...
		$_G['Plang'] = $_G['setting']['wechatviewpluginid'] ? lang('plugin/'.$_G['setting']['wechatviewpluginid']) : [];
...
class uploadUcAvatar {
	...
	public static function uc_api_post_ex($module, $action, $arg = [], $extra = '') {
		$s = $sep = '';
		foreach($arg as $k => $v) {
			$k = urlencode($k);
			if(is_array($v)) {
	...
	function post() {
		...
		if(submitcheck('replysubmit')) {
			...
			if($activity['starttimefrom'] > TIMESTAMP) {
				showmessage('wechat:show_no_begin', NULL, []);
			}
