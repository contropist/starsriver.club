...
class mobile_core {
	...
	public static function getvalues($variables, $keys, $subkeys = []) {
		$return = [];
	...
	function viewthread_postbottom_output() {
		...
		foreach($postlist as $k => $post) {
			if($post['mobiletype'] == 1) {
				...
				$threadmessage = $_G['setting']['wechatviewpluginid'] ? lang('plugin/'.$_G['setting']['wechatviewpluginid'], 'lang_wechat_threadmessage', array('tid' => $_G['tid'], 'pid' => $post['pid'])) : [];
