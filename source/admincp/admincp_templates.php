	...
	if(!submitcheck('tplsubmit')) {
		...
		foreach(C::t('common_template')->fetch_all_data() as $tpl) {
			...
			$templates .= showtablerow('', array('class="td25"', '', 'class="td29"'), array(
				"<input type=\"checkbox\" name=\"delete[]\" ".($tpl['templateid'] == 1 ? 'disabled ' : '')."value=\"$tpl[templateid]\">",
				"<input type=\"text\" size=\"8\" name=\"namenew[$tpl[templateid]]\" value=\"$tpl[name]\">".
				...
				"<input type=\"text\" size=\"20\" name=\"directorynew[$tpl[templateid]]\" value=\"$tpl[directory]\">",
				!empty($tpl['copyright']) ?
					$tpl['copyright'] :
					"<input type=\"text\" size=\"8\" name=\"copyrightnew[$tpl[templateid]]\" value=>"
			), TRUE);
		}
