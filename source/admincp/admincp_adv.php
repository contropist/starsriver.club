	...
	if(!submitcheck('advsubmit')) {
		...
		foreach(C::t('common_advertisement')->fetch_all_search($title, $starttime, $endtime, $type, $target, $orderby, $start_limit, $advppp) as $adv) {
			...
			showtablerow('', array('class="td25"', 'class="td25"', 'class="td25"'), array(
				"<input type=\"checkbox\" name=\"delete[]\" value=\"$adv[advid]\">",
				"<input type=\"text\" size=\"2\" name=\"displayordernew[$adv[advid]]\" value=\"$adv[displayorder]\">",
				"<input type=\"checkbox\" name=\"availablenew[$adv[advid]]\" value=\"1\" ".($adv['available'] ? 'checked' : '').">",
				"<input type=\"text\" size=\"15\" name=\"titlenew[$adv[advid]]\" value=\"".dhtmlspecialchars($adv['title'])."\">",
...
<input type="button" class="btn" name="searchsubmit" value="'.cplang('search').'" onclick="if(this.form.title.value==\''.cplang('adv_inputtitle').'\'){this.form.title.value=\'\'}location.href=\''.ADMINSCRIPT.'?action=adv&operation=ad'.($type ? '&type='.rawurlencode($type) : '').'&title=\'+this.form.title.value+\'&starttime=\'+this.form.starttime.value+\'&endtime=\'+this.form.endtime.value+\'&target=\'+this.form.target.value+\'&orderby=\'+this.form.orderby.value;"> &nbsp;
	...
	if(!submitcheck('advsubmit')) {
		...
		echo '<script src="static/js/calendar.js"></script>'.
			...
			: (file_exists(DISCUZ_ROOT.'./static/image/admincp/src/img/'.$type.'.gif') ? '<img src="static/image/admincp/src/img/'.$type.'.gif" />' : '')).
		...
		foreach($advs as $adv) {
			...
			if($adv['class'] == 'custom') {
				...
				if(count($eclass) > 1) {
					...
					echo file_exists(DISCUZ_ROOT.'./static/image/admincp/src/img/'.$adv['class'].'.gif') ? '<img src="static/image/admincp/src/img/'.$adv['class'].'.gif" /><br>' : '';
				}
	...
	if($customadv) {
		$img = file_exists(DISCUZ_ROOT.'./static/image/admincp/src/img/'.$customadv['class'].'.gif') ? '<img src="static/image/admincp/src/img/'.$customadv['class'].'.gif" /><br>' : '';
		$i = $row;
		foreach(C::t('common_advertisement_custom')->fetch_all_data() as $custom) {
			if($i == $row) {
				echo '<tr>';
