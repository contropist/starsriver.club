	...
	if(!submitcheck('bbcodessubmit') && !$edit) {
		...
		foreach(C::t('forum_bbcode')->fetch_all_by_available_icon() as $bbcode) {
			showtablerow('', array('class="td25"', 'class="td21"', 'class="td25"', 'class="td25"', 'class="td28 td24"', 'class="td25"', 'class="td21"'), array(
				"<input type=\"checkbox\" name=\"delete[]\" value=\"$bbcode[id]\">",
				"<input type=\"text\" size=\"15\" name=\"tagnew[$bbcode[id]]\" value=\"$bbcode[tag]\">",
				"<input type=\"checkbox\" name=\"availablenew[$bbcode[id]]\" value=\"1\" ".($bbcode['available'] ? 'checked="checked"' : NULL).">",
				"<input type=\"checkbox\" name=\"displaynew[$bbcode[id]]\" value=\"1\" ".($bbcode['available'] == '2' ? 'checked="checked"' : NULL).">",
				"<input type=\"text\" size=\"2\" name=\"displayordernew[$bbcode[id]]\" value=\"$bbcode[displayorder]\">",
				$bbcode['icon'] ? "<em class=\"editor\"><a class=\"customedit\"><img src=\"static/image/common/$bbcode[icon]\" border=\"0\"></a></em>" : ' ',
				"<input type=\"text\" size=\"25\" name=\"iconnew[$bbcode[id]]\" value=\"$bbcode[icon]\">",
		...
		if($totalcount) {
			...
			foreach(C::t('common_word')->fetch_all_by_type_find($ftype, $ffind, $startlimit, $ppp) as $censor) {
				...
				showtablerow('', array('class="td25"', '', '', 'class="td26"'), array(
					"<input type=\"checkbox\" name=\"delete[]\" value=\"$censor[id]\" $disabled>",
					"<input type=\"text\" size=\"30\" name=\"find[$censor[id]]\" value=\"$censor[find]\" $disabled>",
...
EOT;
		...
		if($wordtypecount = C::t('common_word_type')->count()) {
			foreach(C::t('common_word_type')->fetch_all() as $result) {
				showtablerow('', array('class="td25"', ''), array("<input type=\"checkbox\" name=\"delete[]\" value=\"{$result['id']}\" $disabled>", "<input type=\"text\" size=\"10\" name=\"typename[{$result['id']}]\" value=\"{$result['typename']}\">"));
			}
		}
	...
	if(!submitcheck('stampsubmit')) {
		...
		foreach(C::t('common_smiley')->fetch_all_by_type('stamp') as $smiley) {
			...
			showtablerow('', array('class="td25"', 'class="td25"', 'class="td23"'), array(
				"<input type=\"checkbox\" name=\"delete[]\" value=\"$smiley[id]\">",
				"<input type=\"text\" size=\"2\" name=\"displayorder[$smiley[id]]\" value=\"$smiley[displayorder]\">",
				"<input type=\"text\" size=\"2\" name=\"code[$smiley[id]]\" value=\"$smiley[code]\">",
		...
		while($entry = $stampsdir->read()) {
			if(in_array(strtolower(fileext($entry)), $imgextarray) && !in_array($entry, $imgfilter) && is_file(DISCUZ_ROOT.'./static/image/stamp/'.$entry)) {
				showtablerow('', array('class="td25"', 'class="td28 td24 rowform"', 'class="td23"'), array(
					"<input type=\"checkbox\" name=\"addcheck[$newid]\" id=\"addcheck_$newid\">",
					...
					"</ul>",
					"<input type=\"text\" size=\"2\" name=\"adddisplayorder[$newid]\" value=\"0\">",
					"<input type=\"text\" size=\"2\" name=\"addcode[$newid]\" value=\"\">",
					"<img src=\"static/image/stamp/$entry\" />",
					"<input type=\"hidden\" size=\"35\" name=\"addurl[$newid]\" value=\"$entry\">$entry"
				));
				$newid ++;
			}
		}
	...
	if(!submitcheck('typesubmit')) {
		...
		while($type = DB::fetch($query)) {
			...
			$attachtypes .= showtablerow('', array('class="td25"', 'class="td24"'), array(
				"<input type=\"checkbox\" name=\"delete[]\" value=\"$type[id]\" />",
				"<input type=\"text\" size=\"10\" name=\"extension[$type[id]]\" value=\"$type[extension]\" />",
				"<input type=\"text\" size=\"15\" name=\"maxsize[$type[id]]\" value=\"$type[maxsize]\" />"
			), TRUE);
		}
			...
			if(is_array($focus['data'])) {
				foreach($focus['data'] as $k => $v) {
					showtablerow('', array('class="td25"','', 'class="td25"', 'class="td25"'), array(
						...
						"<input type=\"checkbox\" name=\"available[$k]\" value=\"1\" ".($v['available'] ? 'checked' : '').">",
	...
	if(!$do) {
		...
		if(!submitcheck('optionsubmit')) {
			...
			foreach(C::t('common_admincp_cmenu')->fetch_all_by_uid($_G['uid'], $startlimit, $mpp) as $custom) {
				...
				$optionlist .= "
                    <tr>
                        <td class=\"csbox admin align-center\"><input type=\"checkbox\" name=\"delete[]\" id=\"custommenu_$custom[id]\" value=\"$custom[id]\"><label for=\"custommenu_$custom[id]\"></label></td>
                        <td><input type=\"text\" name=\"displayordernew[$custom[id]]\" value=\"$custom[displayorder]\"></td>
                        <td><input type=\"text\" name=\"titlenew[$custom[id]]\" value=\"".cplang($custom['title'])."\"><input type=\"hidden\" name=\"langnew[$custom[id]]\" value=\"$custom[title]\"></td>
                        <td><input type=\"text\" name=\"urlnew[$custom[id]]\" value=\"$custom[url]\"></td>
                    </tr>";
                            ...
                            <table>
                                <script type="text/JavaScript">
                                    var rowtypedata = [
                                        [
                                            [1,\'\'],
                                            [1,\'<input type="text" name="newdisplayorder[]">\'],
                                            [1,\'<input type="text" name="newtitle[]">\'],
                                            [1,\'<input type="text" name="newurl[]">\']
                                        ]
                                    ];
                                </script>
                                <thead>
                                    <tr>
                                        <td width="200" class="icon ft-trash-2 align-center heart-pink"></td>
                                        <td width="200">'.$lang['display_order'].'</td>
                                        <td width="2000">'.$lang['name'].'</td>
                                        <td width="6000">URL</td>
                                    </tr>
                                </thead>
                                <tbody>
                                    '.$optionlist.'
                                    <tr><td colspan="4"><div><a class="btn addrow" href="###" onclick="addrow(this, 0)" title="'.$lang['custommenu_add'].'">+</a></div></td></tr>
                                </tbody>
                           </table>
                        </div>',
			        'footer' => make_submit('optionsubmit', 'submit', 'del', '', $multipage),
                ],
            ]);
