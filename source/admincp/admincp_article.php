...
if($operation == 'tag') {
	...
	for($i=1; $i<=8; $i++) {
		showtablerow('', array('width=80', ''),
			array(lang('portalcp', 'article_tag').$i, "<input type=\"text\" name=\"tag[$i]\" value=\"$tag_names[$i]\" />"));
	}
		...
		if($count) {
			foreach(C::t('portal_article_trash')->range($start, $perpage) as $value) {
				...
				showtablerow('', array('class="td25"', 'class=""', 'class="td28"'), array(
						"<input type=\"checkbox\" name=\"ids[]\" value=\"$value[aid]\">",
						$value[title],
	...
	if(submitcheck('articlesubmit')) {
		...
		foreach($article_tags as $k=>$v) {
			...
			$articletagcheckbox .= "<input type=\"checkbox\" id=\"tag_$k\" name=\"tag[$k]\" value=\"1\"$checked />";
