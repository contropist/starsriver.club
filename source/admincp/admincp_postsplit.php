...
if($operation == 'manage') {
	...
	if(!submitcheck('postsplit_manage')) {
		...
		showtablerow('', array('', '', '', 'class="td25"'), array($tablename, $data_length, "<input type=\"text\" name=\"memo[0]\" value=\"{$posttable_info[0]['memo']}\" />", $opstr));
		...
		foreach(C::t('forum_post')->show_table() as $table) {
			...
			showtablerow('', array('', '', '', 'class="td25"'), array($tablename, $tablestatus['Data_length'], "<input type=\"text\" name=\"memo[$tableid]\" value=\"{$posttable_info[$tableid]['memo']}\" />", $opstr));
		}
