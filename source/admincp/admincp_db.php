...
if($operation == 'export') {
	...
	if(!submitcheck('exportsubmit')) {
		...
		if(UC_CONNECT == 'mysql' && UC_DBHOST == $_G['config']['db'][1]['dbhost'] && UC_DBNAME == $_G['config']['db'][1]['dbname'] && $uc_tablepre == $tablepre) {
			...
			$uc_backup_url = UC_API.'/ucterminal.php?m=db&a=ls&iframe=1';
	...
	if(!submitcheck('deletesubmit')) {
		...
		foreach($exportziplog as $info) {
			...
			$datafile_server = '.'.$info['filename'];
			showtablerow('', '', array(
				"<input type=\"checkbox\" name=\"delete[]\" value=\"".basename($info['filename'])."\">",
				"<a href=\"$info[filename]\">".substr(strrchr($info['filename'], "/"), 1)."</a>",
				'',
				$info['dateline'],
				$lang['db_export_'.$info['type']],
	...
	if(!submitcheck('optimizesubmit')) {
		...
		foreach($tablearray as $tp) {
			...
			while($table = DB::fetch($query)) {
				if($table['Data_free'] && $table[$tabletype] == 'MyISAM') {
					...
					showtablerow('', '', array(
						"<input type=\"checkbox\" name=\"optimizetables[]\" value=\"$table[Name]\" $checked>",
						$table[Name],
						$table[$tabletype],
						$table[Rows],
						$table[Data_length],
		...
		foreach($tablearray as $tp) {
			...
			while($table = DB::fetch($query)) {
				if($table['Data_free'] && $table[$tabletype] == 'MyISAM') {
					$optimizeinput = "<input type=\"checkbox\" name=\"optimizetables[]\" value=\"$table[Name]\" $checked>";
		...
		foreach($discuzdb as $dbtable => $fields) {
			...
			if(($modifylist || $dellist) && !in_array($dbtable, $excepttables)) {
				...
				foreach($modifylist as $value) {
					...
					$showlist .= "<tr><td><input name=\"repair[]\" type=\"checkbox\" value=\"$dbtable|$value[Field]|modify\"> <b>".$value['Field']."</b> ".
				...
				if($modifylist) {
					$showlist .= showtablerow('', 'colspan="3"', "<input onclick=\"setrepaircheck(this, this.form, '$dbtable')\" name=\"repairtable[]\" type=\"checkbox\" value=\"$dbtable\"> <b>$lang[dbcheck_repairtable]</b>", TRUE);
				}

				foreach($dellist as $value) {
					$showlist .= "<tr><td><input name=\"repair[]\" type=\"checkbox\" value=\"$dbtable|$value[Field]|add\"> <strike><b>".$value['Field']."</b></strike></td><td> <b>".$value['Field']."</b> ".$value['Type'].($value['Null'] == 'NO' ? ' NOT NULL' : '')."</td><td>".
