...
if($operation == 'admin') {
	...
	if(!submitcheck('stylesubmit')) {
		...
		foreach($sarray as $id => $style) {
			...
			$preview = file_exists($style['directory'].'/preview.jpg') ? $style['directory'].'/preview.jpg' : './static/image/admincp/src/img/stylepreview.gif';
			...
			$stylelist .=
				...
				<p style=\"margin: 2px 0\"><input type=\"text\" name=\"namenew[$id]\" value=\"$style[name]\" style=\"margin:0; width: 104px;\"></p>".
		...
		if($_GET['updatecsscache']) {
			...
			updatecache('updatenesttemplate');
		...
		foreach(C::t('common_stylevar')->fetch_all_by_styleid($id) as $stylevar) {
			if(array_key_exists($stylevar['variable'], $predefinedvars)) {
				...
				$stylecustom .= showtablerow('', array('class="td25"', 'class="td24 bold"', 'class="td26"'), array(
					"<input type=\"checkbox\" name=\"delete[]\" value=\"$stylevar[stylevarid]\">",
