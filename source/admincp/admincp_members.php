...
EOF;
			...
			if($uids) {
				...
				foreach($allmember as $uid=>$member) {
					...
					$members .= showtablerow('', array('class="td25"', '', 'title="'.implode("\n", $memberextcredits).'"'), array(
						"<input type=\"checkbox\" name=\"uidarray[]\" value=\"$member[uid]\"".($member['adminid'] == 1 ? 'disabled' : '').">",
		...
		if(!submitcheck('confirmed')) {
			...
			if(!submitcheck('includepost')) {
				...
				if($_GET['uidarray']) {
					...
					cpmsg(cplang('members_delete_user_processing_next', array('deletestart' => $deletestart, 'nextdeletestart' => $deletestart+$delmemberlimit)), $nextlink, 'loadingform', []);
				}
				...
				if($deleteitem == 'doing') {
					...
					foreach ($query as $doing) {
                        $doings_idlist[] = $doing['doid'];
					}

					if($doings_idlist) {
						deletedoings($doings_idlist);
					} else {
						$next = 0;
				...
				if($nextdeleteitem != $deleteitem) {
					...
					cpmsg(cplang('members_delete_processing_next', array('deladdmsg' => $deladdmsg, 'item' => cplang('members_delete_'.$deleteitem), 'nextitem' => cplang('members_delete_'.$nextdeleteitem))), $nextlink, 'loadingform', [], $extra);
				} else {
					$nextlink .= "&deleteitem=$deleteitem";
					cpmsg(cplang('members_delete_processing', array('deladdmsg' => $deladdmsg, 'item' => cplang('members_delete_'.$deleteitem), 'current' => $current, 'next' => $next)), $nextlink, 'loadingform', [], $extra);
				}
			}
		}
	}
	...
	if(!submitcheck('newslettersubmit')) {
		...
		if($_GET['do'] == 'mobile') {
			...
			showsubmenusteps('nav_members_newsletter', array(
				array('nav_members_select', !$_GET['submit']),
				array('nav_members_notify', $_GET['submit']),
			), [], array(array('members_grouppmlist', 'members&operation=grouppmlist', 0)));
		}
	...
	if($do) {
		...
		$allmember = $gpmuser ? C::t('common_member')->fetch_all_username_by_uid(array_keys($gpmuser)) : [];
	...
	foreach(C::t('forum_medal')->fetch_all_data(1) as $medal) {
		$medals .= showtablerow('', array('class="td25"', 'class="td23"'), array(
			"<input type=\"checkbox\" name=\"medals[$medal[medalid]]\" value=\"1\" />",
	...
	foreach(C::t('common_magic')->fetch_all_data(1) as $magic) {
		$magics .= showtablerow('', array('class="td25"', 'class="td23"', 'class="td25"', ''), array(
			"<input type=\"checkbox\" name=\"magic[]\" value=\"$magic[magicid]\" />",
	...
	if(!submitcheck('editsubmit')) {
		...
		showsubmenu('members_group_member', [], '', array('username' => $member['username']));
...
EOT;
	...
	if(!submitcheck('medalsubmit')) {
		...
		foreach(C::t('forum_medal')->fetch_all_data(1) as $medal) {
			$medals .= showtablerow('', array('class="td25"', 'class="td23"'), array(
				"<input type=\"checkbox\" name=\"medals[$medal[medalid]]\" value=\"1\" ".(in_array($medal['medalid'], $membermedals) ? 'checked' : '')." />",
				"<img src=\"static/image/common/$medal[image]\" />",
				$medal['name']
...
EOF;
	...
	if(!empty($_G['setting']['connect']['allow']) && $do == 'bindlog') {
		...
		if($member['conopenid']) {
			...
			$bindlogs = $bindloguids = $usernames = [];
	...
	if(!submitcheck('editsubmit')) {
		...
		showsubmenu("$lang[members_edit] - $member[username]", array(
			...
			!empty($_G['setting']['connect']['allow']) ? array('connect_member_bindlog', 'members&operation=edit&do=bindlog&uid='.$uid,  0) : [],
		));
		...
		if(!submitcheck('ipbansubmit')) {
			...
			foreach(C::t('common_banned')->fetch_all_order_dateline() as $banned) {
				...
				$ipbanned .= showtablerow('', array('class="td25"'), array(
					"<input type=\"checkbox\" name=\"delete[$banned[id]]\" value=\"$banned[id]\" $disabled />",
					$theip,
					convertip($theip, "./"),
					$banned[admin],
					$banned[dateline],
					"<input type=\"text\" size=\"10\" name=\"expirationnew[$banned[id]]\" value=\"$banned[expiration]\" $disabled />"
				), TRUE);
			}
			...
			if(!empty($profilegroup)) {
				...
				foreach($profilegroup as $key => $value) {
					if($value['available']) {
						...
						$groupstr .= "<li $class style=\"float: left; width: 10%;\"><input type=\"checkbox\" value=\"$key\" name=\"profilegroup[$key]\" $checked>&nbsp;$value[title]</li>";
					}
				}
				if(!empty($groupstr)) {
					print <<<EOF
				...
				if(is_array($_GET['addextcredits']) && !empty($_GET['addextcredits'])) {
					foreach($_GET['addextcredits'] as $key => $value) {
						...
						if(isset($_G['setting']['extcredits'][$key]) && !empty($value)) {
							...
							$addmsg .= $_G['setting']['extcredits'][$key]['title'].": ".($value > 0 ? '<em>+' : '<em>')."$value</em> ".$_G['setting']['extcredits'][$key]['unit'].' &nbsp; ';
						}
					}
				}
			} else {
				if(is_array($_GET['resetextcredits']) && !empty($_GET['resetextcredits'])) {
					foreach($_GET['resetextcredits'] as $key => $value) {
						...
						if(isset($_G['setting']['extcredits'][$key]) && !empty($value)) {
							...
							$addmsg .= $_G['setting']['extcredits'][$key]['title'].': <em>'.cplang('members_reward_clean').'</em> &nbsp; ';
						}
					}
				}
			}
		...
		if($_GET['notifymembers'] == 'pm') {
			...
			if(!$gpmid) {
				$pmdata = array(
						...
						'dateline' => TIMESTAMP,
						'message' => $subject.'<div class="summary">'.$message.'</div><div class="extro">'.$addmsg.'</div>',
						'numbers' => $membernum
					);
