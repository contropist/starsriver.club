...
if($operation == 'filecheck') {
	...
	if($step == 1) {
        ...
        C::t('common_cache')->insert([
            ...
            'cachevalue' => serialize([$modifiedfiles, $deletedfiles, $unknownfiles, $doubt, 'dateline' => $_G['timestamp']]),
	...
	if($step == 1) {
		...
		if($diffnum) {
			...
			showtablerow('', 'colspan="4"', "<div class=\"margintop marginbot\">".
				'<a onclick="show_all_hook(\'dir_\', \'tbody\')">'.$lang[show_all].'</a> | <a onclick="hide_all_hook(\'dir_\', \'tbody\')">'.$lang[hide_all].'</a>'.
