...
SCRIPT;
		...
		if($_POST['name']) {
			...
			foreach($_POST['name'] as $key=>$value) {
				...
				if($sets) {
					...
					C::t('common_nest_data')->update('portal/list_'.$key, getnestdirectory($portalcategory[$key]['primaltplname']), array('name'=>$value));
					C::t('common_nest_data')->update('portal/view_'.$key, getnestdirectory($portalcategory[$key]['articleprimaltplname']), array('name'=>$value));
					$cachearr[] = 'nesttemplatename';
				}
			}
		}
	...
	if(!submitcheck('permsubmit')) {
		...
		foreach($permissions as $uid => $value) {
			...
			if(!empty($value['inheritedcatid'])) {
				...
				showtablerow('', array('class="td25"'), array(
					"<input type=\"checkbox\" name=\"delete[$value[uid]]\" value=\"$value[uid]\" /><input type=\"hidden\" name=\"perm[$value[uid]]\" value=\"$value[catid]\" />
					...
					"$value[username]",
					"<input type=\"checkbox\" name=\"allowpublish[$value[uid]]\" value=\"1\" ".($value['allowpublish'] ? 'checked' : '').' />',
	...
	if(!submitcheck('deletesubmit')) {
		...
		if(!$article_count && empty($catechildren)) {
			...
			updatecache(array('portalcategory','nesttemplatename'));
	...
	if(!submitcheck('detailsubmit')) {
		...
		if(!empty($_GET['listprimaltplname'])) {
			...
			if(!isset($_GET['signs']['list'][dsign($primaltplname)])) {
				cpmsg(cplang('nest_sign_invalid').','.cplang('return'), NULL, 'error');
			}
		...
		if($_GET['catid']) {
			...
			if($cate['catname'] != $_GET['catname']) {
				C::t('common_nest_data')->update('portal/list_'.$cate['catid'], getnestdirectory($cate['primaltplname']), array('name'=>$_GET['catname']));
				C::t('common_nest_data')->update('portal/view_'.$cate['catid'], getnestdirectory($cate['articleprimaltplname']), array('name'=>$_GET['catname']));
				$cachearr[] = 'nesttemplatename';
			}
		} else {
		...
		if($_GET['listprimaltplname'] && (empty($cate['primaltplname']) || $cate['primaltplname'] != $primaltplname)) {
			remakenesttemplate($primaltplname, 'portal/list_'.$_GET['catid'], $_GET['catname'], getnestdirectory($cate['primaltplname']));
		}

		if($cate['articleprimaltplname'] != $viewprimaltplname) {
			remakenesttemplate($viewprimaltplname, 'portal/view_'.$_GET['catid'], $_GET['catname'].'-'.cplang('portalcategory_viewpage'), getnestdirectory($cate['articleprimaltplname']));
		}
...
function showcategoryrow($key, $level = 0, $last = '') {
	...
	if($level == 2) {
		...
		<a href="'.ADMINSCRIPT.'?action=nesttemplate&operation=perm&targettplname=portal/list_'.$value['catid'].'&tpldirectory='.getnestdirectory($value['primaltplname']).'">'.cplang('portalcategory_blockperm').'</a></td>
...
function deleteportalcategory($ids) {
	...
	C::t('common_nest_data')->delete($tpls, NULL);
...
function remakenesttemplate($primaltplname, $targettplname, $nesttplname, $olddirectory){
	...
	$newnestdata = C::t('common_nest_data')->fetch($targettplname, $tpldirectory);
	if($newnestdata) {
		return false;
	}
	$nestdata = C::t('common_nest_data')->fetch($targettplname, $olddirectory);
	$nestcontent = empty($nestdata['nestcontent']) ? '' : $nestdata['nestcontent'];
	if($nestdata) {
		C::t('common_nest_data')->update($targettplname, $olddirectory, array('primaltplname'=>$primaltplname, 'tpldirectory'=>$tpldirectory));
	} else {
		$nestcontent = '';
