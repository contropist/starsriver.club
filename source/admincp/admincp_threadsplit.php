...
if($operation == 'manage') {
	...
	if(!submitcheck('threadsplit_update_submit')) {
		...
		showtablerow('', array(), array($thread_table_orig['Name'], $thread_table_orig['Rows'], $thread_table_orig['Data_length'], $thread_table_orig['Index_length'], $thread_table_orig['Create_time'], "<input type=\"text\" name=\"memo[0]\" value=\"{$threadtable_info[0]['memo']}\" />", ''));
		...
		foreach($threadtableids as $tableid) {
			...
			showtablerow('', array(), array($table_info['Name'], "<input type=\"text\" name=\"displayname[$tableid]\" value=\"{$threadtable_info[$tableid]['displayname']}\" />", $table_info['Rows'], $table_info['Data_length'], $table_info['Index_length'], $table_info['Create_time'], "<input type=\"text\" name=\"memo[$tableid]\" value=\"{$threadtable_info[$tableid]['memo']}\" />", "<a href=\"?action=threadsplit&operation=droptable&tableid=$tableid\">{$lang['delete']}</a>"));
		}
		showtablefooter();
			...
			if($_GET['detail']) {
				...
				foreach($threadlist as $thread) {
					...
					$threads .= showtablerow('', array('class="td25"', '', '', '', '', ''), array(
						"<input type=\"checkbox\" name=\"tidarray[]\" value=\"$thread[tid]\" checked=\"checked\" />",
