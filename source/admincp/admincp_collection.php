...
if($operation == 'comment') {
	...
	if(!submitcheck('searchsubmit', 1)) {
		...
		foreach($collectioncomment as $uniquecomment) {
			...
			showtablerow('', array('class="td25"', 'width=400', ''), array(
				"<input type=\"checkbox\" name=\"cidarray[]\" value=\"$uniquecomment[cid]\" />",
	...
	if(!submitcheck('searchsubmit', 1)) {
		...
		foreach($collection as $uniquecollection) {
			showtablerow('', array('class="td25"', 'width=400', ''), array(
				"<input type=\"checkbox\" name=\"ctidarray[]\" value=\"$uniquecollection[ctid]\" />",
	...
	if(!submitcheck('submit', 1)) {
		...
		if($collectionrecommend['ctids']) {
			...
			foreach($collectiondata as $collection) {
				showtablerow('', array('class="td25"', 'width=400', ''), array(
					"<input type=\"checkbox\" name=\"ctidarray[]\" value=\"$collection[ctid]\" />",
					...
					"<input type=\"text\" name=\"ctidorder[{$collection[ctid]}]\" value=\"{$collectionrecommend['ctids'][$collection[ctid]]}\" />",
				));
			}
		} else {
