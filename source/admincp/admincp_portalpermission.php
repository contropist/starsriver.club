...
if($operation == 'article') {
	...
	showsubtitle(array('username', 'nesttemplate_name', 'block_perm_manage', 'block_perm_recommend', 'block_perm_needverify', 'block_perm_inherited'));
	showtagheader('tbody', '', true);
	...
	if($count) {
		...
		$multipage = multi($count, $perpage, $page, $mpurl.'&perpage='.$perpage);
		foreach($permissions as $value){
			$targettplname = $_G['cache']['nesttemplatename'][$value['targettplname']];
			showtablerow('', '', array(
				$members[$value['uid']]['username'],
				'<a href="'.ADMINSCRIPT.'?action=nesttemplate&operation=perm&targettplname='.$value['targettplname'].'">'.$targettplname.'</a>',
				$value['allowmanage'] ? $right : $line,
				$value['allowrecommend'] ? $right : $line,
				$value['needverify'] ? $right : $line,
				$value['inheritedtplname'] ? '<a href="'.ADMINSCRIPT.'?action=nesttemplate&operation=perm&targettplname='.$value['inheritedtplname'].'">'.$_G['cache']['nesttemplatename'][$value['inheritedtplname']].'</a>' : $line,
			));
		}
		echo '<tr><td colspan="6">'.$multipage.'</td></tr>';
	}
