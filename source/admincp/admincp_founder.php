			...
			if(!submitcheck('submit')) {
				...
				foreach($members as $id => $member) {
					...
					showtablerow('style="height:20px"', array('class="td25"', 'class="td24"', 'class="td24"'), array(
						!$isfounder || isset($adminmembers[$member['uid']]['cpgroupid']) ? "<input type=\"checkbox\" name=\"delete[]\" value=\"$id]\">" : '',
		...
		if(!submitcheck('submit')) {
			...
			foreach($groups as $id => $group) {
				showtablerow('style="height:20px"', array('class="td25"', ''), array(
					"<input type=\"checkbox\" name=\"permnew[]\" ".($allperms[$group['cpgroupid']]['perm'] ? 'checked="checked"' : '')." value=\"$id\">",
					$group['cpgroupname']
					));
			}
			showsubmit('submit');
		...
		if(!submitcheck('submit')) {
			...
			foreach($notifyusers as $uid => $user) {
				...
				foreach($notifytypes as $key => $typename) {
					...
					$types .= "<div class=\"item$checked\"><label class=\"txt\"><input onclick=\"checkclk(this)\" type=\"checkbox\" name=\"notifytypes_{$uid}[{$typename}]\" value=\"1\"$checked>".$tname.'</label></div>';
				}
