...
if($operation=='perm') {
	...
	if(!submitcheck('permsubmit')) {
		...
		foreach($block_per as $uid => $value) {
			if(!empty($value['inheritedtplname'])) {
				...
				showtablerow('', array('class="td25"'), array(
					"<input type=\"checkbox\" name=\"delete[$uid]\" value=\"$uid\" />
					...
					"{$members[$uid]['username']}",
					"<input type=\"checkbox\" name=\"allowmanage[$uid]\" value=\"1\" ".($value['allowmanage'] ? 'checked' : '').' />',
					"<input type=\"checkbox\" name=\"allowrecommend[$uid]\" value=\"1\" ".($value['allowrecommend'] ? 'checked' : '').' />',
					"<input type=\"checkbox\" name=\"needverify[$uid]\" value=\"1\" ".($value['needverify'] ? 'checked' : '').' />',
					$line,
				));
			}
		}
	...
	if(submitcheck('deletesubmit')) {
		...
		loadcache(array('nesttemplatename'));
		...
		'block_dateline', 'block_id', 'block_name', 'block_blockclass', 'block_add_jscall', 'block_choose_blockclass_to_add_jscall', 'block_nesttemplate', 'block_permname', 'block_permname_tips');
		foreach ($keys as $key) {
			$searchlang[$key] = cplang($key);
		}
		$nesttemplatename_sel = '<select name="targettplname" id="targettplname">';
		$nesttemplatename_sel .= '<option value="">'.cplang('nesttemplate_name').'</option>';
		foreach($_G['cache']['nesttemplatename'] as $key=>$value) {
			$selected = ($key == $_GET['blockclass'] ? ' selected' : '');
			$nesttemplatename_sel .= "<option value=\"$key\"$selected>$value</option>";
		}
		$nesttemplatename_sel .= '</select>';
		...
		$firstrow = "<th>$searchlang[block_nesttemplate]</th><td>$nesttemplatename_sel</td><th>$searchlang[block_blockclass]</th><td colspan=\"2\">$blockclass_sel $addjscall</td>";
			...
			if(($count = C::t('common_block')->count_by_admincpwhere($wheresql))) {
				foreach(C::t('common_block')->fetch_all_by_admincpwhere($wheresql, $ordersql, $start, $perpage) as $value) {
					if($value['targettplname']) {
						...
						$nesturl = $nesturl['url'];
						$tplname = isset($_G['cache']['nesttemplatename'][$value['targettplname']]) ? $_G['cache']['nesttemplatename'][$value['targettplname']] : $value['targettplname'];
						$nestpage[$value['bid']][$value['targettplname']] = $nesturl ? '<a href="'.$nesturl.'" target="_blank">'.$tplname.'</a>' : $tplname;
					}
				...
				if($list) {
					foreach($list as $bid => $value) {
						...
						showtablerow('', array('class="td25"'), array(
							...
							$value['styleid'] ? $theclass['style'][$value['styleid']]['name'] : lang('portalcp', 'blockstyle_nest'),
