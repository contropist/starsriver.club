		...
		if(!submitcheck('loginsubmit', 1, $seccodestatus)) {
			...
			if($result['status'] == -1) {
				if(!$this->setting['fastactivation']) {
					...
					showmessage('location_activation', 'member.php?mod='.$this->setting['regname'].'&action=activation&auth='.rawurlencode($auth).'&referer='.rawurlencode(dreferer()), [], array('location' => true));
				} else {
    ...
    function __construct() {
		...
		if($_G['setting']['bbclosed']) {
			if(($_GET['action'] != 'activation' && !$_GET['activationauth']) || !$_G['setting']['closedallowactivation'] ) {
				showmessage('register_disable', NULL, [], array('login' => 1));
			}
		}
