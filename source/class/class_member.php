<?php
    ...
    class logging_ctl {
            ...
            if (!submitcheck('loginsubmit', 1, $seccodestatus)) {
                ...
                if (!empty($_GET['auth'])) {
                    [
                        ...
                        $password,
                        $questionexist,
                    ] = explode("\t", authcode($_GET['auth'], 'DECODE', $_G['config']['security']['authkey']));
                ...
                if (!empty($_GET['auth'])) {
                    [
                        $_GET['username'],
                        $_GET['password'],
                    ] = daddslashes(explode("\t", authcode($_GET['auth'], 'DECODE', $_G['config']['security']['authkey'])));
                }
                ...
                if ($result['status'] > 0) {
                    ...
                    if ($_G['member']['adminid'] != 1) {
                        if ($this->setting['accountguard']['loginoutofdate'] && $_G['member']['lastvisit'] && TIMESTAMP - $_G['member']['lastvisit'] > 90 * 86400) {
                            C::t('common_member')->update($_G['uid'], ['freeze' => 2]);
                            ...
                            showmessage('location_login_outofdate', 'home.php?mod=spacecp&ac=profile&op=password&resend=1', ['type' => 1], [
                                'showdialog'   => true,
                                'striptags'    => false,
                                'locationtime' => true,
                            ]);
                        ...
                        if ($this->setting['accountguard']['loginpwcheck'] && $pwold) {
                            ...
                            if ($this->setting['accountguard']['loginpwcheck'] == 2 && $freeze) {
                                C::t('common_member')->update($_G['uid'], ['freeze' => 1]);
                            }
                        }
                    }
                    ...
                    if ($inviter['id']) {
                        $result = C::t('common_invite')->count_by_uid_fuid($inviter['uid'], $uid);
                        if (!$result) {
                            C::t('common_invite')->update($inviter['id'], [
                                'fuid'      => $uid,
                    ...
                    if ($inviter['uid']) {
                        ...
                        friend_make($inviter['uid'], $inviter['username'], false);
                    ...
                    if (!$freeze || !$this->setting['accountguard']['loginpwcheck']) {
                        ...
                        $location = $inviter || $_G['groupid'] == 8 ? 'home.php?mod=space&do=home' : dreferer();
                    } else {
                        ...
                        if (defined('IN_MOBILE')) {
                            ...
                            if (!empty($_GET['lssubmit'])) {
                                ...
                                showmessage('location_login_succeed', $location, [], [
                                        ...
                                        'extrajs'    => '<script>' . 'setTimeout("window.location.href =\'' . $href . '\';", 3000);' . '$(\'succeedmessage_href\').href = \'' . $href . '\';' . '$(\'main_message\').style.display = \'none\';' . '$(\'main_succeed\').style.display = \'\';' . '$(\'succeedlocation\').innerHTML = \'' . lang('message', $loginmessage, $param) . '\';</script>' . $ucsynlogin,
                                        'striptags'  => false,
                                        'showdialog' => true,
                                    ]);
                            }
        ...
        function on_logout() {
            ...
            if (defined('IN_MOBILE')) {
                showmessage('location_logout_succeed_mobile', dreferer(), [
                    'formhash' => FORMHASH,
                    'referer'  => rawurlencode(dreferer()),
                ]);
            } else {
    ...
    class register_ctl {
        ...
        function on_register() {
            ...
            if ($_G['uid']) {
                ...
                showmessage('login_succeed', $url_forward ? $url_forward : './', [
                    'username'  => $_G['member']['username'],
                    'usergroup' => $_G['group']['grouptitle'],
                    'uid'       => $_G['uid'],
                ], ['extrajs' => $ucsynlogin]);
                ...
                if ($_GET['action'] == 'activation' || $_GET['activationauth']) {
                    ...
                    showmessage(!$this->setting['regclosemessage'] ? 'register_disable' : str_replace([
                        "\r",
                        "\n",
            ...
            if (!submitcheck('regsubmit', 0, $seccodecheck, $secqaacheck)) {
                ...
                if ($sendurl) {
                    ...
                    $email_register_message = lang('email', 'email_register_message', [
                    ...
                    showmessage('register_email_send_succeed', dreferer(), ['bbname' => $this->setting['bbname']], [
                        'showdialog' => false,
                        'msgtype'    => 3,
                        'closetime'  => 10,
                    ]);
                ...
                if ($this->setting['regstatus'] == 2 && empty($inviter) && !$inviterstatus) {
                ...
                foreach ($_G['cache']['fields_register'] as $field) {
                    ...
                    if (!profile_check($field_key, $field_val)) {
                        $showid = !in_array($field['fieldid'], [
                            'birthyear',
                            'birthmonth',
                        ]) ? $field['fieldid'] : 'birthday';
                        showmessage($field['title'] . lang('message', 'profile_illegal'), '', [], [
                            'showid'  => 'chk_' . $showid,
                            'extrajs' => $field['title'] . lang('message', 'profile_illegal') . ($field['formtype'] == 'text' ? '<script>' . '$(\'registerform\').' . $field['fieldid'] . '.className = \'px er\';' . '$(\'registerform\').' . $field['fieldid'] . '.onblur = function () { if(this.value != \'\') {this.className = \'px\';$(\'chk_' . $showid . '\').innerHTML = \'\';}}' . '</script>' : ''),
                        ]);
                    }
                    if ($field['needverify']) {
                ...
                if ($inviter && $this->setting['inviteconfig']['invitegroupid']) {
                ...
                if ($inviter['uid']) {
                    ...
                    if ($this->setting['inviteconfig']['invitedaddcredit']) {
                        updatemembercount($inviter['uid'], [$this->setting['inviteconfig']['inviterewardcredit'] => $this->setting['inviteconfig']['invitedaddcredit']]);
                    }
                    require_once libfile('function/friend');
                    friend_make($inviter['uid'], $inviter['username'], false);
                    notification_add($inviter['uid'], 'friend', 'invite_friend', ['actor' => '<a href="home.php?mod=space&uid=' . $inviter['uid'] . '" target="_blank">' . $inviter['username'] . '</a>'], 1);
                    
                    space_merge($inviter, 'field_home');
                    if (!empty($inviter['privacy']['feed']['invite'])) {
                        
                        $avatar1 = avatar($inviter['uid'], '', true);
                ...
                switch ($this->setting['regverify']) {
                    case 1:
                        ...
                        C::t('common_member_field_forum')->update($_G['uid'], ['authstr' => $authstr]);
                        $verifyurl = $_G['setting']['securesiteurl'] . "member.php?mod=activate&amp;uid={$_G[uid]}&amp;id=$idstring";
                        $email_verify_message = lang('email', 'email_verify_message', [
                            'username' => $_G['member']['username'],
                            'bbname'   => $this->setting['bbname'],
                            'siteurl'  => $_G['setting']['securesiteurl'],
                            'url'      => $verifyurl,
                ...
                $extra = [
                    'showid'    => 'succeedmessage',
                    'extrajs'   => '<script>' . 'setTimeout("window.location.href =\'' . $href . '\';", ' . $refreshtime . ');' . '$(\'succeedmessage_href\').href = \'' . $href . '\';' . '$(\'main_message\').style.display = \'none\';' . '$(\'main_succeed\').style.display = \'\';' . '$(\'succeedlocation\').innerHTML = \'' . lang('message', $locationmessage) . '\';' . '</script>',
                    'striptags' => false,
                ];
