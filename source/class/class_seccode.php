...
class seccode {
	...
	function background() {
		...
		if($this->background && function_exists('imagecreatefromjpeg') && function_exists('imagecolorat') && function_exists('imagecopymerge') &&
			...
			if($backgrounds) {
				...
				imagecopymerge($this->im, $imwm, 0, 0, rand(0, 200 - $this->width), rand(0, 80 - $this->height), imageSX($imwm), imageSY($imwm), 100);
				imagedestroy($imwm);
			}
		}
		if(!$this->background || !$backgrounds) {
			for($i = 0;$i < 3;$i++) {
				$start[$i] = rand(200, 255);$end[$i] = rand(100, 150);$step[$i] = ($end[$i] - $start[$i]) / $this->width;$c[$i] = $start[$i];
			}
	...
	function ttffont() {
        ...
        $seccodelength = strlen($seccode)*2;

		for($i = 0; $i < $seccodelength; $i++) {
			$font[$i]['font'] = $seccoderoot.$seccodettf[array_rand($seccodettf)];
			$font[$i]['angle'] = $this->angle ? rand(-15, 15) : 0;
			$font[$i]['size'] = $this->type ? $this->width / 7 : $this->width / 6;
			$this->size && $font[$i]['size'] = rand($font[$i]['size'] - $this->width / 40, $font[$i]['size'] + $this->width / 20);
		...
		for($i = 0; $i < $seccodelength; $i++) {
			...
			$y = $font[0]['angle'] > 0 ? rand($font[$i]['height'], $this->height) : rand($font[$i]['height'] - $font[$i]['hd'], $this->height - $font[$i]['hd']);
	...
	function giffont() {
		...
		for($i = 0; $i <= 3; $i++) {
			...
			if(!empty($this->imcodefile) && file_exists($this->imcodefile)) {
				...
				$font[$i]['file'] = '';
				$font[$i]['width'] = 8 + rand(0, $this->width / 5 - 5);
				$widthtotal += $font[$i]['width'];
			}
		}
		$x = rand(1, $this->width - $widthtotal);
		for($i = 0; $i <= 3; $i++) {
			$this->color && $this->fontcolor = array(rand(0, 255), rand(0, 255), rand(0, 255));
			if($font[$i]['file']) {
				$this->imcode = imagecreatefromgif($font[$i]['file']);
				if($this->size) {
					$font[$i]['width'] = rand($font[$i]['width'] - $this->width / 20, $font[$i]['width'] + $this->width / 20);
					$font[$i]['height'] = rand($font[$i]['height'] - $this->width / 20, $font[$i]['height'] + $this->width / 20);
				}
	...
	function flash() {
		...
		$fontcolor = '0x'.(sprintf('%02s', dechex (rand(0, 255)))).(sprintf('%02s', dechex (rand(0, 128)))).(sprintf('%02s', dechex (rand(0, 255))));
		$strAction = "
    ...
    function adulterate() {
        ...
        for($i = 0; $i <= $linenums;$i++) {
            $color = $this->color ? imagecolorallocate($this->im, rand(0, 255), rand(0, 255), rand(0, 255)) : imagecolorallocate($this->im, $this->fontcolor[0], $this->fontcolor[1], $this->fontcolor[2]);
            $x = rand(0, $this->width);
    ...
    function adulteratefont() {
        ...
        $text_color = imagecolorallocate($this->im, $this->fontcolor[0], $this->fontcolor[1], $this->fontcolor[2]);
        for($i = 0; $i <= 3; $i++) {
            $adulteratecode = $seccodeunits[rand(0, 23)];
            imagechar($this->im, 5, $x * $i + rand(0, $x - 10), rand($y, $this->height - 10 - $y), $adulteratecode, $text_color);
        }
    }
