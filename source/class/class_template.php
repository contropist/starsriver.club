...
class template {
	...
	var $subtemplates = [];
	var $csscurmodules = '';
	var $replacecode = array('search' => [], 'replace' => []);
	var $blocks = [];
	var $language = [];
	var $file = '';
	...
	function parse_template($tplfile, $templateid, $tpldir, $file, $cachefile) {
		...
		$basefile = basename(DISCUZ_ROOT.$tplfile, $tpl_suffix);
	...
	function loadcsstemplate() {
		...
		$content = @implode('', file(DISCUZ_ROOT.'./data/cache/module.css'));
		...
		if($this->csscurmodules) {
			...
			if(@$fp = fopen(DISCUZ_ROOT.'./data/cache/'.$_G['basescript'].'_'.CURMODULE.'.css', 'w')) {
