...
class table_portal_category_permission extends discuz_table
{
	...
	public function fetch($catid, $uid){
		return ($catid = dintval($catid)) && ($uid = dintval($uid)) ? DB::fetch_first('SELECT * FROM %t WHERE catid=%d AND uid=%d', array($this->_table, $catid, $uid)) : [];
	}

	public function fetch_all_by_catid($catid, $uid = 0) {
		return ($catid = dintval($catid)) ? DB::fetch_all('SELECT * FROM %t WHERE catid=%d'.($uid ? ' AND '.DB::field('uid', $uid) : ''), array($this->_table, $catid), 'uid') :[];
	}
