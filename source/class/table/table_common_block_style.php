...
class table_common_block_style extends discuz_table
{
	...
	public function fetch_all_by_blockclass($blockclass) {
		return $blockclass ? DB::fetch_all('SELECT * FROM %t WHERE blockclass=%s', array($this->_table, $blockclass), $this->_pk) : [];
	}

	public function fetch_all_by_hash($hash) {
		return $hash ? DB::fetch_all('SELECT * FROM %t WHERE `hash` IN (%n)', array($this->_table, $hash), $this->_pk) : [];
	}
	...
	public function update($val, $data, $unbuffered = false, $low_priority = false) {
		if(($val = dintval($val, true)) && $data && is_array($data)) {
			...
			$this->_cache_ttl = getglobal('setting/memory/nestblock');
			$this->_allowmem = getglobal('setting/memory/nestblock') && memory('check');
