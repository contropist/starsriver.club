...
class table_common_member_grouppm extends discuz_table
{
	...
	public function fetch_all_by_gpmid($gpmid, $type, $start = 0, $limit = 0) {
		return $gpmid ? DB::fetch_all('SELECT * FROM %t WHERE gpmid=%d AND dateline'.($type ? '>' : '=').'0'.DB::limit($start, $limit), array($this->_table, $gpmid), 'uid') : [];
	}

	public function fetch_all_by_uid($uid, $type) {
		return $uid ? DB::fetch_all('SELECT * FROM %t WHERE uid=%d AND `status`'.($type ? '>=' : '=').'0', array($this->_table, $uid), 'gpmid') : [];
	}
