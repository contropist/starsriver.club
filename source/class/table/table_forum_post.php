...
class table_forum_post extends discuz_table
{
	...
	public function update_cache($tableid, $id, $idtype, $data, $condition = [], $glue = 'merge') {
	...
	public function increase_rate_by_pid($tableid, $pid, $rate, $ratetimes) {
		...
		if($return && $this->_allowmem) {
			$this->update_cache($tableid, $pid, 'pid', array('rate' => $rate, 'ratetimes' => $ratetimes), [], '+');
		}
		return $return;
	}
	...
	public function increase_status_by_pid($tableid, $pid, $status, $glue, $unbuffered = false) {
		...
		if($return && $this->_allowmem) {
			$this->update_cache($tableid, $pid, 'pid', array('status' => $status), [], $glue);
		}
		return $return;
	}
