    ...
    class model_forum_post extends discuz_model {
        ...
        public function newreply($parameters) {
            ...
            if (trim($this->param['subject']) == '' && trim($this->param['message']) == '' && $this->thread['special'] != 2) {
                ...
                return $this->showmessage($post_invalid, '', [
                    'minpostsize' => $this->setting['minpostsize'],
                    'maxpostsize' => $this->setting['maxpostsize'],
                ]);
            } elseif (checkflood()) {
            ...
            [
                ,
                $this->param['modnewreplies'],
            ] = threadmodstatus($this->param['subject'] . "\t" . $this->param['message'] . $this->param['extramessage']);
            ...
            if ($this->param['geoloc'] && IN_MOBILE == 2) {
                [
                    ...
                    $mapy,
                    $location,
                ] = explode('|', $this->param['geoloc']);
                if ($mapx && $mapy && $location) {
                    C::t('forum_post_location')->insert([
                        'pid'      => $this->pid,
                        'tid'      => $this->thread['tid'],
                        'uid'      => $this->member['uid'],
                        'mapx'     => $mapx,
            ...
            if (!empty($this->param['noticeauthor']) && !$this->param['isanonymous'] && !$this->param['modnewreplies']) {
                [
                    $ac,
                    $nauthorid,
                ] = explode('|', authcode($this->param['noticeauthor'], 'DECODE'));
                if ($nauthorid != $this->member['uid']) {
                    if ($ac == 'q') {
                        notification_add($nauthorid, 'post', 'reppost_noticeauthor', [
                            'tid'         => $this->thread['tid'],
            ...
            if ($this->thread['authorid'] != $this->member['uid'] && getstatus($this->thread['status'], 6) && empty($this->param['noticeauthor']) && !$this->param['isanonymous'] && !$this->param['modnewreplies']) {
                ...
                notification_add($thapost['authorid'], 'post', 'reppost_noticeauthor', [
                    'tid'         => $this->thread['tid'],
                    'subject'     => $this->thread['subject'],
                    'fid'         => $this->forum['fid'],
                    'pid'         => $this->pid,
            ...
            if ($this->param['modnewreplies']) {
                ...
                if (!empty($row)) {
                    C::t('forum_threadaddviews')->update($this->thread['tid'], ['addviews' => 0]);
                ...
                if ($this->thread['displayorder'] != -4) {
                    ...
                    if ($this->forum['status'] == 3) {
                        ...
                        C::t('forum_forumfield')->update($this->forum['fid'], ['lastupdate' => TIMESTAMP]);
                    ...
                    C::t('forum_forum')->update($this->forum['fid'], ['lastpost' => $lastpost]);
                    C::t('forum_forum')->update_forum_counter($this->forum['fid'], 0, 1, 1);
                    if ($this->forum['type'] == 'sub') {
                        C::t('forum_forum')->update($this->forum['fup'], ['lastpost' => $lastpost]);
                    }
                }
        ...
        public function editpost($parameters) {
            ...
            if ($isfirstpost) {
                ...
                if ($this->thread['tid'] > 1) {
                    if ($this->thread['closed'] > 1) {
                        C::t('forum_thread')->update($this->thread['closed'], ['subject' => $this->param['subject']], true);
                    } elseif (empty($this->thread['isgroup'])) {
                        $threadclosed = C::t('forum_threadclosed')->fetch($thread['tid']);
                        if ($threadclosed['redirect']) {
                            C::t('forum_thread')->update($threadclosed['redirect'], ['subject' => $this->param['subject']], true);
                        }
                    }
                }
                $class_tag = new tag();
            ...
            if ($this->setting['editedby'] && (TIMESTAMP - $this->post['dateline']) > 60 && $this->member['adminid'] != 1) {
                ...
                $this->param['message'] = lang('forum/misc', $this->param['htmlon'] ? 'post_edithtml' : (!$this->forum['allowbbcode'] || $this->param['bbcodeoff'] ? 'post_editnobbcode' : 'post_edit'), [
                        'editor'   => $editor,
                        'edittime' => $edittime,
                    ]) . $this->param['message'];
            }
            ...
            if (getglobal('forum_auditstatuson') && $this->param['audit'] == 1) {
                C::t('forum_post')->update($this->thread['posttableid'], $this->post['pid'], ['status' => 4], false, false, null, -2, null, 0);
            ...
            if (!getglobal('forum_auditstatuson') || $this->param['audit'] != 1) {
                if ($isfirstpost && $this->param['modnewthreads']) {
                    C::t('forum_thread')->update($this->thread['tid'], ['displayorder' => -2]);
                    manage_addnotify('verifythread');
                } elseif (!$isfirstpost && $this->param['modnewreplies']) {
                    C::t('forum_thread')->increase($this->thread['tid'], ['replies' => -1]);
                    manage_addnotify('verifypost');
                }
                if ($this->param['modnewreplies'] || $this->param['modnewthreads']) {
                    C::t('forum_forum')->update($this->forum['fid'], ['modworks' => '1']);
                }
            }
            ...
            if ($this->thread['lastpost'] == $this->post['dateline'] && ((!$this->post['anonymous'] && $this->thread['lastposter'] == $this->post['author']) || ($this->post['anonymous'] && $this->thread['lastposter'] == '')) && $this->post['anonymous'] != $this->param['isanonymous']) {
                C::t('forum_thread')->update($this->thread['tid'], ['lastposter' => $this->param['isanonymous'] ? '' : $this->post['author']], true);
            }
        ...
        public function deletepost($parameters) {
            ...
            if (!$this->param['handlereplycredit']) {
                if (!$isfirstpost && !$this->param['isanonymous']) {
                    ...
                    if ($postreplycredit) {
                        C::t('forum_post')->update('tid:' . $this->thread['tid'], $this->post['pid'], ['replycredit' => 0]);
            ...
            if ($isfirstpost) {
                ...
                if ($this->setting['globalstick'] && in_array($this->thread['displayorder'], [
                        2,
                        3,
                    ])) {
            ...
            if ($this->post['dateline'] == $this->forum['lastpost'][2] && ($this->post['author'] == $this->forum['lastpost'][3] || ($this->forum['lastpost'][3] == '' && $this->post['anonymous']))) {
                ...
                C::t('forum_forum')->update($this->forum['fid'], ['lastpost' => "$lastthread[tid]\t$lastthread[subject]\t$lastthread[lastpost]\t$lastthread[lastposter]"]);
            }
