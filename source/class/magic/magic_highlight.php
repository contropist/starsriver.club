...
class magic_highlight {
	...
	function usesubmit() {
		...
		if(!in_array($idtype, $this->idtypearray)) {
			showmessage(lang('magic/highlight', 'highlight_info_notype'), dreferer(), [], array('showdialog' => 1, 'locationtime' => true));
		}
		if(empty($_GET['id'])) {
		...
		showmessage(lang('magic/highlight', 'highlight_succeed_'.$idtype), dreferer(), [], array('alert' => 'right', 'showdialog' => 1, 'locationtime' => true));
	}
