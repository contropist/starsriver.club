...
if($_GET['nest']=='yes' && !$_G['group']['allowaddtopic'] && !$_G['group']['allowmanagetopic']) {
	$_GET['nest'] = '';
...
if($_GET['nest'] == 'yes' && $topic['uid'] != $_G['uid'] && !$_G['group']['allowmanagetopic']) {
	$_GET['nest'] = '';
	showmessage('topic_edit_nopermission');
}

if(!empty($_G['setting']['makehtml']['flag']) && $topic['htmlmade'] && !isset($_G['makehtml']) && empty($_GET['nest'])) {
...
include template('nest:'.$file, NULL, $tpldirectory, NULL, $primaltplname);
