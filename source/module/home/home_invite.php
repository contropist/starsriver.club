...
if($_G['setting']['regstatus'] < 2) {
	showmessage('not_open_invite', '', [], array('return' => true));
}
if($_G['uid']) {
	...
	if($_GET['accept'] == 'yes') {
		...
		if(empty($cookies)) {
			showmessage('invite_code_error', '', [], array('return' => true));
		}
		if(count($cookies) == 3) {
...
if($id) {
	...
	if($invite['fuid'] && $invite['fuid'] != $_G['uid']) {
		showmessage('invite_code_fuid', '', [], array('return' => true));
	}
	if($invite['endtime'] && $_G['timestamp'] > $invite['endtime']) {
		C::t('common_invite')->delete($id);
		showmessage('invite_code_endtime_error', '', [], array('return' => true));
	}
...
if($acceptconfirm) {
	...
	if($_G['uid'] == $uid) {
		showmessage('should_not_invite_your_own', '', [], array('return' => true));
	}
