...
function getranklist_member_blog($num) {
    ...
    $blogs = [];
    $sql = "SELECT m.uid,m.username,m.groupid,c.blogs FROM " . DB::table('common_member') . " m LEFT JOIN " . DB::table('common_member_count') . " c ON m.uid=c.uid WHERE c.blogs>0 ORDER BY blogs DESC LIMIT 0, $num";
