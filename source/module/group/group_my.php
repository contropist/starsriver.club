...
if($view == 'groupthread' || $view == 'mythread') {
	...
	if($view == 'groupthread' && empty($typeid) && !empty($usergroups['grouptype'])) {
		...
		foreach($attentiongroups as $groupid) {
			...
			if($page == 1) {
				foreach(C::t('forum_thread')->fetch_all_by_fid_displayorder($groupid, 0, null, null, 0, 10, 'lastpost', 'DESC', '=') as $thread) {
					...
					$attentionthread[$groupid][$thread['tid']]['dateline'] = date('Y-m-d',$thread['dateline']);
					...
					$attentionthread[$groupid][$thread['tid']]['folder'] = $thread['folder'];
					$attentionthread[$groupid][$thread['tid']]['special'] = $thread['special'];
	...
	if($mygrouplist) {
		...
		if($mygroupfid) {
			...
			foreach(C::t('forum_thread')->fetch_all_by_fid_authorid_displayorder($mygroupfid, $authorid, $displayorder, $lastpost, $start, $perpage) as $thread) {
				...
				$groupthreadlist[$thread['tid']]['dateline'] = dgmdate($thread['dateline'],'u');
				...
				$groupthreadlist[$thread['tid']]['folder'] = $thread['folder'];
	...
	$grouplist = mygrouplist($_G['uid'], 'lastupdate', array('name', 'icon','banner','description'), $perpage, $start, $ismanager);
}
...
include_once template("nest:group/group_my");
