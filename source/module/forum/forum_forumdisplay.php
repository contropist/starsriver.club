...
if($_G['forum']['type'] == 'forum') {
	...
	if(empty($_GET['archiveid'])) {
		$navigation = '<em class="direct"></em><a href="forum.php?gid='.$forum_up['fid'].'">'.$forum_up['name'].'</a><em class="direct"></em> <a href="forum.php?mod=forumdisplay&fid='.$_G['forum']['fid'].'">'.$_G['forum']['name'].'</a>';
	} else {
		$navigation = '<em class="direct"></em>'.'<a href="forum.php?mod=forumdisplay&fid='.$_G['fid'].'">'.$_G['forum']['name'].'</a><em class="direct"></em>'.$forumarchive[$_GET['archiveid']]['displayname'];
	}
	...
	if(empty($_GET['archiveid'])) {
		...
		$navigation = '<em class="direct"></em><a href="forum.php?gid='.$forum_top['fid'].'">'.$forum_top['name'].'</a><em class="direct"></em> <a href="forum.php?mod=forumdisplay&fid='.$forum_up['fid'].'">'.$forum_up['name'].'</a><em class="direct"></em> '.$_G['forum']['name'];
	} else {
		$navigation = '<em class="direct"></em><a href="forum.php?mod=forumdisplay&fid='.$_G['forum']['fup'].'">'.$forum_up['name'].'</a><em class="direct"></em>'.'<a href="forum.php?mod=forumdisplay&fid='.$_G['fid'].'">'.$_G['forum']['name'].'</a><em class="direct"></em>'.$forumarchive[$_GET['archiveid']]['displayname'];
	}
...
if($_G['forum']['status'] == 3) {
	...
	if($_G['forum']['level'] == -1) {
		showmessage('group_verify', '', [], array('alert' => 'info'));
	}
...
if($subforumonly) {
	...
	if(!defined('IN_ARCHIVER')) {
		include template('nest:forum/forumdisplay:'.$_G['fid']);
	} else {
...
foreach($threadlist as $thread) {
	...
	if($thread['typeid'] && !empty($_G['forum']['threadtypes']['prefix']) && isset($_G['forum']['threadtypes']['types'][$thread['typeid']])) {
		if($_G['forum']['threadtypes']['prefix'] == 1) {
			$thread['typehtml'] = '<a href="forum.php?mod=forumdisplay&fid='.$_G['fid'].'&amp;filter=typeid&amp;typeid='.$thread['typeid'].'">'.$_G['forum']['threadtypes']['types'][$thread['typeid']].'</a>';
		} elseif($_G['forum']['threadtypes']['icons'][$thread['typeid']] && $_G['forum']['threadtypes']['prefix'] == 2) {
			$thread['typehtml'] = '<a title="'.$_G['forum']['threadtypes']['types'][$thread['typeid']].'" href="forum.php?mod=forumdisplay&fid='.$_G['fid'].'&amp;filter=typeid&amp;typeid='.$thread['typeid'].'">'.'<img style="vertical-align: middle;padding-right:4px;" src="'.$_G['forum']['threadtypes']['icons'][$thread['typeid']].'" alt="'.$_G['forum']['threadtypes']['types'][$thread['typeid']].'" /></a>';
		}
	...
	$thread['sorthtml'] = $thread['sortid'] && !empty($_G['forum']['threadsorts']['prefix']) && isset($_G['forum']['threadsorts']['types'][$thread['sortid']]) ?
		'<a href="forum.php?mod=forumdisplay&fid='.$_G['fid'].'&amp;filter=sortid&amp;sortid='.$thread['sortid'].'">'.$_G['forum']['threadsorts']['types'][$thread['sortid']].'</a>' : '';
	$thread['multipage'] = '';
	...
	if($_G['forum']['status'] != 3 && ($thread['closed'] || ($_G['forum']['autoclose'] && $thread['fid'] == $_G['fid'] && TIMESTAMP - $thread[$closedby] > $_G['forum']['autoclose']))) {
		...
		$thread['folder'] = 'common';
		$thread['daynew'] = TIMESTAMP - 86400 <= $thread['dbdateline'];
...
$template = 'nest:forum/forumdisplay:'.$_G['fid'];

if($_G['forum']['status'] == 3) {
	$groupviewed_list = get_viewedgroup();
	write_groupviewed($_G['fid']);
	$template = 'nest:group/group:'.$_G['fid'];
}
