...
if($_G['fid']) {
	...
	$groupcache = getgroupcache($_G['fid'], array('replies', 'views', 'digest', 'lastpost', 'ranking', 'activityuser', 'newuserlist'), 86400);
...
if($action == 'index') {
	...
	if($status != 2) {
		...
		$newthreadlist = getgroupcache($_G['fid'], array('dateline'), 0, 15, 0, 1);
	...
	include template('nest:group/group:'.$_G['fid']);
	...
	if(!submitcheck('createsubmit')) {
		...
		updatestat('group');
		if($levelid == -1) {
			showmessage('group_create_mod_succeed', "group.php?mod=my&view=manager", [], array('alert' => 'right', 'showdialog' => 1, 'showmsg' => true, 'locationtime' => true));
		}
		showmessage('group_create_succeed', "forum.php?mod=group&action=manage&fid=$newfid", [], array('showdialog' => 1, 'showmsg' => true, 'locationtime' => true));
	}
	...
	if(submitcheck('grouprecommend')) {
		...
		showmessage('grouprecommend_succeed', '', [], array('alert' => 'right', 'closetime' => true, 'showdialog' => 1));
	} else {
