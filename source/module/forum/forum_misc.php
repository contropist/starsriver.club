...
if($_GET['action'] == 'paysucceed') {
	...
	if($attach['readperm'] && $attach['readperm'] > $_G['group']['readaccess']) {
		showmessage('attachment_forum_nopermission', NULL, [], array('login' => 1));
	}
	...
	if(C::t('common_credit_log')->count_by_uid_operation_relatedid($_G['uid'], 'BAC', $aid)) {
		showmessage('attachment_yetpay', "forum.php?mod=attachment&aid=$aidencode", [], array('redirectmsg' => 1));
	}
...
if($_GET['action'] == 'votepoll' && submitcheck('pollsubmit', 1)) {
	...
	if(!$_G['group']['allowvote']) {
		...
		showmessage('thread_poll_closed', NULL, [], array('login' => 1));
	...
	if(!$_G['uid']) {
		showmessage('not_loggedin', NULL, [], array('login' => 1));
	}
	...
	if(submitcheck('activitysubmit')) {
		...
		showmessage('activity_cancel_success', "forum.php?mod=viewthread&tid=$_G[tid]&do=viewapplylist".($_GET['from'] ? '&from='.$_GET['from'] :''), [], array('showdialog' => 1, 'closetime' => true));
	}
		...
		if(empty($_GET['applyidarray'])) {
			...
			if($_GET['operation'] == 'notification') {
				...
				if($uidarray) {
					...
					showmessage('activity_notification_success', "forum.php?mod=viewthread&tid=$_G[tid]&do=viewapplylist".($_GET['from'] ? '&from='.$_GET['from'] : ''), [], array('showdialog' => 1, 'closetime' => true));
				}
				...
				$applynumber = C::t('forum_activityapply')->fetch_count_for_thread($_G['tid']);
				C::t('forum_activity')->update($_G['tid'], array('applynumber' => $applynumber));
				showmessage('activity_delete_completion', "forum.php?mod=viewthread&tid=$_G[tid]&do=viewapplylist".($_GET['from'] ? '&from='.$_GET['from'] : ''), [], array('showdialog' => 1, 'closetime' => true));
			} else {
				if($unverified) {
				...
				showmessage('activity_auditing_completion', "forum.php?mod=viewthread&tid=$_G[tid]&do=viewapplylist".($_GET['from'] ? '&from='.$_GET['from'] : ''), [], array('showdialog' => 1, 'closetime' => true));
			}
		}
	}
	...
	if(!submitcheck('tradesubmit')) {
		...
		$stickcount = 0;$trades = $tradesstick = [];
	...
	if(!$_G['uid']) {
		showmessage('debate_poll_nopermission', NULL, [], array('login' => 1));
	}
	...
	if($isfirst) {
		...
		showmessage('debate_poll_succeed', 'forum.php?mod=viewthread&tid='.$_G['tid'], [], array('showmsg' => 1, 'locationtime' => true));
	}
	...
	if($debate['uid'] == $_G['uid']) {
		showmessage('debate_poll_myself', "forum.php?mod=viewthread&tid=$_G[tid]".($_GET['from'] ? '&from='.$_GET['from'] : ''), [], array('showmsg' => 1));
	} elseif(strpos("\t".$debate['voterids'], "\t$_G[uid]\t") !== FALSE) {
		showmessage('debate_poll_voted', "forum.php?mod=viewthread&tid=$_G[tid]".($_GET['from'] ? '&from='.$_GET['from'] : ''), [], array('showmsg' => 1));
	} elseif($debate['endtime'] && $debate['endtime'] < TIMESTAMP) {
		showmessage('debate_poll_end', "forum.php?mod=viewthread&tid=$_G[tid]".($_GET['from'] ? '&from='.$_GET['from'] : ''), [], array('showmsg' => 1));
	}

	C::t('forum_debatepost')->update_voters($_GET['pid'], $_G['uid']);

	showmessage('debate_poll_succeed', "forum.php?mod=viewthread&tid=$_G[tid]".($_GET['from'] ? '&from='.$_GET['from'] : ''), [], array('showmsg' => 1));
	...
	if(empty($_G['uid'])) {
		showmessage('to_login', null, [], array('showmsg' => true, 'login' => 1));
	}
	...
	if($_G['tid'] && $_G['group']['alloweditusertag']) {
		if(!submitcheck('addusertag')) {
			...
			if($tagarray) {
				...
				showmessage('forum_usertag_succeed', '', [], array('alert' => 'right'));
			} else {
	...
	if($_G['uid'] == $post['authorid']) {
		showmessage('noreply_yourself_error', '', [], array('msgtype' => 3));
	}
	...
	if(empty($hotreply)) {
		...
		if(C::t('forum_hotreply_member')->fetch($post['pid'], $_G['uid'])) {
			showmessage('noreply_voted_error', '', [], array('msgtype' => 3));
		}
	}
