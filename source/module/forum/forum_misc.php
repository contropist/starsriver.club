 ...
 ********************************************************************/
    ...
    if ($_GET['action'] == 'paysucceed') {
        ...
        showmessage('attachment_credit', "forum.php?mod=attachment&aid=$aidencode&ck=$ck", [
            'policymsg' => $_G['policymsg'],
        ...
        if (!submitcheck('paysubmit')) {
            ...
            if (!empty($_GET['buyall'])) {
                ...
                foreach (C::t('forum_attachment_n')->fetch_all_by_id('aid:' . $aid, 'pid', $attach['pid'], '', false, true) as $tmp) {
                    ...
                    $prices[$tmp['aid']] = $status != 2 ? [
                        $tmp['price'],
                        round($tmp['price'] * (1 - $_G['setting']['creditstax'])),
                    ] : [
                        0,
                        0,
                    ];
            ...
            if ($status == 1) {
                showmessage('credits_balance_insufficient', '', [
                    'title'      => $_G['setting']['extcredits'][$_G['setting']['creditstransextra'][1]]['title'],
                    'minbalance' => $attach['price'],
                ]);
            }
            foreach ($aids as $aid) {
                $updateauthor = 1;
        ...
        $type = !empty($_GET['type']) && in_array($_GET['type'], [
            'image',
            'file',
        ]) ? $_GET['type'] : 'image';
        $attachexts = $imgexts = '';
        ...
        if (!$post || !($_G['setting']['commentpostself'] || $post['authorid'] != $_G['uid']) || !(($post['first'] && $_G['setting']['commentfirstpost'] && in_array($_G['group']['allowcommentpost'], [
                    1,
                    3,
                ]) || (!$post['first'] && in_array($_G['group']['allowcommentpost'], [
                        2,
                        3,
                    ]))))) {
        ...
        foreach (C::t('forum_postcomment')->fetch_all_by_search(null, $_GET['pid'], null, null, null, null, null, $start_limit, $commentlimit) as $comment) {
            ...
            $comment['comment'] = str_replace([
                ...
                '</b>',
                '</font>',
            ], preg_replace("/\[color=([#\w]+?)\]/i", "<font color=\"\\1\">", $comment['comment']));
            $comments[] = $comment;
        }
        ...
        if (submitcheck('postappendsubmit')) {
            ...
            C::t('forum_post')->update('tid:' . $_G['tid'], $_GET['pid'], [
                'message'   => $message,
                'bbcodeoff' => $bbcodeoff,
                'port'      => $_G['remoteport'],
            ]);
            showmessage('postappend_add_succeed', "forum.php?mod=viewthread&tid=$post[tid]&pid=$post[pid]&page=$_GET[page]&extra=$_GET[extra]#pid$post[pid]", [
                'tid' => $post['tid'],
                'pid' => $post['pid'],
            ]);
        } else {
        ...
        if (!empty($tid)) {
            ...
            if ($thread['authorid'] == $_G['uid']) {
                ...
                C::t('forum_thread')->update($tid, ['status' => $thread['status']], true);
        ...
        C::t('forum_thread')->update($_G['tid'], ['heats' => 0]);
        ...
        if ($_G['setting']['darkroom']) {
            ...
            $crimelist = [];
            $i = 0;
            foreach (C::t('common_member_crime')->fetch_all_by_cid($cid, [
                4,
                5,
            ], $limit) as $crime) {
                $i++;
...
';
        ...
        if ($_G['forum']['type'] == 'forum') {
            $navigation = '<a href="forum.php">' . $_G['setting']['navs'][2]['navname'] . '</a><em class="direct"></em><a href="forum.php?mod=forumdisplay&fid=' . $_G['fid'] . '">' . $_G['forum']['name'] . '</a><em class="direct"></em><a href="forum.php?mod=viewthread&tid=' . $_G['tid'] . '">' . $thread['subject'] . '</a>';
            $navtitle = strip_tags($_G['forum']['name']) . ' - ' . $thread['subject'];
        } elseif ($_G['forum']['type'] == 'sub') {
            $fup = C::t('forum_forum')->fetch($_G['forum']['fup']);
            $navigation = '<a href="forum.php">' . $_G['setting']['navs'][2]['navname'] . '</a><em class="direct"></em><a href="forum.php?mod=forumdisplay&fid=' . $fup['fid'] . '">' . $fup['name'] . '</a><em class="direct"></em><a href="forum.php?mod=forumdisplay&fid=' . $_G['fid'] . '">' . $_G['forum']['name'] . '</a><em class="direct"></em><a href="forum.php?mod=viewthread&tid=' . $_G['tid'] . '">' . $thread['subject'] . '</a>';
    ...
    if ($_GET['action'] == 'votepoll' && submitcheck('pollsubmit', 1)) {
        ...
        if (!$poll) {
            ...
            showmessage('poll_choose_most', null, ['maxchoices' => $poll['maxchoices']], ['login' => 1]);
        }
        ...
        if ($overt && !empty($space['privacy']['feed']['newreply'])) {
            ...
            getThread_sample($_G['tid'], $feed['body_data']['original']);
        ...
        foreach ($query as $options) {
            ...
            $options['polloption'] = preg_replace("/\[url=(https?){1}:\/\/([^\[\"']+?)\](.+?)\[\/url\]/i", "<a href=\"\\1://\\2\" target=\"_blank\">\\3</a>", $options['polloption']);
            $polloptions[] = $options;
        }
        ...
        if (!submitcheck('ratesubmit')) {
            ...
            if ($post['first']) {
                ...
                C::t('forum_thread')->update($_G['tid'], ['rate' => $threadrate]);
        ...
        if (($balance = getuserprofile('extcredits' . $_G['setting']['creditstransextra'][1]) - $thread['price']) < ($minbalance = 0)) {
            if ($_G['setting']['creditstrans'][0] == $_G['setting']['creditstransextra'][1]) {
                showmessage('credits_balance_insufficient_and_charge', '', [
                    'title'      => $_G['setting']['extcredits'][$_G['setting']['creditstransextra'][1]]['title'],
                    'minbalance' => $thread['price'],
                ]);
            } else {
                showmessage('credits_balance_insufficient', '', [
                    'title'      => $_G['setting']['extcredits'][$_G['setting']['creditstransextra'][1]]['title'],
        ...
        C::t('forum_thread')->update($_G['tid'], ['price' => $thread['price']]);
        C::t('forum_post')->update('tid:' . $_G['tid'], $_GET['pid'], [
        ...
        if (submitcheck('activitysubmit')) {
            ...
            if ($thread['authorid'] != $_G['uid']) {
                ...
                if (!empty($space['privacy']['feed']['newreply'])) {
                    ...
                    // Thread data
                    require_once libfile('function/thread');
                    getThread_sample($_G['tid'], $feed['body_data']['original'], [$activity['aid']]);
            ...
            showmessage('activity_completion', "forum.php?mod=viewthread&tid=$_G[tid]" . ($_GET['from'] ? '&from=' . $_GET['from'] : ''), [], [
                'showdialog'   => 1,
                'showmsg'      => true,
                'locationtime' => true,
                'alert'        => 'right',
            ...
            showmessage('activity_cancel_success', "forum.php?mod=viewthread&tid=$_G[tid]&do=viewapplylist" . ($_GET['from'] ? '&from=' . $_GET['from'] : ''), [], [
                'showdialog' => 1,
                'closetime'  => true,
            ]);
        }
        ...
        $isactivitymaster = $thread['authorid'] == $_G['uid'] || (in_array($_G['group']['radminid'], [
                    1,
                    2,
                ]) || ($_G['group']['radminid'] == 3 && $_G['forum']['ismoderator']) && $_G['group']['alloweditactivity']);
        if (!$isactivitymaster) {
                ...
                if ($_GET['operation'] == 'notification') {
                    ...
                    if ($uidarray) {
                        foreach ($uidarray as $uid) {
                            notification_add($uid, 'activity', 'activity_notification', [
                                'tid'     => $_G['tid'],
                                'subject' => $activity_subject,
                                'msg'     => $reason,
                            ]);
                        }
                        showmessage('activity_notification_success', "forum.php?mod=viewthread&tid=$_G[tid]&do=viewapplylist" . ($_GET['from'] ? '&from=' . $_GET['from'] : ''), [], [
                            'showdialog' => 1,
                            'closetime'  => true,
                        ]);
                    }
                    ...
                    $applynumber = C::t('forum_activityapply')->fetch_count_for_thread($_G['tid']);
                    C::t('forum_activity')->update($_G['tid'], ['applynumber' => $applynumber]);
                    showmessage('activity_delete_completion', "forum.php?mod=viewthread&tid=$_G[tid]&do=viewapplylist" . ($_GET['from'] ? '&from=' . $_GET['from'] : ''), [], [
                        'showdialog' => 1,
                        'closetime'  => true,
                    ]);
                } else {
                    ...
                    showmessage('activity_auditing_completion', "forum.php?mod=viewthread&tid=$_G[tid]&do=viewapplylist" . ($_GET['from'] ? '&from=' . $_GET['from'] : ''), [], [
                        'showdialog' => 1,
                        'closetime'  => true,
                    ]);
                }
        ...
        if (!submitcheck('tradesubmit')) {
            ...
            include template('forum/viewthread_trade_displayorder');
            ...
            foreach ($query as $trade) {
                ...
                C::t('forum_trade')->update($_G['tid'], $trade['pid'], ['displayorder' => $displayordernew]);
            }
        ...
        if ($isfirst) {
            ...
            showmessage('debate_poll_succeed', 'forum.php?mod=viewthread&tid=' . $_G['tid'], [], [
                'showmsg'      => 1,
                'locationtime' => true,
            ]);
        }
        ...
        if (!submitcheck('umpiresubmit')) {
            ...
            if (!($bestdebaterstand = C::t('forum_debatepost')->get_stand_by_bestuid($_G['tid'], $bestdebateruid, [
                $debate['uid'],
                $_G['uid'],
            ]))) {
                showmessage('debate_umpire_bestdebater_invalid');
            ...
            [
                $bestdebatervoters,
                $bestdebaterreplies,
            ] = C::t('forum_debatepost')->get_numbers_by_bestuid($_G['tid'], $bestdebateruid);
            
            $umpirepoint = dhtmlspecialchars($_GET['umpirepoint']);
            $bestdebater = dhtmlspecialchars($_GET['bestdebater']);
            $winner = intval($_GET['winner']);
            C::t('forum_thread')->update($_G['tid'], ['closed' => 1]);
            C::t('forum_debate')->update($_G['tid'], [
                'umpirepoint' => $umpirepoint,
                'winner'      => $winner,
                'bestdebater' => "$bestdebater\t$bestdebateruid\t$bestdebaterstand\t$bestdebatervoters\t$bestdebaterreplies",
        ...
        if (empty($thread['closed'])) {
            C::t('forum_thread')->update($_G['tid'], ['lastpost' => TIMESTAMP]);
        }
        C::t('forum_memberrecommend')->insert([
            'tid'          => $_G['tid'],
            'recommenduid' => $_G['uid'],
            'dateline'     => $_G['timestamp'],
        ]);
        ...
        if ($_G['setting']['recommendthread']['daycount']) {
            ...
            showmessage('recommend_daycount_succeed', '', [
                'recommendv' => $recommendv,
                'recommendc' => $thread['recommends'],
                'daycount'   => $daycount,
            ], ['msgtype' => 3]);
        } else {
            showmessage('recommend_succeed', '', [
                'recommendv' => $recommendv,
                'recommendc' => $thread['recommends'],
            ], ['msgtype' => 3]);
        }
            ...
            if (!submitcheck('addusertag')) {
                ...
                if ($tagarray) {
                    ...
                    if ($uids) {
                        ...
                        showmessage('forum_usertag_set_continue', '', [
                            'limit' => $limit,
                            'next'  => min($limit + $per, $count),
                            'count' => $count,
                        ], ['alert' => 'right']);
                    }
        ...
        if (empty($hotreply)) {
            $hotreply['pid'] = C::t('forum_hotreply_number')->insert([
                'pid'     => $post['pid'],
                'tid'     => $post['tid'],
                'support' => 0,
                'against' => 0,
        ...
        C::t('forum_hotreply_member')->insert([
            'tid'      => $post['tid'],
            'pid'      => $post['pid'],
            'uid'      => $_G['uid'],
            'attitude' => $typeid,
        ...
        showmessage('thread_poll_succeed', '', [], [
            'msgtype' => 3,
            'extrajs' => '<script>postreviewupdate(' . $post['pid'] . ', ' . $typeid . ');</script>',
        ]);
        ...
        if ($thread['hidden'] + 1 == $_G['setting']['threadhidethreshold']) {
            notification_add($thread['authorid'], 'post', 'thread_hidden', [
                'tid'     => $thread['tid'],
                'subject' => $thread['subject'],
            ], 1);
        }
        ...
        showmessage('thread_hidden_success', dreferer(), [], [
            'showdialog' => true,
            'closetime'  => true,
            'extrajs'    => '<script reload="1">$(\'normalthread_' . $_GET['tid'] . '\').style.display = \'none\'</script>',
        ]);
