...
if($_GET['action'] == 'checkusername') {
	...
	if($usernamelen < 3) {
		showmessage('profile_username_tooshort', '', [], array('handle' => false));
	} elseif($usernamelen > 15) {
		showmessage('profile_username_toolong', '', [], array('handle' => false));
	}
	...
	if($ucresult == -1) {
		showmessage('profile_username_illegal', '', [], array('handle' => false));
	} elseif($ucresult == -2) {
		showmessage('profile_username_protect', '', [], array('handle' => false));
	} elseif($ucresult == -3) {
		if(C::t('common_member')->fetch_by_username($username) || C::t('common_member_archive')->fetch_by_username($username)) {
			showmessage('register_check_found', '', [], array('handle' => false));
		} else {
			showmessage('register_activation', '', [], array('handle' => false));
		}
	}
	...
	if(!$invitecode) {
		showmessage('no_invitation_code', '', [], array('handle' => false));
	}
	$result = [];
	...
	if(empty($result)) {
		showmessage('wrong_invitation_code', '', [], array('handle' => false));
	}
	...
	if(C::t('common_member')->fetch_by_username(trim($_GET['username'])) || C::t('common_member_archive')->fetch_by_username(trim($_GET['username']))) {
		showmessage('<img width="20px" src="'.IMGURL.'/common/icon/checked.svg"> 用户有效', '', [], array('msgtype' => 3));
	} else {
		showmessage('username_nonexistence', '', [], array('msgtype' => 3));
	}
	...
	if(empty($groupname)) {
		showmessage('group_name_empty', '', [], array('msgtype' => 3));
	}
	$tmpname = cutstr($groupname, 20, '');
	if($tmpname != $groupname) {
		showmessage('group_name_oversize', '', [], array('msgtype' => 3));
	}
	if(C::t('forum_forum')->fetch_fid_by_name($groupname)) {
		showmessage('group_name_exist', '', [], array('msgtype' => 3));
	}
	showmessage('', '', [], array('msgtype' => 3));
	include template('common/header_ajax');
	include template('common/footer_ajax');
	dexit();
	...
	include template('common/header_ajax');
	if($image['aid']) {
		echo '<img src="'.getforumimg($image['aid'], 1, 300, 300, 'fixnone').'" id="image_'.$image['aid'].'" onclick="insertAttachimgTag(\''.$image['aid'].'\')" width="'.($image['width'] < 120 ? $image['width'] : 120).'" cwidth="'.($image['width'] < 300 ? $image['width'] : 300).'" />';
	}
	include template('common/footer_ajax');
	dexit();
	...
	if($_G['forum'] && ($aid || $imgurl)) {
		...
		if(empty($thread) || (!$_G['forum']['ismoderator'] && $_G['uid'] != $thread['authorid'])) {
			if($_GET['newthread']) {
				showmessage('set_cover_faild', '', [], array('msgtype' => 3));
			} else {
				showmessage('set_cover_faild', '', [], array('closetime' => 3));
			}
		}
		if(setthreadcover($pid, $tid, $aid, 0, $imgurl)) {
			...
			if($_GET['newthread']) {
				showmessage('set_cover_succeed', '', [], array('msgtype' => 3));
			} else {
				showmessage('set_cover_succeed', '', [], array('alert' => 'right', 'closetime' => 1));
			}
		}
	}
	if($_GET['newthread']) {
...
EOF;
	...
	if(!empty($threadclass)) {
		$threadclass = '<select name="typeid" id="typeid" width="168">'.$threadclass.'</select>';
	}
	include template('common/header_ajax');
	echo $threadclass;
	include template('common/footer_ajax');
	...
	if(!submitcheck('qclearsubmit')) {
		...
		showmessage('quickclear_success', $_POST['redirect'], [], array('showdialog'=>1, 'closetime' => true, 'msgtype' => 2, 'locationtime' => 1));
	}
	...
	if(!submitcheck('funcsubmit')) {
		...
		showmessage('do_success', dreferer(), [], array('header'=>true));
	}
	exit;
...
showmessage('succeed', '', [], array('handle' => false));
