...
if(!$gid && $_G['setting']['collectionstatus'] && ($_G['setting']['collectionrecommendnum'] || !$_G['setting']['hidefollowcollection'])) {
	...
	if($_G['setting']['showfollowcollection']) {
		$followcollections = $_G['uid'] ? C::t('forum_collectionfollow')->fetch_all_by_uid($_G['uid']) : [];;
...
if(empty($gid) && empty($_G['member']['accessmasks']) && empty($showoldetails)) {
	...
	if(defined('FORUM_INDEX_PAGE_MEMORY') && FORUM_INDEX_PAGE_MEMORY) {
		...
		if(!defined('IN_ARCHIVER')) {
			include template('nest:forum/discuz');
		} else {
...
if($_G['setting']['grid']['showgrid']) {
	...
	if(TIMESTAMP - $_G['cache']['grids']['cachetime'] < $cachelife) {
		...
		if($_G['setting']['grid']['gridtype']) {
			$grids['digest'] = C::t('forum_thread')->fetch_all_for_guide('digest', 0, [], 3, 0, 0, 8, $_G['setting']['grid']['fids']);
		} else {
		...
		$grids['newthread'] = C::t('forum_thread')->fetch_all_for_guide('newthread', 0, [], 0, 0, 0, 8, $_G['setting']['grid']['fids']);

		$grids['newreply'] = C::t('forum_thread')->fetch_all_for_guide('reply', 0, [], 0, 0, 0, 8, $_G['setting']['grid']['fids']);
		$grids['hot'] = C::t('forum_thread')->fetch_all_for_guide('hot', 0, [], 3, 0, 0, 8, $_G['setting']['grid']['fids']);
...
include template('nest:forum/discuz:'.$gid);
		...
		foreach($_G['cache']['announcements'] as $announcement) {
			if(!$announcement['endtime'] || $announcement['endtime'] > TIMESTAMP && (empty($announcement['groups']) || in_array($_G['member']['groupid'], $announcement['groups']))) {
				if(empty($announcement['type'])) {
					$announcements .= '<li><span><a href="forum.php?mod=announcement&id='.$announcement['id'].'" target="_blank">'.$announcement['subject'].
						'</a></span><em>('.dgmdate($announcement['starttime'], 'd').')</em></li>';
				} elseif($announcement['type'] == 1) {
					$announcements .= '<li><span><a href="'.$announcement['message'].'" target="_blank">'.$announcement['subject'].
						'</a></span><em>('.dgmdate($announcement['starttime'], 'd').')</em></li>';
				}
			}
		}
...
function get_index_page_guest_cache() {
	...
	if(TIMESTAMP - $indexcache['filemtime'] > $_G['setting']['cacheindexlife']) {
        ...
        echo '<script type="text/javascript">$("debuginfo") ? $("debuginfo").innerHTML = ", Updated at '.$updatetime.', Processed in '.sprintf("%0.6f", microtime(TRUE) - $start_time).' second(s)'.$gzip.'." : "";</script>';
        ob_end_flush();
		exit();
	}
}
