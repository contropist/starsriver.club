...
if($_GET['from'] == 'portal') {
	...
	$navigation = '<em class="direct"></em><a href="portal.php">'.lang('core', 'portal').'</a>';
	...
	$navigation = '<em class="direct"></em><a href="group.php">'.$_G['setting']['navs'][3]['navname'].'</a> '.$nav['nav'];
	...
	if($_G['forum']['type'] == 'sub') {
		...
		$navigation .= '<em class="direct"></em><a href="'.$t_link.'">'.($_G['cache']['forums'][$fup]['name']).'</a>';
	}
	...
	$navigation .= '<em class="direct"></em><a href="'.$t_link.'">'.($_G['forum']['name']).'</a>';
	...
	if($archiveid) {
		...
		$navigation .= '<em class="direct"></em><a href="forum.php?mod=forumdisplay&fid='.$_G['fid'].'&archiveid='.$archiveid.'">'.$t_name.'</a>';
	}
...
$allowblockrecommend = $_G['group']['allownest'] || getstatus($_G['member']['allowadmincp'], 4) || getstatus($_G['member']['allowadmincp'], 5) || getstatus($_G['member']['allowadmincp'], 6);
...
if(empty($_GET['viewpid'])) {
	...
	include template('nest:forum/viewthread'.$sufix.':'.$_G['fid']);
} else {
	$_G['setting']['admode'] = 0;
...
function viewthread_loadcache() {
    ...
    $threadcachemark = 100
        - 20*$_G['thread']['digest']
	    - min(50, $_G['thread']['views'] / max($_G['forum']['livedays'], 10) * 2)
        - min(15, $_G['thread']['replies'] / $_G['setting']['postperpage'] * 1.5)
...
function getLinkByKey($key, $post, $returnarray = 0) {
	...
	if(!$returnarray) {
		if($v['link']) {
			$v = '<a href="'.$v['link'].'" target="_blank">'.$v['value'].'</a>';
		} else {
			$v = $v['value'];
		}
	}
