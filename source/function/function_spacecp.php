...
function pic_save($FILE, $albumid, $title, $iswatermark = true, $catid = 0) {
	...
	if($_G['setting']['maxthumbwidth'] && $_G['setting']['maxthumbheight']) {
		if($_G['setting']['maxthumbwidth'] < 815) $_G['setting']['maxthumbwidth'] = 815;
		if($_G['setting']['maxthumbheight'] < 1600) $_G['setting']['maxthumbheight'] = 1600;
...
function ckrealname($return=0) {
	...
	if($_G['adminid'] != 1 && $_G['setting']['verify'][6]['available'] && empty($_G['setting']['verify'][6]['viewrealname'])) {
		...
		if(empty($_G['member']['realname']) || !$_G['member']['verify6']) {
			if(empty($return)) showmessage('no_privilege_realname', '', [], array('return' => true));
			$result = false;
		}
	}
	return $result;
...
function ckvideophoto($tospace=[], $return=0) {
	global $_G;
	...
	if($return) {
		...
		showmessage('no_privilege_videophoto', '', [], array('return' => true));
	}
}
