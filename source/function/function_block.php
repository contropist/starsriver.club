...
function block_fetch_content($bid, $isjscall=false, $forceupdate=false) {
	...
	if($allowmem === null) {
		$allowmem = ($cachettl = getglobal('setting/memory/nestblockoutput')) !== null && memory('check');
	}
	...
	if(!$hidediv) {
		...
		if(($_GET['nest'] === 'yes' || $_GET['inajax']) && check_nest_perm()) {
...
function block_template($bid) {
	...
	if(!empty($block['itemlist'])) {
		...
		$fields = array('picwidth'=>[], 'picheight'=>[], 'target'=>[], 'currentorder'=>[]);
...
function block_makeform($blocksetting, $values){
	...
	foreach($blocksetting as $settingvar => $setting) {
		...
		if($type == 'radio') {
			...
			$s .= '<label for="randomid_'.(++$randomid).'"><input type="radio" name="'.$varname.'" id="randomid_'.$randomid.'" value="1" '.$check['true'].'>'.lang('core', 'yes').'</label>'.
				'<label for="randomid_'.(++$randomid).'"><input type="radio" name="'.$varname.'" id="randomid_'.$randomid.'" value="0" '.$check['false'].'>'.lang('core', 'no').'</label>';
		} elseif($type == 'text' || $type == 'password' || $type == 'number') {
			$s .= '<input type="'.$type.'" name="'.$varname.'" value="'.dhtmlspecialchars($value).'" />';
		} elseif($type == 'textarea') {
			$s .= '<textarea name="'.$varname.'" rows="4" cols="40">'.dhtmlspecialchars($value).'</textarea>';
		} elseif($type == 'mtextarea') {
			$s .= '<textarea name="'.$varname.'" rows="4" cols="40" onblur="blockCheckTag(this);">'.dhtmlspecialchars($value).'</textarea>';
		} elseif($type == 'select') {
			$s .= '<select name="'.$varname[0].'">';
			...
			if(is_array($varname)) {
				...
				foreach($varname[1] as $varary) {
					if(is_array($varary) && !empty($varary)) {
						$s .= '<li'.($radiocheck[$varary[0]] ? ' class="checked"' : '').'><label for="randomid_'.(++$randomid).'"><input type="radio" name="'.$varname[0].'" id="randomid_'.$randomid.'" value="'.$varary[0].'"'.$radiocheck[$varary[0]].'>'.($langscript ? lang('block/'.$langscript, $varary[1]) : $varary[1]).'</label></li>';
					}
				}
				$s .= '</ul>';
			}
			...
			foreach($varname[1] as $varary) {
				if(is_array($varary) && !empty($varary)) {
					...
					$s .= '<li'.($checked ? ' class="checked"' : '').'><label for="randomid_'.(++$randomid).'"><input type="checkbox" name="'.$varname[0].'[]" id="randomid_'.$randomid.'" value="'.$varary[0].'"'.$checked.'>'.($langscript ? lang('block/'.$langscript, $varary[1]) : $varary[1]).'</label></li>';
				}
			}
			$s .= '</ul>';
		} elseif($type == 'mselect') {
			$s .= '<select name="'.$varname[0].'" multiple="multiple" size="10">';
			foreach($varname[1] as $option) {
			...
			if(! $calendar_loaded) {
				$s .= "<script src=\"{$_G[setting][jspath]}calendar.js?".VERHASH."\"></script>";
				$calendar_loaded = true;
			}
			$s .= '<input type="text" name="'.$varname.'" value="'.dhtmlspecialchars($value).'" onclick="showcalendar(event, this, true)" />';
			...
			if($districthtml) {
				$s .= $districthtml;
				$s .= '&nbsp;&nbsp;<a onclick="showdistrict(\''.$containerid.'\', ['.dimplode($elems).'], '.count($elems).', \'\', \''.$containertype.'\'); return false;">'.lang('spacecp', 'profile_edit').'</a>';
...
function block_getnesturl($tplname, $nestmod = false) {
	$mod = $id = $script = $url = '';
	$flag = 0;
	if (empty ($tplname)) {
		$flag = 2;
		...
		$url = empty($mod) || $flag == '1' ? '' : $script.'.php?mod='.$mod.($nestmod?'&nest=yes':'');
	}
...
function block_getstyle($styleids = []) {
	...
	static $allowmem = null, $cachettl =null;
	if($allowmem === null) {
		$allowmem = ($cachettl = getglobal('setting/memory/nestblock')) !== null && memory('check');
	}
