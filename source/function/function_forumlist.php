...
function getcacheinfo($tid) {
	...
	$cache['filename'] = $fulldir.$tid.$_G['config']['output']['tpl_suffix'];
...
function recommendupdate($fid, &$modrecommend, $force = '', $position = 0) {
	...
	$recommendlist = $recommendimagelist = $modedtids = [];
...
function forumleftside() {
	global $_G;
	$leftside = array('favorites' => [], 'forums' => []);
