...
function get_groupnav($forum) {
    ...
    if ($firsttype) {
        $groupnav = '<em class="direct"></em><a href="group.php?gid=' . $firsttype['fid'] . '">' . $firsttype['name'] . '</a>';
    }
    if ($secondtype) {
        $groupnav .= '<em class="direct"></em><a href="group.php?sgid=' . $secondtype['fid'] . '">' . $secondtype['name'] . '</a>';
    }
    if ($forum['type'] == 'sub') {
        $mod_action = $_GET['mod'] == 'forumdisplay' || $_GET['mod'] == 'viewthread' ? 'mod=forumdisplay&action=list' : 'mod=group';
        $groupnav .= ($groupnav ? '<em class="direct"></em>' : '') . '<a href="forum.php?' . $mod_action . '&fid=' . $forum['fid'] . '">' . $forum['name'] . '</a>';
    }
    ...
    if ($groupviewed) {
        ...
        foreach ($query as $row) {
            $list[$row['fid']] = [
                ...
                'banner' => get_groupimg($row['banner'], 'banner'),
...
function grouponline($fid, $getlist = '') {
    ...
    if (empty($getlist)) {
        ...
        $onlinemember = array('count' => 0, 'list' => []);
