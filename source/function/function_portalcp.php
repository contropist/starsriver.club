...
function get_uploadcontent($attach, $type='portal', $dotype='') {
	...
	if($attach['isimage']) {
		...
		$return .= '<a class="opattach"><span class="opattach_ctrl">';
		...
		$return .= '<label for="setconver'.$attach['attachid'].'" class="cur1 xi2"><input type="radio" name="setconver" id="setconver'.$attach['attachid'].'" value="1" onclick="setConver(\''.addslashes(serialize(array('pic'=>$type.'/'.$attach['attachment'], 'thumb'=>$attach['thumb'], 'remote'=>$attach['remote']))).'\') '.$check.'>'.lang('portalcp', 'set_to_conver').'</label>';
		$return .= '';
...
function get_upload_content($attachs, $dotype='') {
	...
	foreach($attachs as $key => $attach) {
		...
		if($attach['isimage']) {
			...
			$html .= '<a class="opattach">';
			$html .= '<span class="opattach_ctrl">';
			$html .= '<span onclick="insertImage(\''.$pic.'\');" class="cur1">'.lang('portalcp', 'insert_large_image').'</span>';
			...
			$html .= '<label for="setconver'.$attach['attachid'].'" class="cur1 xi2"><input type="radio" name="setconver" id="setconver'.$attach['attachid'].'" value="1" onclick=setConver(\''.addslashes(serialize(array('pic'=>$type.'/'.$attach['attachment'], 'thumb'=>$attach['thumb'], 'remote'=>$attach['remote']))).'\') '.$check.'>'.lang('portalcp', 'set_to_conver').'</label>';
			if($type == 'portal') {
				$html .= '<span class="cur1 xi2" onclick="deleteAttach(\''.$attach['attachid'].'\', \'portal.php?mod=attachment&id='.$attach['attachid'].'&aid='.$aid.'&op=delete\');">'.lang('portalcp', 'delete').'</span>';
			}
		} else {
...
function getallownesttemplate($uid){
...
function getnesttpldir($targettplname) {
	global $_G;
	$tpldir = $pre = '';
...
function save_nest_data($tpldirectory, $primaltplname, $targettplname, $data, $database = false, $optype = '') {
	global $_G;
	...
	$content = preg_replace("/\<script src\=\"misc\.php\?mod\=nesthelp\&action\=get.+?\>\<\/script\>/", '', $content);
	foreach ($data['layoutdata'] as $key => $value) {
		...
		$html .= '</div>';
		$content = preg_replace("/(\<\!\-\-\[nest\=$key\]\-\-\>).+?(\<\!\-\-\[\/nest\]\-\-\>)/is", "\\1".$html."\\2", $content);
	}
	$data['spacecss'] = str_replace('.content', '.dxb_bc', $data['spacecss']);
	$data['spacecss'] = trimdxtpllang($data['spacecss']);
	$content = preg_replace("/(\<style id\=\"nest_style\" type\=\"text\/css\"\>).*?(\<\/style\>)/is", "\\1".$data['spacecss']."\\2", $content);
	if (!empty($data['style'])) {
	...
	if($flag) {
		...
		@unlink('./data/nest/'.$tpldirectory.'/'.$targettplname.'_nest_preview'.$tpl_suffix);
	}

	$tplfile =DISCUZ_ROOT.'./data/nest/'.$tpldirectory.'/'.$targettplname.$tpl_suffix;
	...
	if ($r && $database && !$flag) {
		$nesttplname = getnesttplname($targettplname, $tpldirectory);
		C::t('common_nest_data')->insert(array(
			'targettplname' => $targettplname,
			'tpldirectory' => $tpldirectory,
			'primaltplname' => $primaltplname,
			'nestcontent' => serialize($data),
			'name' => $nesttplname,
			'uid' => $_G['uid'],
...
function getnesttplname($targettplname, $tpldirectory) {
	$nestdata = C::t('common_nest_data')->fetch($targettplname, $tpldirectory);
	$nesttplname = $nestdata ? $nestdata['name'] : '';
	if(empty($nesttplname) && ($data = getnesttplnames(array($targettplname)))) {
		$nesttplname = array_shift(array_shift($data));
	}
	return $nesttplname;
}
...
function import_nest($file) {
	...
	if ($nestcontent) {
		...
		$mapping = [];
		if (!empty($nestcontent['blockdata'])) {
			$mapping = block_import($nestcontent['blockdata']);
			unset($nestcontent['blockdata']);
		}
		...
		require_once libfile('class/xml');
		$xml = array2xml($nestcontent['layoutdata'],true);
		$xml = str_replace($oldbids, $newbids, $xml);
		$xml = str_replace((array)array_keys($_G['curtplframe']), $newframe, $xml);
		$nestcontent['layoutdata'] = xml2array($xml);

		$css = str_replace($oldbids, $newbids, $nestcontent['spacecss']);
...
function checkprimaltpl($template) {
	...
	if(!$template || preg_match("/(\.)(exe|jsp|asp|aspx|cgi|fcgi|pl)(\.|$)/i", $template)) {
		return 'nest_template_filename_invalid';
	}
	...
	$primaltplname = (!$tpldirectory ? DISCUZ_ROOT.$_G['cache']['style_default']['tpldir'] : $tpldirectory).'/'.$template.$tpl_suffix;
	if (!file_exists($primaltplname)) {
		$primaltplname = DISCUZ_ROOT.'./template/default/'.$template.$tpl_suffix;
	}
...
function updatetopic($topic = ''){
	...
	if($topicid) {
		...
		C::t('common_nest_data')->update('portal/portal_topic_content_'.$topicid, getnestdirectory($topic['primaltplname']), array('name'=>$setarr['title']));
	} else {
		$setarr['uid'] = $_G['uid'];
	...
	if($primaltplname && $topic['primaltplname'] != $primaltplname) {
		...
		C::t('common_nest_data')->update($targettplname, getnestdirectory($topic['primaltplname']), array('primaltplname'=>$primaltplname, 'tpldirectory'=>$tpldirectory));
		updatenesttemplate($targettplname);
	}

	if($primaltplname && empty($topic['primaltplname'])) {
		$tpldirectory = ($tpldirectory ? $tpldirectory : $_G['cache']['style_default']['tpldir']);
		$content = file_get_contents(DISCUZ_ROOT.$tpldirectory.'/'.$primaltplname.$tpl_suffix);
		$tplfile = DISCUZ_ROOT.'./data/nest/'.$tpldirectory.'/portal/portal_topic_content_'.$topicid.$tpl_suffix;
		$tplpath = dirname($tplfile);
		if (!is_dir($tplpath)) {
			dmkdir($tplpath);
		}
	...
	include_once libfile('function/cache');
	updatecache(array('nesttemplatename', 'setting'));
...
function addtopic($topic) {
	...
	if($topic && is_array($topic)) {
		...
		if(!empty($topicid)) {
			...
			C::t('common_nest_data')->insert($nestdata);
		}
	}
	return $topicid;
}
...
function getnestdirectory($value) {
	$directory = '';
