...
function getuserprofile($field) {
	...
	static $tablefields = [
        'count' => [
            ...
            'friends', 'posts', 'threads', 'digestposts', 'doings', 'blogs', 'albums', 'sharings',
            'attachsize', 'views', 'oltime', 'todayattachs', 'todayattachsize', 'follower', 'following', 'newfollower', 'blacklist'
        ],
        'status' => [
            'regip', 'lastip', 'lastvisit', 'lastactivity', 'lastpost', 'lastsendmail', 'invisible',
            'buyercredit', 'sellercredit', 'favtimes', 'sharetimes', 'profileprogress'
        ],
        'field_forum' => [
            'publishfeed', 'customshow', 'customstatus', 'medals', 'sightml', 'groupterms',
            'authstr', 'groups', 'attentiongroup'
        ],
        'field_home' => [
            'spacename', 'spacedescription', 'domain', 'addsize', 'addfriend', 'menunum', 'theme',
            'spacecss', 'blockposition', 'recentnote', 'spacenote', 'privacy', 'feedfriend', 'acceptemail', 'magicgift', 'stickblogs'
        ],
        'profile' => [
            'realname', 'gender', 'birthyear', 'birthmonth', 'birthday', 'constellation', 'zodiac', 'telephone', 'mobile', 'idcardtype', 'idcard', 'address', 'zipcode',
            'nationality', 'birthprovince', 'birthcity', 'resideprovince', 'residecity', 'residedist', 'residecommunity', 'residesuite', 'graduateschool', 'company', 'education',
            'occupation', 'position', 'revenue', 'affectivestatus', 'lookingfor', 'bloodtype', 'height', 'weight', 'alipay', 'icq', 'qq', 'yahoo', 'msn', 'taobao', 'site', 'bio',
            'interest', 'field1', 'field2', 'field3', 'field4', 'field5', 'field6', 'field7', 'field8'
        ],
        'verify' => [
            'verify1', 'verify2', 'verify3', 'verify4', 'verify5', 'verify6', 'verify7', 'verify8', 'verify9',
            'verify10', 'verify11', 'verify12', 'verify13', 'verify14', 'verify15'
        ],
    ];
    $profiletable = '';
...
function avatar($uid, $size = 'middle', $returnsrc = FALSE, $real = FALSE, $static = FALSE, $ucenterurl = '') {
	...
	if(!$staticavatar && !$static) {
		...
		return $returnsrc ? $file : '<img class="avatar-main" src="'.$file.'" onerror="this.onerror=null;this.src=\''.$ucenterurl.'/images/noavatar_'.$size.'.gif\'" />';
	}
}
...
function template($file, $templateid = 0, $tpldir = '', $gettplfile = 0, $primaltpl='') {
	...
	if(strpos($file, ':') !== false) {
		...
		if($templateid == 'nest') {
			...
			$_G['style']['prefile'] = '';
			$nestpath = DISCUZ_ROOT.'./data/nest/'.$_G['style']['tpldirectory'].'/'; //NEST模板文件目录
			$preend = '_nest_preview';
			$_GET['preview'] = !empty($_GET['preview']) ? $_GET['preview'] : '';
			$curtplname = $oldfile;
			$basescript = $_G['mod'] == 'viewthread' && !empty($_G['thread']) ? 'forum' : $_G['basescript'];
			if(isset($_G['cache']['nesttemplatename'.$basescript])) {
				$nesttemplatename = &$_G['cache']['nesttemplatename'.$basescript];
			} else {
				if(!isset($_G['cache']['nesttemplatename'])) {
					loadcache('nesttemplatename');
				}
				$nesttemplatename = &$_G['cache']['nesttemplatename'];
			}
			$tplsavemod = 0;
			if(isset($nesttemplatename[$file]) && file_exists($nestpath.$file.$tpl_suffix) && ($tplsavemod = 1) || empty($_G['forum']['styleid']) && ($file = $primaltpl ? $primaltpl : $oldfile) && isset($nesttemplatename[$file]) && file_exists($nestpath.$file.$tpl_suffix)) {
				...
				if(isset($_GET['nest']) && $_GET['nest'] == 'yes' || isset($_GET['nest']) && $_GET['preview'] == 'yes') { //NEST模式或预览模式下做以下判断
					$flag = file_exists($nestpath.$file.$preend.$tpl_suffix);
					if($_GET['preview'] == 'yes') {
						$file .= $flag ? $preend : '';
					} else {
			...
			if($innest && ($tplrefresh ==1 || ($tplrefresh > 1 && !($_G['timestamp'] % $tplrefresh))) && filemtime($nestpath.$file.$tpl_suffix) < filemtime(DISCUZ_ROOT.$_G['style']['tpldirectory'].'/'.($primaltpl ? $primaltpl : $oldfile).$tpl_suffix)) {
				if (!updatenesttemplate($file, $_G['style']['tpldirectory'])) {
					unlink($nestpath.$file.$tpl_suffix);
					$tpldir = '';
				}
			}
	...
	if(defined('IN_MOBILE') && !defined('TPL_DEFAULT')) {
		...
		if(strpos($tpldir, 'plugin') && (file_exists(DISCUZ_ROOT.$mobiletplfile) || file_exists(substr(DISCUZ_ROOT.$mobiletplfile, 0, -4).'.php'))) {
			...
			$mobiletplfile = './template/default/'.$file.$tpl_suffix;
	...
	if($templateid != 1 && !file_exists(DISCUZ_ROOT.$tplfile) && !file_exists(substr(DISCUZ_ROOT.$tplfile, 0, -4).'.php')
			&& !file_exists(DISCUZ_ROOT.($tplfile = $tpldir.$filebak.$tpl_suffix))) {
		$tplfile = './template/default/'.$filebak.$tpl_suffix;
	}
...
function dump($arg){
    echo '
    <div class="window moveable debug-win">
        <div class="header">
            <span class="title">Debug</span>
            <a class="close"><span></span><span></span></a>
            <a class="max"><span></span></a>
            <a class="min"><span></span></a>     
        </div>
        <div class="body">
            <pre class="debug">'.var_export($arg, true).'</pre>
        </div>
...
function multi($num, $perpage, $curpage, $mpurl, $maxpages = 0, $page = 7, $autogoto = FALSE, $simple = FALSE, $jsfunc = FALSE) {
				...
				if($tablename == 'field_home') {
					...
					foreach (array('feed','view','profile') as $pkey) {
						if(empty($_G[$var]['privacy'][$pkey]) && !isset($_G[$var]['privacy'][$pkey])) {
							$_G[$var]['privacy'][$pkey] = isset($_G['setting']['privacy'][$pkey]) ? $_G['setting']['privacy'][$pkey] : [];
						}
					}
...
function dmkdir($dir, $mode = 0777, $makeindex = TRUE){
	...
	if(!is_dir($dir)) {
		...
		@mkdir($dir, $mode);
		if(!empty($makeindex)) {
			@touch($dir.'/index'.$tpl_suffix);
			@chmod($dir.'/index'.$tpl_suffix, 0777);
		}
	}
	return true;
}
...
function updatenesttemplate($targettplname = '', $tpldirectory = '') {
	$r = false;
	$alldata = !empty($targettplname) ? array( C::t('common_nest_data')->fetch($targettplname, $tpldirectory)) : C::t('common_nest_data')->range();
	require_once libfile('function/portalcp');
	foreach($alldata as $value) {
		$r = save_nest_data($value['tpldirectory'], $value['primaltplname'], $value['targettplname'], dunserialize($value['nestcontent']));
	}
	return $r;
}
...
function getexpiration() {
	...
	return mktime(0, 0, 0, $date['mon'], $date['mday'], $date['year']) + 24*3600*7;
}
...
function check_nest_perm($topic = [], $flag = '') {
	static $ret;
	if(!isset($ret)) {
		global $_G;
		...
		$ret['layout'] = $common && ($_G['group']['allownest'] || (
