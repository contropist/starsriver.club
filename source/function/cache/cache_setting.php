...
function build_cache_setting() {
	...
	$output = array('str'=>[], 'preg' => []); //str为二级域名的查找和替换，preg为rewrite和默认域名的查找和替换
	$_G['domain'] = [];
	...
	$data['output'] = $output;
	$data['connect'] = in_array('qqconnect', $data['plugins']['available']) ? $data['connect'] : [];
...
function get_cachedata_setting_plugin($method = '') {
	...
	$data['plugins']['func'] = $data['plugins']['available'] = [];
...
function get_cachedata_spacenavs() {
	...
	foreach(C::t('common_nav')->fetch_all_by_navtype(2) as $nav) {
		...
		$nav['subcode'] = $nav['allowsubnew'] ? '<span><a href="'.$nav['suburl'].'"'.($nav['target'] == 1 ? ' target="_blank"' : '').$nav['extra'].'>'.htmlspecialchars_decode($nav['subname']).'</a></span>' : '';

		if($nav['icon']) {
            if(strpos($nav['icon'],'<ifont>') === false){
                ...
                $nav['icon'] = '<i class="icon"><img src="'.$navicon.'"/></i>';
            } else {
                $nav['icon'] = '<i class="icon '.str_replace(['<ifont>','</ifont>'], '', $nav['icon']).'"></i>';
            }
        }
		if($nav['name'] != '{hr}') {
