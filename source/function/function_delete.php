...
function deletemember($uids, $delpost = true) {
    ...
    foreach (array('common_member_field_forum', 'common_member_field_home', 'common_member_count', 'common_member_profile', 'common_member_status',) as $table) {
    ...
    foreach (array('home_doing', 'home_share', 'home_album', 'common_credit_rule_log', 'common_credit_rule_log_field', 'home_pic', 'home_blog', 'home_blogfield', 'home_class', 'home_clickuser', 'home_show', 'forum_collectioncomment', 'forum_collectionfollow', 'forum_collectionteamworker') as $table) {
...
function deletepost($ids, $idtype = 'pid', $credit = false, $posttableid = false, $recycle = false) {
    ...
    if ($credit) {
        $tuidarray = $ruidarray = $_G['deleteauthorids'] = [];
...
function deletethread($tids, $membercount = false, $credit = false, $ponly = false) {
    ...
    foreach (array('forum_forumrecommend', 'forum_polloption', 'forum_poll', 'forum_polloption_image', 'forum_activity', 'forum_activityapply', 'forum_debate', 'forum_debatepost', 'forum_threadmod', 'forum_relatedthread', 'forum_pollvoter', 'forum_threadimage', 'forum_threadpreview') as $table) {
...
function deletetrasharticle($aids) {
    ...
    foreach (C::t('portal_article_trash')->fetch_all($aids) as $value) {
        ...
        if (!empty($article['idtype']))
            $pushs[$article['idtype']][] = $article['id'];
        if ($article['pic']) {
    ...
    if (!empty($pushs) && is_array($pushs)) {
        foreach ($pushs as $idtype => $fromids) {
            switch ($idtype) {
                case 'blogid':
                    if (!empty($fromids))
                        C::t('home_blogfield')->update($fromids, array('pushedaid' => '0'));
                    break;
                case 'tid':
                    if (!empty($fromids))
                        C::t('forum_thread')->update($fromids, array('pushedaid' => '0'));
                    break;
            }
        }
    }
...
function deleteportaltopic($dels) {
    ...
    C::t('common_nest_data')->delete($targettplname, null);
...
function deletecollection($ctid) {
    global $_G;
    $collection = C::t('forum_collection')->fetch_all($ctid);
    ...
    if (!empty($collection)) {
        ...
        foreach ($collectionteamworker as $worker) {
            notification_add($worker['uid'], "system", 'collection_removed', ['ctid' => $collectiondata['ctid'], 'collectionname' => $collectiondata['name']], 1);
        }

        if (!empty($collection[$ctid]['cover'])) {
            @unlink($_G['setting']['attachurl'] . 'collection/' . $collection[$ctid]['cover']);
        }
