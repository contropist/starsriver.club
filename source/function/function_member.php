...
function checkemail($email) {
	...
	if(strlen($email) > 32) {
		showmessage('profile_email_illegal', '', [], array('handle' => false));
	}
	if($_G['setting']['regmaildomain']) {
		...
		if($_G['setting']['regmaildomain'] == 1 && !preg_match($maildomainexp, $email)) {
			showmessage('profile_email_domain_illegal', '', [], array('handle' => false));
	...
	if($ucresult == -4) {
		...
		showmessage('profile_email_duplicate', '', [], array('handle' => false));
	}
}
