...
function cplang($name, $replace = [], $output = false) {
	global $_G;
	$ret = '';
...
function showheader($key, $url) {
	...
	echo '<li><a href="'.ADMINSCRIPT.'?action='.$url.'" id="header_'.$key.'" hidefocus="true" onclick="toggleMenu(\''.$key.'\', \''.$url.'\');doane(event);">'.$menuname.'</a></li>';
}
...
function cpmsg($message, $url = '', $type = '', $values = [], $extra = '', $halt = TRUE, $cancelurl = '') {
	global $_G;
...
function showsubmenu($title, $menus = [], $right = '', $replace = []) {
	if(empty($menus)) {
		$s = $right.'<span class="title">'.cplang($title, $replace).'</span>';
	} elseif(is_array($menus)) {
		$s = $right.'<span class="title">'.cplang($title, $replace).'</span><ul class="child-nav">';
		foreach($menus as $k => $menu) {
			if(is_array($menu[0])) {
				$s .= '<li class="'.($menu[1] ? 'active' : 'hasdropmenu').'" id="addjs'.$k.'" onmouseover="dropmenu(this);"><a href="#">'.cplang($menu[0]['menu']).'</a><div class="nav-menu" id="addjs'.$k.'child" style="display:none;">';
				if(is_array($menu[0]['submenu'])) {
					foreach($menu[0]['submenu'] as $submenu) {
						$s .= $submenu[1] ? '<a class="'.($submenu[2] ? 'active' : '').'" href="'.ADMINSCRIPT.'?action='.$submenu[1].'" onclick="'.$submenu[3].'">'.cplang($submenu[0]).'</a>' : '<a>'.cplang($submenu[0]).'</a>';
					}
				}
				$s .= '</div></li>';
			} else {
				$s .= '<li'.($menu[2] ? ' class="active"' : '').'><a href="'.(!$menu[4] ? ADMINSCRIPT.'?action='.$menu[1] : $menu[1]).'"'.(!empty($menu[3]) ? ' target="_blank"' : '').'>'.cplang($menu[0]).'</a></li>';
			}
		}
		$s .= '</ul>';
	}
	echo !empty($menus) ? '<div class="floattop">'.$s.'</div><div class="empty"></div>' : $s;
}

function showsubmenusteps($title, $menus = [], $mleft = [], $mright = []) {
	$s = $title ? '<span class="title">'.cplang($title).'</span>' : '';
	if(is_array($mleft)) {
		$s .= '<ul class="child-nav">';
...
EOT;
	...
	foreach($menus as $k => $menu) {
		if($menu && is_array($menu)) {
			if(is_array($menu[0])) {
				$s .= '<li id="nav_m'.$k.'" class="hasdropmenu" onmouseover="dropmenu(this);"><a href="#">'.cplang($menu[0]['menu']).'</a><div id="nav_m'.$k.'child" class="nav-menu" style="display:none;"><ul>';
				if(is_array($menu[0]['submenu'])) {
					foreach($menu[0]['submenu'] as $submenu) {
						$s .= '<li '.($submenu[2] ? ' class="active"' : '').(!$submenu[3] ? ' id="nav_'.$submenu[1].'" onclick="showanchor(this)"' : '').'><a href="'.($submenu[3] ? ADMINSCRIPT.'?action='.$submenu[1] : '#').'">'.cplang($submenu[0]).'</a></li>';
					}
				}
				$s .= '</ul></div></li>';
			} else {
				$s .= '<li'.($menu[2] ? ' class="active"' : '').(!$menu[3] ? ' id="nav_'.$menu[1].'" onclick="showanchor(this)"' : '').'><a href="'.($menu[3] ? ADMINSCRIPT.'?action='.$menu[1] : '#').'">'.cplang($menu[0]).'</a></li>';
			}
		}
	}
	$s .= '</ul>';
...
function showformheader($action, $extra = '', $name = 'cpform', $method = 'post') {
	echo make_formheader($action, $extra, $name, $method);
}
...
function showtableheader($title = '', $classname = '', $extra = '', $titlespan = 15) {
	...
	echo '<table class="tb tb2 '.$classname.'"'.($extra ? " $extra" : '').'>';
	if($title) {
...
function showsetting($setname, $varname, $value, $type = 'radio', $disabled = '', $hidden = 0, $comment = '', $extra = '', $setid = '', $nofaq = false) {
	...
	if($type == 'radio') {
		...
		$s .= '<input id="'.$id.'_0" style="display:'.($defaulttype ? 'none' : '').'" name="'.($defaulttype ? 'TMP' : '').$varname.'" value="" type="file" class="txt uploadbtn marginbot" '.$check['disabled'].' '.$extra.' />'.
			...
			'<a id="'.$id.'_0a" style="'.(!$defaulttype ? 'font-weight:bold' : '').'" onclick="$(\''.$id.'_1a\').style.fontWeight = \'\';this.style.fontWeight = \'bold\';$(\''.$id.'_1\').name = \'TMP'.$varname.'\';$(\''.$id.'_0\').name = \''.$varname.'\';$(\''.$id.'_0\').style.display = \'\';$(\''.$id.'_1\').style.display = \'none\'">'.cplang('switch_upload').'</a>&nbsp;'.
			'<a id="'.$id.'_1a" style="'.($defaulttype ? 'font-weight:bold' : '').'" onclick="$(\''.$id.'_0a\').style.fontWeight = \'\';this.style.fontWeight = \'bold\';$(\''.$id.'_0\').name = \'TMP'.$varname.'\';$(\''.$id.'_1\').name = \''.$varname.'\';$(\''.$id.'_1\').style.display = \'\';$(\''.$id.'_0\').style.display = \'none\'">'.cplang('switch_url').'</a>';
		...
		$s .= "<input id=\"c{$colorid}_v\" type=\"text\" style=\"float:left; width:210px;\" value=\"$value\" name=\"$varname\" onchange=\"updatecolorpreview('c{$colorid}')\">\n".
			"<input id=\"c$colorid\" onclick=\"c{$colorid}_frame.location='static/image/admincp/getcolor".$_G['config']['output']['tpl_suffix']."?c{$colorid}|c{$colorid}_v';showMenu({'ctrlid':'c$colorid'})\" type=\"button\" class=\"colorwd\" value=\"\" style=\"background: $background\"><span id=\"c{$colorid}_menu\" style=\"display: none\"><iframe name=\"c{$colorid}_frame\" src=\"\" frameborder=\"0\" width=\"210\" height=\"148\" scrolling=\"no\"></iframe></span>\n$extra";
	} elseif($type == 'calendar') {
		$s .= "<input type=\"text\" name=\"$varname\" value=\"".dhtmlspecialchars($value)."\" onclick=\"showcalendar(event, this".($extra ? ', 1' : '').")\">\n";
		...
		$s .= "<input type=\"text\" name=\"$varname[0]\" value=\"".dhtmlspecialchars($value[0])."\" style=\"width: 108px; margin-right: 5px;\"$onclick>".($type == 'multiply' ? ' X ' : ' -- ')."<input type=\"text\" name=\"$varname[1]\" value=\"".dhtmlspecialchars($value[1])."\"style=\"width: 108px; margin-left: 5px;\"$onclick>";
	} else {
		$s .= $type;
	}
	$name = cplang($setname);
...
function showsubmit($name = '', $value = 'submit', $before = '', $after = '', $floatright = '', $entersubmit = true) {
    ...
    echo make_submit($name, $value, $before, $after, $floatright, $entersubmit);
}
...
function get_pluginsetting($type) {
	$pluginsetting = $pluginvalue = [];
	@include_once DISCUZ_ROOT.'./data/sysdata/cache_pluginsetting.php';
	$pluginsetting = isset($pluginsetting[$type]) ? $pluginsetting[$type] : [];
...
function getposttableselect() {
	...
	if(!empty($_G['cache']['posttable_info']) && is_array($_G['cache']['posttable_info'])) {
		$posttableselect = '<select name="posttableid" id="posttableid">';
...
function make_app($data ,$class = '', $id = ''){
    return '<div class="app '.$class.'" id="'.$id.'" >'.$data.'</div>';
}

function make_vessel($data = []){
    ...
    if(!empty($tpl['header'])){
        $header = '<div class="header"><span class="title">'.$data['title'].'</span>'.$tpl['header'].'</div>';
    } else {
        $header = '<div class="header"><span class="title">'.$data['title'].'</span></div>';
    }

    $body = '<div class="body">'.($tpl['body'] ?? '').'</div>';

    if(!empty($tpl['footer'])){
        $footer = '<div class="footer">'.$tpl['footer'].'</div>';
    }

    if(!empty($tpl['script'])){
        $script = '<div class="script">'.$tpl['script'].'</div>';
    }

    return '
        <div class="vessel" id="'.$data['id'].'" '.($tagdata ?? '').'>
            '.($header ?? '').'
            '.($body ?? '').'
            '.($footer ?? '').'
            '.($script ?? '').'
...
function make_formheader($action, $extra = '', $name = 'cpform', $method = 'post') {
    global $_G;
    $anchor = isset($_GET['anchor']) ? dhtmlspecialchars($_GET['anchor']) : '';
    return '<form name="'.$name.'" method="'.$method.'" autocomplete="off" action="'.ADMINSCRIPT.'?action='.$action.'" id="'.$name.'"'.($extra == 'enctype' ? ' enctype="multipart/form-data"' : " $extra").'>'.
        '<input type="hidden" name="formhash" value="'.FORMHASH.'" />'.
        '<input type="hidden" id="formscrolltop" name="scrolltop" value="" />'.
        '<input type="hidden" name="anchor" value="'.$anchor.'" />';
}

function make_submit($name = '', $value = 'submit', $before = '', $after = '', $floatright = '', $entersubmit = true) {
    global $_G;
    if(!empty($_G['showsetting_multi'])) {
        return '';
    }
    $str = '';
    $str .= $name && in_array($before, array('del', 'select_all', 'td')) ? ($before != 'td' ? '<div class="ccbox admin"><input type="checkbox" name="chkall" id="chkall'.($chkkallid = random(4)).'" class="checkbox" onclick="checkAll(\'prefix\', this.form, \'delete\')" /><label for="chkall'.$chkkallid.'">'.lang('home/template','select_all').cplang($before) : '').'</label></div>' : '';
    $str .= $floatright ? '<div class="right">'.$floatright.'</div>' : '';
