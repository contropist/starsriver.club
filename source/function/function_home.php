...
function getusernestdata($space) {
	global $_G;
	if(empty($_G['blockposition'])) {
		$usernest = getuserdefaultnest();
		if (!empty($space['blockposition'])) {
			$blockdata = dunserialize($space['blockposition']);
			foreach ((array)$blockdata as $key => $value) {
				if ($key == 'parameters') {
					foreach ((array)$value as $k=>$v) {
						if (!empty($v)) $usernest[$key][$k] = $v;
					}
				} else {
					if (!empty($value)) $usernest[$key] = $value;
				}
			}
		}
		$_G['blockposition'] = $usernest;
	}
	return $_G['blockposition'];
}


function getuserdefaultnest() {
	$defaultnest = array(
			'currentlayout' => '1:2:1',
			'block' => array(
					'frame`frame1' => array(
