 ...
 ********************************************************************/
    ...
    if ($_G['setting']['commentnumber'] && !empty($_GET['comment'])) {
        ...
        $pcid = C::t('forum_postcomment')->insert([
            'tid'      => $post['tid'],
            'pid'      => $post['pid'],
            'author'   => $_G['username'],
            'authorid' => $_G['uid'],
        ...
        C::t('forum_post')->update('tid:' . $_G['tid'], $_GET['pid'], ['comment' => 1]);
        
        $comments = $thread['comments'] ? $thread['comments'] + 1 : C::t('forum_postcomment')->count_by_tid($_G['tid']);
        C::t('forum_thread')->update($_G['tid'], ['comments' => $comments]);
        ...
        if (!empty($_G['uid']) && $_G['uid'] != $post['authorid']) {
            notification_add($post['authorid'], 'pcomment', 'comment_add', [
                ...
                'commentmsg'  => cutstr(str_replace([
                    '[b]',
                    '[/b]',
                    '[/color]',
                ], '', preg_replace("/\[color=([#\w]+?)\]/i", "", $comment)), 200),
            ]);
        }
        ...
        if (!empty($_G['uid']) && $_GET['commentitem']) {
            ...
            if ($pcid) {
                C::t('forum_postcomment')->update($pcid, [
                    'comment'  => $totalv,
                    'dateline' => TIMESTAMP + 1,
                ]);
            } else {
                C::t('forum_postcomment')->insert([
                    'tid'      => $post['tid'],
                    'pid'      => $post['pid'],
                    'author'   => '',
                    'authorid' => '-1',
        ...
        showmessage('comment_add_succeed', "forum.php?mod=viewthread&tid=$post[tid]&pid=$post[pid]&page=$_GET[page]&extra=$extra#pid$post[pid]", [
            'tid' => $post['tid'],
            'pid' => $post['pid'],
        ]);
    }
    ...
    if (getstatus($thread['status'], 3)) {
        ...
        if ($rushinfo['creditlimit'] != -996) {
            ...
            if ($checkcreditsvalue < $rushinfo['creditlimit']) {
                ...
                showmessage('post_rushreply_creditlimit', '', [
                    'creditlimit_title' => $creditlimit_title,
                    'creditlimit'       => $rushinfo['creditlimit'],
                ]);
            }
        }
    ...
    if (!submitcheck('replysubmit', 0, $seccodecheck, $secqaacheck)) {
        ...
        if (isset($_GET['repquote']) && $_GET['repquote'] = intval($_GET['repquote'])) {
            ...
            if (!($thread['price'] && !$thread['special'] && $thaquote['first'])) {
                ...
                $message = messagecutstr($message, 512);
                $message = implode("\n", array_slice(explode("\n", $message), 0, 3));
                ...
                $post_reply_quote = lang('forum/misc', 'post_reply_quote', [
                    'author' => $thaquote['author'],
                    'time'   => $time,
                ]);
                $noticeauthormsg = dhtmlspecialchars($message);
                if (!defined('IN_MOBILE')) {
                    $message = "[quote][url=forum.php?mod=redirect&goto=findpost&pid=$_GET[repquote]&ptid={$_G['tid']}][color=#00C2AE]{$post_reply_quote}：[/color][/url][color=#888888]{$message}[/color][/quote]";
                } else {
                    $message = "[quote][color=#00C2AE]{$post_reply_quote}：[/color][color=#888888]{$message}[/color][/quote]";
                }
            ...
            $quotemessage = discuzcode($message, 0, 0);
            $noticeauthormsg = dhtmlspecialchars(messagecutstr($thapost['message'], 512));
        ...
        if ($_GET['trade'] && $thread['special'] == 2 && $_G['group']['allowposttrade']) {
            $bfmethods[] = [
                'class'  => 'extend_thread_trade',
                'method' => 'before_newreply',
            ];
        }
        ...
        $bfmethods[] = [
            'class'  => 'extend_thread_rushreply',
            'method' => 'before_newreply',
        ];
        if ($_G['group']['allowat']) {
            $bfmethods[] = [
                'class'  => 'extend_thread_allowat',
                'method' => 'before_newreply',
            ];
        }
        
        $bfmethods[] = [
            'class'  => 'extend_thread_comment',
            'method' => 'before_newreply',
        ];
        
        $modpost->attach_before_method('newreply', [
            'class'  => 'extend_thread_filter',
            'method' => 'before_newreply',
        ]);
        ...
        if ($thread['replycredit'] > 0 && $thread['authorid'] != $_G['uid'] && $_G['uid']) {
            $afmethods[] = [
                'class'  => 'extend_thread_replycredit',
                'method' => 'after_newreply',
            ];
        }
        ...
        if ($special == 5) {
            $afmethods[] = [
                'class'  => 'extend_thread_debate',
                'method' => 'after_newreply',
            ];
        }
        
        $afmethods[] = [
            'class'  => 'extend_thread_image',
            'method' => 'after_newreply',
        ];
        ...
        if ($_G['forum']['allowfeed']) {
            if ($special == 2 && !empty($_GET['trade'])) {
                ...
                // Used for generate feed after add new Goods
                
                $modpost->attach_before_method('replyfeed', [
                    'class'  => 'extend_thread_trade',
                    'method' => 'before_replyfeed',
                ]);
                $modpost->attach_after_method('replyfeed', [
                    'class'  => 'extend_thread_trade',
                    'method' => 'after_replyfeed',
                ]);
            } elseif ($special == 3 && $thread['authorid'] != $_G['uid']) {
                $modpost->attach_before_method('replyfeed', [
                    'class'  => 'extend_thread_reward',
                    'method' => 'before_replyfeed',
                ]);
