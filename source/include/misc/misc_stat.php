...
if ($beginunixstr > $endunixstr) {
	showmessage('start_time_is_greater_than_end_time', NULL, [], array('return' => true));
} else if ($beginunixstr == $endunixstr) {
	showmessage('start_time_end_time_is_equal_to', NULL, [], array('return' => true));
}
if (!empty($_GET['xml'])) {
	$xaxis = '';
	$graph = [];
