...
$theclass['script'] = isset($theclass['script']) ? $theclass['script'] : [];
...
if($op == 'block') {
	...
	if(submitcheck('blocksubmit')) {
		...
		if($summary) {
			...
			if($tag != $summary) {
				$msg = lang('portalcp', 'block_nest_summary_html_tag').$tag.lang('portalcp', 'block_nest_summary_not_closed');
				showmessage($msg);
			}
		}
	...
	foreach(array('title_b', 'title_i', 'title_u', 'title_c', 'summary_b', 'summary_i', 'summary_u', 'title_c') as $value) {
		if(!empty($item['showstyle'][$value])) {
			$showstylearr[$value] = 'class="active"';
		}
	}
	...
	if(submitcheck('itemsubmit') || submitcheck('recommendsubmit') || submitcheck('verifydatasubmit') || submitcheck('managedatasubmit')) {
		...
		if(submitcheck('itemsubmit')) {
			...
			if($op == 'additem' && !$item['id']) {
				$item['id'] = ($pushid = intval($_POST['push_id'])) ? $pushid : rand(1,9999);
				$item['idtype'] = 'rand';
			}
			...
			if($showrecommendrate) {
				...
				showmessage('do_success', dreferer('portal.php'), [], array('showdialog' => true, 'closetime' => true));
			}
	...
	if(strexists($block['summary'], '<script')) {
		$block['summary'] = lang('portalcp', 'block_nest_nopreview');
		$_G['block'][$bid] = $block;
