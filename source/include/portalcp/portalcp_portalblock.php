...
$allownest = checkperm('allownest');
if(!$allownest && !$admincp4 && !$admincp5 && !$admincp6) {
...
if(submitcheck('getblocklistsubmit') || submitcheck('verifieddatasubmit') || submitcheck('verifydatasubmit')) {
	...
	if($allownest) {
		$tpls = array_keys($_G['cache']['nesttemplatename']);
	} else {
		$permissions = getallownesttemplate($_G['uid']);
	...
	if(submitcheck('getblocklistsubmit')) {
		...
		$updatebids = !$allownest ? array_intersect($bids, $updatebids) : $updatebids;
		if($updatebids) {
		...
		if($_POST['ids']) {
			foreach(C::t('common_block_item_data')->fetch_all($_POST['ids']) as $value) {
				if($allownest || in_array($value['bid'], $bids)) {
	...
	if($allownest) {
		$tpls = $_G['cache']['nesttemplatename'];
	} else {
		$tplpermissions = getallownesttemplate($_G['uid']);
		foreach($tplpermissions as $value) {
			if($value['allowmanage'] || ($value['allowrecommend'] && empty($value['needverify'])) || ($op=='recommend' && $value['allowrecommend'])) {
				$tpls[$value['targettplname']] = isset($_G['cache']['nesttemplatename'][$value['targettplname']]) ? $_G['cache']['nesttemplatename'][$value['targettplname']] : $value['targettplname'];
			}
		}
	...
	if($blocks) {
		...
		if($bids) {
			foreach(C::t('common_template_block')->fetch_all_by_bid($bids) as $value) {
				$alldata[] = $value;
				if(!isset($_G['cache']['nesttemplatename'][$value['targettplname']])) {
					$losttpls[$value['targettplname']] = $value['targettplname'];
				}
			}

			if($losttpls) {
				$lostnames = getnesttplnames($losttpls);
				foreach($lostnames as $pre => $datas) {
					foreach($datas as $id => $name) {
						$_G['cache']['nesttemplatename'][$pre.$id] = $tpls[$pre.$id] = $name;
					}
				}
			}
			...
			foreach($alldata as $value) {
				...
				$blocks[$value['bid']]['page'][$value['targettplname']] = $nesturl ? '<a href="'.$nesturl.'" target="_blank">'.$tplname.'</a>' : $tplname;
			}
		}
		if($initemdata) {
			$isverified = $op === 'verifieddata' ? 1 : 0;
			$count = C::t('common_block_item_data')->count_by_bid($bids, $isverified);
			$blockdata = $count ? C::t('common_block_item_data')->fetch_all_by_bid($bids, $isverified, $start, $perpage) : [];
			$multi = multi($count, $perpage, $page, $theurl);
		}
	}
}
...
function formatblockpermissoin($block) {
	static $allownest = null;
	$allownest = isset($allownest) ? $allownest : checkperm('allownest');;
