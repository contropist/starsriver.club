...
if(submitcheck("articlesubmit", 0, $seccodecheck, $secqaacheck)) {
	...
	showmessage('pushplus_do_success', $tourl, [], array('header'=>1, 'refreshtime'=>0));
	...
	if($_GET['from_idtype'] && $_GET['from_id']) {
		...
		if($havepush) {
			if($_GET['from_idtype'] == 'blogid') {
				showmessage('article_push_blogid_invalid_repeat', '', [], array('return'=>true));
			} else {
				showmessage('article_push_tid_invalid_repeat', '', [], array('return'=>true));
			}
		}
