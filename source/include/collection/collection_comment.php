...
if(empty($_G['uid'])) {
	showmessage('login_before_enter_home', null, [], array('showmsg' => true, 'login' => 1));
}
...
if(empty($op) || $op == 'add') {
	...
	if(!$memberrate) {
		$newcomment['rate'] = (int)$_GET['ratescore'] > 5 ? 5 : ((int)$_GET['ratescore'] < 0 ? 0 : (int)$_GET['ratescore']);
	} else {
        $newcomment['rate'] = $memberrate;
	}
	...
	if(!submitcheck('formhash')) {
		...
		if(!$_GET['threadurl']) {
			showmessage('collection_recommend_url', '', [], array('alert'=> 'error', 'closetime' => true, 'showdialog' => 1));
		}
		...
		if($return > 0) {
			...
			showmessage('collection_recommend_succ', '', [], array('alert'=> 'right', 'closetime' => true, 'showdialog' => 1));
		}
	}
}
