...
if(!$op || $op == 'related') {
	...
	if(!$op && $op != 'related') {
		...
		$userCollections = C::t('forum_collection')->fetch_all_by_uid($_G['collection']['uid'], 0, 8, $_G['collection']['ctid']);
	}

	if($_G['collection']['commentnum'] > 0) {
		$commentlist = C::t('forum_collectioncomment')->fetch_all_by_ctid($_G['collection']['ctid'], 0, 4);
	...
	$followers = C::t('forum_collectionfollow')->fetch_all($ctid, true, 0, 12);
