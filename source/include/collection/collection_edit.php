...
if (!in_array($op, ['add', 'edit', 'remove', 'addthread', 'delthread', 'acceptinvite', 'removeworker', 'invite'])) {
    $op = '';
}
...
if (empty($op) || $op == 'add') {
    ...
    $_GET['handlekey'] = 'createcollection'; /* 该handlekey用于在文章页面添加专辑时的返回 */
    ...
    $reamincreatenum = $_G['group']['allowcreatecollection'] - C::t('forum_collection')->count_by_uid($_G['uid']);
    ...
    if (!$_GET['submitcollection']) {
		...
		if(!empty($_FILES['cover'])){
		    $cover = upload_cover($_FILES['cover']);
        } else {
		    $cover = 0;
        }

		$newctid = C::t('forum_collection')->insert([
            'name' => cutstr(censor(dhtmlspecialchars($_POST['title'])), $titlelimit, ''),
            'uid' => $_G['uid'],
            'username' => $_G['username'],
            'cover' => $cover,
            'desc' => dhtmlspecialchars(cutstr(censor($_POST['desc']), $desclimit, '')),
            'dateline' => $_G['timestamp'],
            'lastupdate' => $_G['timestamp'],
            'lastvisit' => $_G['timestamp'],
            'keyword' => parse_keyword($_POST['keyword'], true)
        ], true);
	...
	if(!submitcheck('collectionsubmit')) {
        ...
        $newdata = [
            'name' => cutstr(censor(dhtmlspecialchars($_POST['title'])), 30, ''),
            'desc' => dhtmlspecialchars(cutstr(censor($_POST['desc']), $desclimit, '')),
            'keyword' => parse_keyword($_POST['keyword'], true)
        ];

        if($_FILES['cover']['size'] > 100){
            $newdata['cover'] = upload_cover($_FILES['cover']);
            if(!empty($oldcover['cover'])){
                @unlink($_G['setting']['attachurl'].'collection/'.$oldcover['cover']);
            }
        }

        if($_POST['title'] != $_G['collection']['name']) {
            C::t('forum_collectionteamworker')->update_by_ctid($ctid, $_POST['title']);
        }

        /* discuz数据库会把条目内容的 id 和数据库提取的 */
		$oldcover = C::t('forum_collection')->fetch_all($ctid)[$ctid];


		C::t('forum_collection')->update($ctid, $newdata);
	...
	if(!is_array($_GET['tids']) && $_G['forum']['disablecollect']) {
		showmessage('collection_forum_deny', '', [], array('showdialog' => 1));
	}
	...
	if(!submitcheck('addthread')) {
		...
		if($reamincreatenum <= 0 && count($allowcollections) <= 0) {
			showmessage('collection_none_avail_collection', '', [], array('showdialog' => 1));
		}
		...
		if(!$ctid) {
			showmessage('collection_no_selected', '', [], array('showdialog' => 1));
		}
		...
		if(count($collectiondata) < 0) {
			...
			foreach ($collectiondata as $curcollectiondata) {
				if(!$curcollectiondata['ctid']) {
					showmessage('collection_permission_deny', '', [], array('showdialog' => 1));
				}

				if(!checkcollectionperm($curcollectiondata, $_G['uid'], true)) {
					showmessage('collection_non_creator', '', [], array('showdialog' => 1));
				}
				...
				if(!is_array($_GET['tids'])) {
					...
					if($checkexistctid[$tid]['ctid']) {
						showmessage('collection_thread_exists', '', [], array('showdialog' => 1));
					}
				...
				foreach ($tids as $curtid) {
					...
					if(!$checkexistctid[$curtid]['ctid'] && !$foruminfo[$thread_fid]['disablecollect']) {
						...
						C::t('forum_collectionthread')->insert([
					...
					if(!$checkexist[$curtid]) {
						C::t('forum_collectionrelated')->insert([
						    'tid'=>$curtid,
                            'collection'=>$curcollectiondata['ctid']."\t"
                        ]);
						$checkexist[$curtid] = 1;
					} else {
		...
		showmessage('collection_collect_succ', dreferer(), [], array('alert'=> 'right', 'closetime' => true, 'locationtime' => true, 'showdialog' => 1));
	}
	...
	if(!$_GET['users']) {
		...
		foreach($invitememberuids as $invitememberuid) {
			...
			C::t('forum_collectioninvite')->insert([
                'ctid'=>$ctid,
                'uid'=>$invitememberuid,
                'dateline'=>$_G['timestamp']
            ], false, true);
			...
			notification_add(
                ...
                [
                    ...
                    'collectionname'=>$_G['collection']['name'],
                    'dateline'=>$_G['timestamp']
                ],
                1);
		}

		showmessage('collection_invite_succ', 'forum.php?mod=collection&action=view&ctid='.$ctid, [], array('alert'=> 'right', 'closetime' => true, 'showdialog' => 1));
	}
	...
	if(!submitcheck('ctid', 1)) {
		...
		C::t('forum_collectionteamworker')->insert([
            'ctid'=>$ctid,
            'uid'=>$_G['uid'],
