...
function accessimg($access) {
	return $access == -1 ? '<img class="prem-icon" src="'.IMGURL.'/common/icon/disabled.svg" />' :
		($access == 1 ? '<img class="prem-icon" src="'.IMGURL.'/common/icon/accessed_b.svg" />' : '<img class="prem-icon" src="'.IMGURL.'/common/icon/owned.svg" />');
}
