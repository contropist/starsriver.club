			...
			if(($uids = array_keys($members))) {
				...
				if($sendemail) {
					...
					foreach($members as $uid => $member) {
						$member['regdate'] = date('Y-m-d',$member['regdate']);
...
$moderation = array('validate' => [], 'delete' => [], 'ignore' => []);
			...
			foreach($attachtablearr as $attachtable => $pids) {
				foreach(C::t('forum_attachment_n')->fetch_all_by_id($attachtable, 'pid', $pids) as $attach) {
					...
					$attach['url'] = $attach['isimage']
					    ? "<img src=\"{$_G[setting][attachurl]}forum/$attach[attachment]\" onclick='zoom(this, this.src, 0, 0, 0)'>"
					    : "<a class='file' href=\"".$_G['setting']['attachurl']."forum/$attach[attachment]\" target=\"_blank\">".attachtype(fileext($attach['filename'])."\t").$attach[filename]."</a>";
					$postlist[$attach['pid']]['message'] .= $attach['url'];
				}
			}
		}
	}
	...
	if($modcount) {
		...
		foreach(C::t('common_moderate')->fetch_all_by_search_for_thread($moderatestatus, ($modfids ? explode(',', $modfids) : null), $start_limit, $_G['tpp']) as $thread) {
			...
			if($thread['authorid'] && $thread['author'] != '') {
				$thread['author'] = "<a class='name' href=\"home.php?mod=space&uid=$thread[authorid]\" target=\"_blank\">$thread[author]</a>";
			} elseif($thread['authorid']) {
				$thread['author'] = "<a class='name' href=\"home.php?mod=space&uid=$thread[authorid]\" target=\"_blank\">UID��$thread[authorid]</a>";
			} else {
				$thread['author'] = 'guest';
			}
		...
		foreach($posttablearr as $posttable => $tids) {
			...
			foreach(C::t('forum_post')->fetch_all_by_tid($posttable, $tids, true, '', 0, 0, 1) as $post) {
                ...
                $thread['message'] = discuzcode($thread['message']);
                $thread['message'] = preg_replace("/\[attach\]\d+\[\/attach\]/i", '', $thread['message']);
				...
				if($thread['sortid']) {
					...
					foreach($threadsortshow['optionlist'] as $option) {
						$thread['sortinfo'] .= $option['title'].' '.$option['value'];
					}
				} else {
					$thread['sortinfo'] = '';
				}
				...
				foreach(C::t('forum_attachment_n')->fetch_all_by_id($attachtable, 'tid', $tids) as $attach) {
					...
					$attach['url'] =
                        $attach['isimage']
                            ? "<img src=\"".$_G['setting']['attachurl']."forum/$attach[attachment]\" onclick=\"zoom(this, this.src, 0, 0, 0)\">"
                            : "<a class='file' href=\"".$_G['setting']['attachurl']."forum/$attach[attachment]\" target=\"_blank\">".attachtype(fileext($attach['filename'])."\t").$attach[filename]."</a>";
					$postlist[$tid]['attach'] .= $attach['url'];
				}
			}
		}
	}
