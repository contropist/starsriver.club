...
foreach(C::t('common_adminnote')->fetch_all_by_access(explode(',', $access)) as $note) {
	if($note['expiration'] < TIMESTAMP) {
		...
		$note['checkbox'] = '<input id="delNote'.$note["id"].'" type="checkbox" name="delete[]" '.($note['admin'] == $_G['member']['username'] || $_G['adminid'] == 1 ? "value=\"$note[id]\"" : 'disabled').'><label for="delNote'.$note["id"].'"></label>';
