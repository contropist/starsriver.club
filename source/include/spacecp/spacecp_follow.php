...
if($op == 'add') {
	...
	if(submitcheck('topicsubmit')) {
		...
		if($_GET['subject'] == 'empty') {$_GET['subject'] = cutstr($_GET['message'], 80, '');}
	...
	if(submitcheck('relaysubmit')) {
		...
		showmessage('relay_feed_success', dreferer(), [], array('showdialog'=>true, 'closetime' => true));
	}
	...
	if(empty($feed)) {
		showmessage('follow_specify_follow_not_exist', '', [], array('return' => true));
	} elseif($feed['uid'] != $_G['uid'] && $_G['adminid'] != 1) {
		showmessage('quickclear_noperm', '', [], array('return' => true));
	}
