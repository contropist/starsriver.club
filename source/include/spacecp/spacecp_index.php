...
$op = in_array($_GET['op'], array('start', 'layout', 'block', 'style', 'nest', 'image', 'getblock', 'edit', 'setmusic', 'getspaceinfo', 'savespaceinfo', 'editnv', 'getpersonalnv')) ? $_GET['op'] : 'start';
...
if ($op == 'start') {
	...
	$_GET['ajaxtarget'] = empty($_GET['ajaxtarget']) ? 'nestimages' : $_GET['ajaxtarget'];
	...
	if (!empty($blockdata[$blockname]) && check_ban_block($blockname, $space)) {
		space_merge($space,'field_home');
		$usernest = getusernestdata($space);
		$para = $usernest['parameters'][$blockname];
...
if (submitcheck('nestsubmit')) {
	...
	$layoutdata = $layoutdata['nestpage'];
...
if (submitcheck('uploadsubmit')) {
	...
	echo "<script>";
	if($uploadStat == 1) {
		echo "parent.spaceNest.getnest('nest', 'albumid', '$albumid');";
		echo "parent.spaceNest.setBgImage('$picurl');";
