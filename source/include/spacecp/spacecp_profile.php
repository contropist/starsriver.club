...
$operation = in_array($_GET['op'], ['base', 'contact', 'edu', 'work', 'info', 'password', 'verify']) ? trim($_GET['op']) : $defaultop;
...
if (submitcheck('profilesubmit')) {
    ...
    foreach ($_POST as $key => $value) {
        ...
        if (in_array($key, $forumfield)) {
            ...
            if ($value >= -12 && $value <= 12 || $value == 9999) {
                C::t('common_member')->update($_G['uid'], ['timeoffset' => intval($value)]);
            }
        } elseif ($key == 'site') {
            if (!in_array(strtolower(substr($value, 0, 6)), ['http:/', 'https:', 'ftp://', 'rtsp:/', 'mms://']) &&
                !preg_match('/^static\//', $value) &&
    ...
    if ($_G['member']['freeze'] == 2 || $_G['member']['freeze'] == -1) {
        ...
        if ($status) {
            C::t('common_member_validate')->update($_G['uid'], [
...
if ($operation == 'password') {
    ...
    if (!empty($space['newemail'])) {
        $acitvemessage = lang('spacecp', 'email_acitve_message', [
            'newemail' => $space['newemail'],
            'imgdir'   => $_G['style']['imgdir']
        ]);
    }
    $actives = ['password' => ' class="active"'];
    ...
    if (in_array($operation, ['base', 'contact', 'edu', 'work', 'info'])) {
        ...
        if (empty($_G['setting']['verify'][$vid]['groupid']) || in_array($_G['groupid'], $_G['setting']['verify'][$vid]['groupid'])) {
            $actives = array('verify' => ' class="active"');
            $opactives = array($operation . $vid => ' class="active"');
    ...
    if (!empty($verify) && is_array($verify)) {
        foreach ($verify as $key => $flag) {
            if (substr($key,0,6) == 'verify' && $flag == 1) {
...
include template("home/spacecp_cent_profile");
