...
if($op == 'add') {
	...
	if(friend_request_check($uid)) {
		...
		$op = 'add2';
		$groupselect = empty($space['privacy']['groupname']) ? array(1 => ' checked') : [];
		...
		if(C::t('home_friend_request')->count_by_uid_fuid($uid, $_G['uid'])) {
			showmessage('waiting_for_the_other_test', '', [], array('alert' => 'info'));
		}
		...
		if(submitcheck('addsubmit')) {
			...
			showmessage('request_has_been_sent', dreferer(), [], array('showdialog'=>1, 'showmsg' => true, 'closetime' => true));
