...
if($_GET['op'] == 'checknewpm') {
	...
	if($waittime > 0) {
		showmessage('message_can_not_send_2', '', [], array('return' => true));
	}

	cknewuser();

	if(!checkperm('allowsendpm')) {
		showmessage('no_privilege_sendpm', '', [], array('return' => true));
	}

	if($touid) {
		if(isblacklist($touid)) {
			showmessage('is_blacklist', '', [], array('return' => true));
		}
	}
	...
	if(submitcheck('pmsubmit')) {
		...
		if(empty($message)) {
			showmessage('unable_to_send_air_news', '', [], array('return' => true));
		}
		...
		if($touid || $pmid) {
			if($touid) {
				if(($value = getuserbyuid($touid))) {
					...
					if($_G['group']['allowsendallpm'] || $value['onlyacceptfriendpm'] == 2 || ($value['onlyacceptfriendpm'] == 1 && friend_check($touid))) {
						...
						showmessage('message_can_not_send_onlyfriend', '', [], array('return' => true));
					}
				} else {
					showmessage('message_bad_touid', '', [], array('return' => true));
				}
			} else {
			...
			if(empty($membersarr)) {
				showmessage('message_bad_touser', '', [], array('return' => true));
			}
			if(isset($membersarr[$_G['uid']])) {
				showmessage('message_can_not_send_to_self', '', [], array('return' => true));
			}
			...
			showmessage('message_can_not_send_9', '', [], array('return' => true));
		}
		...
		if($return > 0) {
			...
			if(!empty($newusers)) {
				...
				if(!defined('IN_MOBILE')) {
					showmessage('do_success', 'home.php?mod=space&do=pm&subop=view&touid='.$touid, array('pmid' => $return), $_G['inajax'] ? array('msgtype' => 3, 'showmsg' => false) : []);
				} else {
			...
			if(in_array($return, range(-16, -1))) {
				...
				showmessage('message_can_not_send', '', [], array('return' => true));
			}
		}
	}
	...
	if(submitcheck('ignoresubmit')) {
		...
		if($single) {
			...
			showmessage('do_success', dreferer(), [], array('showdialog'=>1, 'showmsg' => true, 'closetime' => true));
		} else {
			uc_pm_blackls_set($_G['uid'], $_POST['ignorelist']);
			showmessage('do_success', 'home.php?mod=space&do=pm&view=ignore', [], array('showdialog'=>1, 'showmsg' => true, 'closetime' => true));
		}
	}
	...
	showmessage('do_success', '', [], array('msgtype' => 3));
