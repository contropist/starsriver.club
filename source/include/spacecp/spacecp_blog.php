<?php
    ...
    $blog = [];
    if ($blogid) {
        $blog = array_merge(C::t('home_blog')->fetch($blogid), C::t('home_blogfield')->fetch($blogid));
        if ($blog['tag']) {
    ...
    if ($_GET['op'] == 'delete') {
        ...
        if (submitcheck('sticksubmit')) {
            if ($space['uid'] === $blog['uid'] && empty($blog['status'])) {
                ...
                C::t('common_member_field_home')->update($space['uid'], ['stickblogs' => $space['stickblogs']]);
        ...
        if (submitcheck('hotsubmit')) {
            ...
            C::t('home_blog')->update($blog['blogid'], ['hot' => $_POST['hot']]);
            if ($_POST['hot'] > 0) {
                require_once libfile('function/feed');
                feed_publish($blog['blogid'], 'blogid');
            } else {
                C::t('home_feed')->update($blog['blogid'], ['hot' => $_POST['hot']], 'blogid');
            }
