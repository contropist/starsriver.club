...
if($_GET['op'] == 'base') {
	...
	if(submitcheck('addfundssubmit')) {
		if(!isset($_GET['bank_type'])) {
			showmessage('memcp_credits_addfunds_msg_notype', '', [], array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
		}
		...
		if($apitype == 'card') {
			...
			if($seccodecheck) {
				if(!check_seccode($_GET['seccodeverify'], $_GET['seccodehash'])) {
					showmessage('submit_seccode_invalid', '', [], array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
				}
			}
			...
			if(!($card = C::t('common_card')->fetch($_POST['cardid']))) {
				...
				if($card['cleardateline'] < TIMESTAMP) {
					showmessage('memcp_credits_card_msg_cleardateline_early', '', [], array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
				}
			...
			if(!$amount) {
				showmessage('memcp_credits_addfunds_msg_incorrect', '', [], array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
			}
			...
			if($apitype == 'card' && C::t('forum_order')->count_by_search($_G['uid'], null, null, null, null, null, null, $_G['timestamp'] - 180)) {
				showmessage('credits_addfunds_ctrl', '', [], array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
			}
			...
			if(C::t('forum_order')->fetch($orderid)) {
				showmessage('credits_addfunds_order_invalid', '', [], array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
			}
    ...
    if($_G['setting']['submitlock'] && discuz_process::islocked('transferlock_'.$_G['uid'], 0, 1)){
        showmessage('credits_transfer_msg_locked', '', [], array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
    }
	...
	if(submitcheck('transfersubmit')) {
		if($_GET['to'] == $_G['username']) {
			showmessage('memcp_credits_transfer_msg_self_incorrect', '', [], array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
		}
		$amount = intval($_GET['transferamount']);
		if($amount <= 0) {
			showmessage('credits_transaction_amount_invalid', '', [], array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
			...
			showmessage('credits_net_amount_iszero', '', [], array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
		}
		$to = C::t('common_member')->fetch_by_username($_GET['to']);
		if(!$to) {
			showmessage('memcp_credits_transfer_msg_user_incorrect', '', [], array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
		}
		...
		if($tmp['uid'] <= 0) {
			showmessage('credits_password_invalid', '', [], array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
		}
		...
		showmessage('credits_transfer_succeed', 'home.php?mod=spacecp&ac=credit&op=transfer', [], array('showdialog' => 1, 'showmsg' => true, 'locationtime' => true));
	}
	...
	if(submitcheck('exchangesubmit')) {
        ...
        if($_G['setting']['submitlock'] && discuz_process::islocked('exchangelock_'.$_G['uid'], 0, 1)){
            showmessage('memcp_credits_exchange_msg_locked', '', [], array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
        }
		...
		if($fromcredits == $tocredits) {
			showmessage('memcp_credits_exchange_msg_num_invalid', '', [], array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
		}
		if($outexange) {
		...
		if(!$outexange && !$_G['setting']['extcredits'][$tocredits]['ratio']) {
			showmessage('credits_exchange_invalid', '', [], array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
		}
		...
		if(!$netamount) {
			showmessage('memcp_credits_exchange_msg_balance_insufficient', '', [], array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
		...
		showmessage('credits_transaction_succeed', 'home.php?mod=spacecp&ac=credit&op=exchange', [], array('showdialog' => 1, 'showmsg' => true, 'locationtime' => true));
	}
...
include_once template("home/spacecp_cent_credit_base");
