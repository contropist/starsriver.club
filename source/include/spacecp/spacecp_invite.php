...
if($_G['setting']['creditstrans']) {
	...
	showmessage('trade_credit_invalid', '', [], array('return' => 1));
}
...
if(submitcheck('emailinvite')) {
	...
	$allcredit = $invitenum * $creditnum;
	if($space[$creditkey] < $allcredit) {
		showmessage('mail_credit_inadequate', $baseurl, [], array('showdialog'=>1, 'showmsg' => true, 'closetime' => true));
	}
...
if($_GET['op'] == 'resend') {
	...
	if(submitcheck('resendsubmit')) {
		...
		if($value = C::t('common_invite')->fetch_by_id_uid($id, $_G['uid'])) {
			...
			showmessage('send_result_resend_error', $baseurl, [], array('showdialog'=>1, 'showmsg' => true, 'closetime' => true));
		}
	}
	...
	if($value = C::t('common_invite')->fetch_by_id_uid($id, $_G['uid'])) {
		...
		showmessage('there_is_no_record_of_invitation_specified', $baseurl, [], array('showdialog'=>1, 'showmsg' => true, 'closetime' => true));
	}
...
function createmail($mail, $mailvar) {
	...
	$message = lang('spacecp', $appinfo?'app_invite_message':'invite_message', $mailvar);
