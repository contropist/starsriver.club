...
if(in_array($do, array('buy', 'exit'))) {
	...
	foreach($_G['cache']['usergroups'] as $gid => $group) {
		...
		$g = '<li><a href="home.php?mod=spacecp&ac=usergroup&gid='.$gid.'"'.(!empty($_GET['gid']) && $_GET['gid'] == $gid ? '' : '').'>'.$group['grouptitle'].'</a><li>';
	...
	$usergroups['my'] = '<li><a href="home.php?mod=spacecp&ac=usergroup">'.$maingroup['grouptitle'].'</a></li>'.$usergroups['my'];
	if($activegs == []) {
		$activegs['my'] = ' a';
	}
...
include_once template("home/spacecp_cent_usergroup");
