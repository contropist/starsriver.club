    ...
    if ($id) {
        ...
        if ($count) {
            ...
            if (empty($list) && empty($cid)) {
                ...
                C::t('home_blog')->update($blog['blogid'], ['replynum' => $count]);
            }
        }
        ...
        if (!$_G['setting']['preventrefresh'] || !$space['self'] && $_G['cookie']['viewid'] != 'blog_' . $blog['blogid']) {
            C::t('home_blog')->increase($blog['blogid'], 0, ['viewnum' => 1]);
        ...
        include_once template("nest:home/space_blog_view");
        ...
        $query = C::t('home_class')->fetch_all_blogclass();
        foreach ($query as $value) {
