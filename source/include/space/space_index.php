 ...
 *
 *      $Id: space_home_index.php 31354 2012-08-16 03:03:08Z chenmengshu $
 */
...
if ($_GET['op'] == 'getmusiclist') {
	...
	if($reauthcode == $_GET['hash']) {
		space_merge($space,'field_home');
		$usernest = getusernestdata($space);
		$musicmsgs = $usernest['parameters']['music'];
	...
	$widths = getlayout($usernest['currentlayout']);
	$leftlist = formatdata($usernest, 'left', $space);
	$centerlist = formatdata($usernest, 'center', $space);
	$rightlist = formatdata($usernest, 'right', $space);

	dsetcookie('home_nestmode', 1);
}
...
include_once(template('home/space_home_index'));
