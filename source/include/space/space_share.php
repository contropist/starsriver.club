<?php
    ...
    $_GET['type'] = in_array($_GET['type'], ['all', 'link', 'video', 'music', 'iframe', 'blog', 'album', 'pic', 'poll', 'space', 'thread', 'article',]) ? $_GET['type'] : 'all';
    if ($id) {
        ...
        if (!IS_ROBOT) {
            ...
            if ($_GET['type'] && $_GET['type'] != 'all') {
                $sub_actives = ['type_' . $_GET['type'] => 'active'];
                $type = $_GET['type'];
            } else {
                $sub_actives = ['type_all' => 'active'];
            }
