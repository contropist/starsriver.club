...
$_GET['type'] = in_array($_GET['type'], array('all', 'link', 'video', 'music', 'iframe', 'blog', 'album', 'pic', 'poll', 'space', 'thread', 'article'))? $_GET['type'] : 'all';
if($id) {
	...
	include_once template("nest:home/space_share_view");
	...
	if(!IS_ROBOT) {
		...
		if($_GET['type'] && $_GET['type'] != 'all') {
			$sub_actives = array('type_'.$_GET['type'] => 'active');
			$type = $_GET['type'];
		} else {
			$sub_actives = array('type_all' => 'active');
		}
	...
	include_once template("nest:home/space_route_share");
}
