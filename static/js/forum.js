...
function removeindexheats() {
	return confirm('你确认要把此文章从热点文章中移除么？');
}
...
function atarget(obj) {
	obj.target = getcookie('atarget') === 'checked' ? '_blank' : '';
}

function setatarget(v) {
	$('atarget').className = v;
	$('atarget').onclick = function() {setatarget(v == 'checked' ? '' : 'checked');};
...
function loadData(quiet, formobj) {
	...
	if(!quiet && !confirm('此操作将覆盖当前回复内容，确定要恢复数据吗？')) {
		return;
	}
...
function checkForumnew(fid, lasttime) {
	...
	x.get('forum.php?mod=ajax&action=forumchecknew&fid=' + fid + '&time=' + lasttime + '&inajax=yes', function(s){
		if(s > 0) {
			...
			var checknew = {'tid':'', 'thread':{'common':{'className':'', 'val':'<a href="javascript:void(0);" onclick="ajaxget(\'forum.php?mod=ajax&action=forumchecknew&fid=' + fid+ '&time='+lasttime+'&uncheck=1&inajax=yes\', \'forumnew\');">有新回复的文章，点击查看', 'colspan': colspan }}};
