	...
	if(typeof aimgcount == 'object' && $('imagelistthumb_' + pid)) {
		for(pid in aimgcount) {
			...
			for(i = 0;i < aimgcount[pid].length;i++) {
				...
				if(fid) {
					imagelist += '<div id="pattimg_' + aimgcount[pid][i] + '_menu" class="tip tip_4" style="display: none;"><div class="tip_c"><a href="forum.php?mod=ajax&action=setthreadcover&aid=' + aimgcount[pid][i] + '&fid=' + fid + '" onclick="showWindow(\'setcover' + aimgcount[pid][i] + '\', this.href)">设为封面</a></div></div>';
				}
...
function succeedhandle_fastpost(locationhref, message, param) {
	...
	if(pid) {
		...
		if(!message) {
			message = '本版回复需要审核，你的回复将在通过审核后显示';
		}
...
function connect_get_thread() {
	...
	if ($('postmessage_' + connect_thread_info.post_id)) {
		connect_thread_info.html_content = preg_replace(["'"], ['%27'], encodeURIComponent(preg_replace(['本文章最后由 .*? 于 .*? 编辑','&nbsp;','<em onclick="copycode\\(\\$\\(\'code0\'\\)\\);">复制代码</em>'], ['',' ', ''], $('postmessage_' + connect_thread_info.post_id).innerHTML)));
	}
...
function show_threadpage(pid, current, maxpage, ispreview) {
	...
	var s = '<div class="pg">';
	if(pstart > 1) {
		s += '<a onclick="' + clickvalue(1) + '">1</a><span class="gap">...</span>';
	}
	for(i = pstart;i <= pend;i++) {
		s += i == current ? '<a class="active">' + i + '</a>' : '<a onclick="' + clickvalue(i)+ '">' + i + '</a>';
	}
	if(pend < maxpage) {
		s += '<span class="gap">...</span><a onclick="' + clickvalue(maxpage)+ '">' + maxpage + '</a>';
	}
	if(current < maxpage) {
		s += '<a onclick="' + clickvalue(current + 1) + '" class="next"></a>';
	}
	s += '<a onclick="' + clickvalue('all') + '">查看所有</a>';
	s += '</div>';
	...
	if(aimgcount[pid]) {
		for(var i = 0; i < aimgcount[pid].length;i++) {
			...
			if(aimgObj.parentElement.className!="mbn") {
				...
				imagelist += '<div class="mbn"><img src="' + src + '" width="720" /></div>';
			}
		}
	}
...
function showmobilebbs(obj) {
	var content = '<h3 class="flb" style="cursor:move;"><em>下载掌上论坛</em><span><a class="flbc" onclick="hideWindow(\'mobilebbs\')" title="close">×</a></span></h3><div class="c"><h4>Andriod版本，扫描二维码可以直接下载到手机</h4><p class="mtm mbm vm"><span class="code_bg"><img src="'+ STATICURL +'image/common/zslt_andriod.png" alt="" /></span><img src="'+ STATICURL +'image/common/andriod.png" alt="适用于装有安卓系统的三星/HTC/小米等手机" /></p><h4>iPhone版本，扫描二维码可以直接下载到手机</h4><p class="mtm mbm vm"><span class="code_bg"><img src="'+ STATICURL +'image/common/zslt_ios.png" alt="" /></span><img src="'+ STATICURL +'image/common/ios.png" alt="适用于苹果手机" /></p></div>';
