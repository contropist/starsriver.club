/*
	...
	$Id: portal_nest.js 31093 2012-07-16 03:54:34Z zhangguosheng $
*/
...
drag.extend({
	...
	openBlockEdit : function (e,op) {
		...
		this.removeMenu();
		showWindow('showblock', 'portal.php?mod=portalcp&ac=block&op='+op+'&bid='+bid+'&tpl='+document.nestform.template.value, 'get', -1);
	},
	getnestClassName : function (id,index) {
	...
	getRule : function (selector,attr) {
		selector = spacenest.checkSelector(selector);
		var value = (!selector || !attr) ? '' : spacenest.styleSheet.getRule(selector, attr);
		return value;
	},
	openStyleEdit : function (e) {
		...
		var bgcolor = '',bgimage = '',bgrepeat = '',html = '',nestClassName = '',fontcolor = '',fontsize = '',linkcolor = '',linkfontsize = '';
		...
		var linkSelector = spacenest.checkSelector(selector+ ' .'+this.contentClass+' a');
		...
		nestClassName = this.getnestClassName(id,0);
		...
		table += '</ul><p class="ptm"><label><input id="borderop" type="checkbox" value="1" '+opchecked+' onclick="$(\'borderul\').className = $(\'borderul\').className == \'borderul\' ? \'borderula\' : \'borderul\'">分别设置</label></p></td></tr>';
		...
		table += '</div><p class="ptm"><label><input id="marginop" type="checkbox" value="1" '+opchecked+' onclick="$(\'margindiv\').className = $(\'margindiv\').className == \'borderul\' ? \'borderula\' : \'borderul\'">分别设置</label></p></td></tr>';
		...
		if (objType == 1) {
			...
			table += '</div><p class="ptm"><label><input id="cmarginop" type="checkbox" value="1" '+opchecked+' onclick="$(\'cmargindiv\').className = $(\'cmargindiv\').className == \'borderul\' ? \'borderula\' : \'borderul\'"> 分别设置</label></p></td></tr>';
		}
		...
		table += '<tr><th>指定class</th><td><input type="text" id="nestClassName" class="px p_fre" value="'+nestClassName+'" size="8" /> <select class="ps vm" id="bgrepeat" onchange="$(\'nestClassName\').value=this.value;" >'+this.getOption(classarr, nestClassName)+'</select></td></tr>';
		table += '</table>';

		var wname = objType ? '模块' : '框架';
		html = '<div class="c nestwin" style="width:450px;position:relative;">'+table+'</div>';
		var h = '<h3 class="flb"><em>编辑'+wname+'样式</em><span><a class="close" onclick="drag.closeStyleEdit(\''+id+'\');return false;" title="close">×\n\
			</a></span></h3>';
		var f = '<p class="o pns"><button onclick="drag.saveStyle(\''+id+'\');drag.closeStyleEdit(\''+id+'\');" class="pn pnc" value="true">\n\
			<strong>确定</strong></button><button onclick="drag.closeStyleEdit(\''+id+'\')" value="true"><strong>取消</strong></button></p>';
		this.removeMenu(e);
	...
	saveStyle : function (id) {
		var className = this.getnestClassName(id);
		var nestClassName = $('nestClassName').value;
		$(id).className = nestClassName+' '+className[2]+' '+className[1];
		var obj = this.getObjByName(id);
		var objType = obj instanceof Block ? 1 : 0;

		if (objType == 1) this.saveBlockClassName(id,nestClassName);

		var selector = '#'+id;
		var random = Math.random();
		spacenest.setStyle(selector, 'background-color', $('bgcolor').value, random);
		...
		spacenest.setStyle(selector, 'background-image', bgimage, random);
		spacenest.setStyle(selector, 'background-repeat', bgrepeat, random);
		spacenest.setStyle(selector+' .'+this.contentClass, 'color', $('fontcolor').value, random);
		spacenest.setStyle(selector+' .'+this.contentClass, 'font-size', this.formatValue('fontsize'), random);
		spacenest.setStyle(spacenest.checkSelector(selector+' .'+this.contentClass+' a'), 'color', $('linkcolor').value, random);
		var linkfontsize = parseInt($('linkfontsize').value);
		linkfontsize = isNaN(linkfontsize) ? '' : linkfontsize+'px';
		spacenest.setStyle(spacenest.checkSelector(selector+' .'+this.contentClass+' a'), 'font-size', this.formatValue('linkfontsize'), random);
		...
		spacenest.setStyle(selector, 'border', '', random);
		spacenest.setStyle(selector, 'border-top-width', bdtwidth, random);
		spacenest.setStyle(selector, 'border-right-width', bdrwidth, random);
		spacenest.setStyle(selector, 'border-bottom-width', bdbwidth, random);
		spacenest.setStyle(selector, 'border-left-width', bdlwidth, random);

		spacenest.setStyle(selector, 'border-top-style', bdtstyle, random);
		spacenest.setStyle(selector, 'border-right-style', bdrstyle, random);
		spacenest.setStyle(selector, 'border-bottom-style', bdbstyle, random);
		spacenest.setStyle(selector, 'border-left-style', bdlstyle, random);

		spacenest.setStyle(selector, 'border-top-color', bdtcolor, random);
		spacenest.setStyle(selector, 'border-right-color', bdrcolor, random);
		spacenest.setStyle(selector, 'border-bottom-color', bdbcolor, random);
		spacenest.setStyle(selector, 'border-left-color', bdlcolor, random);
		...
		spacenest.setStyle(selector, 'margin-top',margint, random);
		spacenest.setStyle(selector, 'margin-right', marginr, random);
		spacenest.setStyle(selector, 'margin-bottom', marginb, random);
		spacenest.setStyle(selector, 'margin-left', marginl, random);
		...
		if (objType == 1) {
			...
			selector = selector + ' .'+this.contentClass;
			spacenest.setStyle(selector, 'margin-top', cmargint, random);
			spacenest.setStyle(selector, 'margin-right', cmarginr, random);
			spacenest.setStyle(selector, 'margin-bottom', cmarginb, random);
			spacenest.setStyle(selector, 'margin-left', cmarginl, random);
		}
	...
	saveBlockClassName : function(id,className){
		if (!$('saveblockclassname')){
			...
			dom.innerHTML = '<form id="saveblockclassname" method="post" action=""><input type="hidden" name="classname" value="" />\n\
				<input type="hidden" name="formhash" value="'+document.nestform.formhash.value+'" /><input type="hidden" name="saveclassnamesubmit" value="true"/></form>';
	...
	openTitleEdit : function (e) {
		...
		if (obj instanceof Tab) {
			...
			common += '<tr><th>切换类型:</th><td><select id="switchType" >'+this.getOption(switchArr,switchType)+'</select></td></tr>';
		}
		common += '</table><hr>';
		var li = '';
		...
		li += '</td></tr><tr><td colspan="2"><hr></td></tr></table></div>';
		var html = '';
		if (obj.titles['first']) {
		...
		var c = len + 1;
			html = '<div class="c nestwin" style="width:450px;height:400px; overflow:auto;"><table cellspacing="0" cellpadding="0" class="tfm pns"><tr><th></th><td><button type="button" id="addTitleInput" onclick="drag.addTitleInput('+c+');"><em>添加新标题</em></button></td></tr></table><div id="titleEdit">'+html+common+'</div></div>';
		var h = '<h3 class="flb"><em>编辑'+titlename+'标题</em><span><a class="close" onclick="drag.closeTitleEdit(\''+fid+'\');return false;" title="close">×\n\
			</a></span></h3>';
		var f = '<p class="o pns"><button onclick="drag.saveTitleEdit(\''+fid+'\');drag.closeTitleEdit(\''+fid+'\');" class="pn pnc" value="true">\n\
			<strong>确定</strong></button><button onclick="drag.closeTitleEdit(\''+fid+'\')" value="true"><strong>取消</strong></button></p>';
		this.removeMenu(e);
	...
	saveBlockTitle : function (id,title) {
		if (!$('saveblocktitle')){
			...
			dom.innerHTML = '<form id="saveblocktitle" method="post" action=""><input type="hidden" name="title" value="" />\n\
				<input type="hidden" name="formhash" value="'+document.nestform.formhash.value+'" /><input type="hidden" name="savetitlesubmit" value="true"/></form>';
			$('append_parent').appendChild(dom.childNodes[0]);
		}
	...
	removeBlock : function (e, flag) {
		...
		var obj = this.getObjByName(id);
		if (!flag) {
			if (!confirm('确实要删除吗,删除以后将不可恢复')) return false;
		}
		if (obj instanceof Block) {
			this.delBlock(id);
		} else if (obj instanceof Frame) {
	...
	delBlock : function (bid) {
		spacenest.removeCssSelector('#'+bid);
		this.stopSlide(bid);
	},
	delFrame : function (frame) {
		spacenest.removeCssSelector('#'+frame.name);
	...
	getBlockData : function (blockname) {
		...
		showWindow('showblock', 'portal.php?mod=portalcp&ac=block&op=block&classname='+blockname+'&bid='+bid+'&eleid='+eleid+'&tpl='+document.nestform.template.value,'get',-1);
		drag.initPosition();
		this.fn = '';
		return true;
	},
	...
	blockForceUpdate : function (e,all) {
		...
		var x = new Ajax();
		x.get('portal.php?mod=portalcp&ac=block&op=getblock&forceupdate=1&inajax=1&bid='+bid+'&tpl='+document.nestform.template.value, function(s) {
			if(s.indexOf('errorhandle_') != -1) {
				bcontent.innerHTML = boldcontent;
				runslideshow();
				showDialog('抱歉，你没有权限添加或编辑模块', 'alert');
				doane();
			} else {
	...
	frameExport : function (e) {
		var flag = true;
		if (drag.isChange) {
			flag = confirm('你已经做过修改，请保存后再做导出，否则导出的数据将不包括你这次所做的修改。');
		}
		if (flag) {
			if ( typeof e == 'object') {
				e = Util.event(e);
			...
			if (!$('frameexport')){
				...
				dom.innerHTML = '<form id="frameexport" method="post" action="" target="_blank"><input type="hidden" name="frame" value="" />\n\
					<input type="hidden" name="tpl" value="'+document.nestform.template.value+'" />\n\
					<input type="hidden" name="tpldirectory" value="'+document.nestform.tpldirectory.value+'" />\n\
					<input type="hidden" name="nestsign" value="'+document.nestform.nestsign.value+'" />\n\
					<input type="hidden" name="formhash" value="'+document.nestform.formhash.value+'" /><input type="hidden" name="exportsubmit" value="true"/></form>';
				$('append_parent').appendChild(dom.childNodes[0]);
			}
			$('frameexport').action = 'portal.php?mod=portalcp&ac=nest&op=export';
	...
	openFrameImport : function (type) {
		type = type || 0;
		showWindow('showimport','portal.php?mod=portalcp&ac=nest&op=import&tpl='+document.nestform.template.value+'&tpldirectory='+document.nestform.tpldirectory.value+'&nestsign='+document.nestform.nestsign.value+'&type='+type, 'get');
	},
	...
	clearAll : function () {
		if (confirm('你确实要清空页面上所在nest数据吗,清空以后将不可恢复')) {
	...
	createObj : function (e,objType,contentType) {
		if (objType == 'block' && !this.checkHasFrame()) {alert("提示：未找到框架，请先添加框架。");spacenest.getnest('frame');return false;}
		e = Util.event(e);
	...
	goonnest : function () {
		if ($('prefile').value == '1') {
			...
			spacenest.disablePreviewButton();
		}
		setInterval(function(){spacenest.save('savecache', 1);},180000);
	}
});

var spacenest = new NEST();
spacenest.extend({
	save : function (optype,rejs) {
		optype = typeof optype == 'undefined' ? '' : optype;
		if (optype == 'savecache' && !drag.isChange) {return false;}
		var tplpre = document.nestform.template.value.split(':');
		if (!optype) {
			if (['portal/portal_topic_content', 'portal/list', 'portal/view'].indexOf(tplpre[0]) == -1) {
				if (document.nestform.template.value.indexOf(':') > -1 && !document.selectsave) {
					var schecked = '',dchecked = '';
					if (document.nestform.savemod.value == '1') {
						dchecked = ' checked';
					} else {
						schecked = ' checked';
					}
					showDialog('<form name="selectsave" action="" method="get"><label><input type="radio" value="0" name="savemod"'+schecked+' />应用于此类全部页面</label>\n\
					<label><input type="radio" value="1" name="savemod"'+dchecked+' />只应用于本页面</label></form>','notice', '', spacenest.save);
					return false;
				}
				if (document.selectsave) {
					if (document.selectsave.savemod[0].checked) {
						document.nestform.savemod.value = document.selectsave.savemod[0].value;
					} else {
						document.nestform.savemod.value = document.selectsave.savemod[1].value;
					}
				}
			} else {
				document.nestform.savemod.value = 1;
			}
		} else if (optype == 'savecache') {
			if (!drag.isChange) return false;
			this.checkPreview_form();
			document.nestform.rejs.value = rejs ? 0 : 1;
		...
		document.nestform.action = document.nestform.action.replace(/[&|\?]inajax=1/, '');
		document.nestform.optype.value = optype;
		document.nestform.spacecss.value = spacenest.getSpacecssStr();
		document.nestform.style.value = spacenest.style;
		document.nestform.layoutdata.value = drag.getPositionStr();
		document.nestform.gobackurl.value = spacenest.cancelnestUrl();
		drag.clearClose();
		if (optype == 'savecache') {
			document.nestform.handlekey.value = 'nestform';
			ajaxpost('nestform','ajaxwaitid','ajaxwaitid','onerror');
		} else {
			saveUserdata('nest_advance_mode', '');
			document.nestform.submit();
		}
	},
	...
	cancelnestUrl : function () {
		return location.href.replace(/[\?|\&]nest\=yes/g,'').replace(/[\?|\&]preview=yes/,'');
	},
	cancel : function () {
		saveUserdata('nest_advance_mode', '');
	...
	recover : function() {
		if (confirm('你确定要恢复到上一版本保存的结果吗？')) {
			drag.clearClose();
			document.nestform.recover.value = '1';
			document.nestform.gobackurl.value = location.href.replace(/(\?nest=yes)|(\&nest=yes)/,'').replace(/[\?|\&]preview=yes/,'');
			document.nestform.submit();
		}
		doane();
	},
	enablePreviewButton : function () {
		if ($('preview')){
			$('preview').className = '';
			if(drag.isChange) {
				$('nest_preview').onclick = function () {spacenest.save('savecache');return false;};
			} else {
				$('nest_preview').onclick = function () {spacenest.save('preview');return false;};
			}
	...
	disablePreviewButton : function () {
		if ($('preview')) {
			...
			$('nest_preview').onclick = function () {return false;};
		}
	},
	cancelNEST : function () {
		this.disablePreviewButton();
		document.nestform.optype.value = 'cancelnest';
		var x = new Ajax();
		x.post($('nestform').action+'&inajax=1','optype=cancelnest&nestsubmit=1&template='+document.nestform.template.value+'&savemod='+document.nestform.savemod.value+'&formhash='+document.nestform.formhash.value+'&tpldirectory='+document.nestform.tpldirectory.value+'&nestsign='+document.nestform.nestsign.value,function(s){});
	},
	...
	getnest : function (type) {
		if (type) {
			...
			var ajaxtarget = type == 'nest' ? 'nestimages' : '';
			var x = new Ajax();
			x.showId = ajaxtarget;
			x.get('portal.php?mod=portalcp&ac=nest'+para+'&inajax=1&ajaxtarget='+ajaxtarget,function(s, x) {
				if (s) {
					...
					$('content'+type).style.display = 'block';
					if (type == 'nest') {
						spacenest.setCurrentnest(spacenest.currentnest);
						if (spacenest.styleSheet.rules.length > 0) {
...
function succeedhandle_nestform (url, message, values) {
	if (values['rejs'] == '1') {
		document.nestform.rejs.value = '';
