...
function getthreadclass() {
	if($('fid')) {ajaxget('forum.php?mod=ajax&action=getthreadclass&fid=' + fid.value, 'typeid', null, null, null, showthreadclass);}
}

function showthreadclass() {
	try{
		$('append_parent').removeChild($('typeid_ctrl_menu'));
		$('typeid_ctrl').parentNode.removeChild($('typeid_ctrl'));
	}catch(e) {}
