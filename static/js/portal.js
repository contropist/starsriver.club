...
function block_delete_item(bid, itemid, itemtype, itemfrom, from) {
	var msg = itemtype==1 ? '你确定要删除该数据吗？' : '你确定要屏蔽该数据吗？';
	if(confirm(msg)) {
...
function recommenditem_byblock(bid, id, idtype) {
	...
	if(editarea) {
		...
		if(!$('recommendback')) {
			...
			back.innerHTML = '<em id="recommendback" onclick="recommenditem_back()" class="ft-chevron-left back l"></em>';
...
function blockBindTips() {
	...
	for(var i = 0; i < elems.length; i++) {
		...
		if(tips && ! elems[i].isBindTips) {
			...
			k++;
			showPrompt(elems[i].id, 'mouseover', tips, 1, true,'danger');
		}
	}
}
