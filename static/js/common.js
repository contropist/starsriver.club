Loadingtext =  '' +
    ...
    '    </g>' +
    '  </svg>' +
    '  <svg version="1.1" id="cloud" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 1 10 7" enable-background="new 0 0 10 10">' +
...
function checkall(form, prefix, checkall ,delay) { //delay 设置为 ture 来避免直接全选导致的反选问题
    var checkall = checkall ? checkall : 'chkall';
    var chklist = form.getElementsByTagName('input');
    var count = 0;
    var foo = function () {
    ...
    if(delay){setTimeout(function () {foo();},0)} else {foo();}
	return count;
}
...
function Ajax(recvType, waitId) {
	...
	aj.showLoading = function() {
		if(aj.waitId && (aj.XMLHttpRequest.readyState != 4 || aj.XMLHttpRequest.status != 200)) {
			...
			aj.waitId.innerHTML = Loadingtext;
		}
	};
...
function loadcss(cssname) {
	if(!CSSLOADED[cssname]) {
		var csspath = (typeof CSSPATH == 'undefined' ? 'data/cache/' : CSSPATH);
		if(!$('css_' + cssname)) {
...
function showMenu(v) {
	...
	var maxh = isUndefined(v['maxh']) ? 720 : v['maxh'];
...
function showDialog(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt, closetime, locationtime) {
	...
	mode = in_array(mode, ['confirm', 'notice', 'info', 'right', 'form']) ? mode : 'alert';
	...
	if(closetime) {
        closetime = closetime >= 3 ? closetime : 3;
		showPrompt(null, null, msg, closetime * 1200, 'popuptext',(mode === 'info' ? mode : 'warning'));
		return;
	}
	...
	var s = hidedom +
		'<div class="fwin">' +
		'	<div class="header" onmousedown="dragMenu($(\'' + menuid+ '\'), event, 1)">' +
		'		<span class="title-tip">' + (t ? t : '注意') + '</span>' +
		'		<a class="close" id="fwin_dialog_close" onclick="hideMenu(\'' + menuid + '\', \'dialog\')" title="close">×</a>' +
		'	</div>';
	if(mode === 'info') {
		s += msg ?
			'<div class="body">' + msg + '</div>' +
			'<div class="footer">' +
			'	<button id="fwin_dialog_cancel" value="true" class="r btn btn-small btn-warning px4-radius" onclick="hideMenu(\'' + menuid + '\', \'dialog\')">'+confirmtxt+'</button>' +
			'</div>'
			:
			'<div class="body">' +
			'	<p>我就是想出来看看你</p>' +
			'</div>' +
			'<div class="footer">' +
			'	<button id="fwin_dialog_cancel" value="true" class="r btn btn-small btn-warning px4-radius" onclick="hideMenu(\'' + menuid + '\', \'dialog\')">滚</button>' +
			'</div>';
	} else if(mode === 'form'){
        s += msg;
	} else {
		s += '' +
			'<div class="body">' +
			'	<div class="' + (mode === 'alert' ? 'alert_error' : (mode === 'right' ? 'alert_right' : 'alert_info')) + '">' +
			'		<p>' + msg + '</p>' +
			'	</div>' +
			'</div>' +
			'<div class="footer">' +
				(leftmsg ? '<i class="str">' + leftmsg + '</i>' : '') +
				(showconfirm ? '<button id="fwin_dialog_submit" value="true" class="r btn btn-small btn-success glass px4-radius">'+confirmtxt+'</button>' : '')+
				(mode === 'confirm' ? '<button id="fwin_dialog_cancel" value="true" class="r btn btn-small btn-warning glass px4-radius" onclick="hideMenu(\'' + menuid + '\', \'dialog\')">'+canceltxt+'</button>' : '') +
			'</div>';
	}
	s += '</div>';
	menuObj.innerHTML = s;
...
function showWindow(k, url, mode = false, cache, menuv) {
	...
	if(!menuObj) {
		...
		menuObj.innerHTML = hidedom
          ...
          + '<div class="black-back"><div  id="fwin_content_' + k + '" class="fwin"></div></div>';
...
function simulateSelect(selectId, widthvalue) {
	...
	selectObj.outerHTML += '<a id="' + selectId + '_ctrl" style="width:' + widthvalue + ';" class="awe-bars typec" tabindex="' + tabindex + '">' + defaultopt + '</a>';
	menuObj.id = selectId + '_ctrl_menu';
	menuObj.className = 'subjectype';
	menuObj.style.display = 'none';
    menuObj.appendChild(ul);
    menuObj.style.width = $(selectId + '_ctrl').clientWidth + 2 + 'px';
	$('append_parent').appendChild(menuObj);

	$(selectId + '_ctrl').onclick = function(e) {
		$(selectId + '_ctrl_menu').style.width = widthvalue;
...
function thumbImg(obj, method) {
	...
	if(method) {
		...
		fixw = typeof imagemaxwidth == 'undefined' ? '720' : imagemaxwidth;
		if(zw > fixw) {
			zw = fixw;
			zh = zw / zr;
...
function opennest(){
	if(DYNAMICURL) {
		window.location.href = SITEURL+DYNAMICURL + (DYNAMICURL.indexOf('?') < 0 ? '?' : '&') + ('nest=yes');
	} else {
		window.location.href = ((window.location.href + '').replace(/[\?\&]nest=yes/g, '').split('#')[0] + ( window.location.search && window.location.search.indexOf('?nest=yes') < 0 ? '&nest=yes' : '?nest=yes'));
	}
}
...
function starrate(ratebody,inputarg) {
	var rate = 0;
	var input = document.querySelector(inputarg);
	var rater =  document.querySelector(ratebody);

	rater.innerHTML = '' +
        '<i class="awe-star-o"></i>' +
        '<i class="awe-star-o"></i>' +
        '<i class="awe-star-o"></i>' +
        '<i class="awe-star-o"></i>' +
        '<i class="awe-star-o"></i>';

	var stars =  rater.querySelectorAll('i');

    var star = function(e){
    	return e.getAttribute('data-rtl') ? e.getAttribute('data-rtl') : 0;
	};

    var starsinit = function(){
        for(var s = 0; s < stars.length ; s++){
        	stars[s].setAttribute('data-rtl',s+1);
            stars[s].className = s < rate ? 'awe-star light' : 'awe-star-o';
        }
	};

    var starslight = function(click = 0){
        starsinit();
        var lev = star(getEventobj());
        if(click === true){
            rater.addClass('animal');
            setTimeout(function () {
                rater.delClass('animal');
            },300 + 60 * lev);

            for(var i = 0; i < stars.length ; i++){
                delEvent(stars[i],'mouseenter',starslight);
                delEvent(stars[i],'mouseleave',starsinit);
            }
		}
		for(i = 0; i < stars.length ; i++){
			stars[i].className = i < lev ? 'awe-star light' : 'awe-star-o';
		}
	};

    var starclick = function () {
        starslight(true);
        input.value = rate = star(getEventobj())
    };

	for(var i = 0 ;i < stars.length; i++){
		stars[i].setAttribute('data-rtl',i+1);
		addEvent(stars[i],'click',starclick);
		addEvent(stars[i],'mouseenter',starslight);
		addEvent(stars[i],'mouseleave',starsinit);
	}
}
...
function toggleBlind(dom) {
	if(dom) {
		if(loadUserdata('is_blindman')) {
            ...
            dom.title = '开';
            dom.parentNode.classList.add('on');
		} else {
			saveUserdata('is_blindman', '1');
            dom.title = '关';
            dom.parentNode.classList.remove('on');
		}
	}
}
function checkBlind() {
...
JSMENU['drag'] = [];
JSMENU['layer'] = 0;
JSMENU['zIndex'] = {'win':110,'menu':120,'dialog':130,'prompt':140};
JSMENU['float'] = '';
