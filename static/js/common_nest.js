/*
	...
	$Id: common_nest.js 31093 2012-07-16 03:54:34Z zhangguosheng $
*/
...
(function(){
	...
	Drag.prototype = {
		...
		initAdvanced : function () {
			...
			this.initTips();
			if(this.goonNEST) this.goonNEST();
			this.openfn();
		},
		...
		initCommon : function () {
			this.advancedStyleSheet = $('nest_common');
			this.menu = [];
		},
		initSample : function () {
			this._getMoveableArea();
		...
		setSampleMode : function (sampleMode) {
			if(loadUserdata('nest_advance_mode')) {
				this.sampleMode = '';
			} else {
				this.sampleMode = sampleMode;
				saveUserdata('nest_advance_mode', sampleMode ? '' : '1');
			}
	...
	NEST.prototype = {
		...
		init : function (mod) {
			drag.init(mod);
			this.style = document.nestform.style.value;
			if (this.style == '') {
			...
			this.currentLayout = typeof document.nestform.currentlayout == 'undefined' ? '' : document.nestform.currentlayout.value;
			this.initStyleSheet();
			if (this.styleSheet.rules) this.initnestStyle();
		},
		initStyleSheet : function () {
			var all = document.styleSheets;
			for (var i=0;i<all.length;i++) {
				var ownerNode = all[i].ownerNode || all[i].owningElement;
				if (ownerNode.id == 'nest_style') {
					this.styleSheet = new styleCss(i);
					return true;
				}
			}
		},
		initnestStyle : function (css) {
			var allCssText = css || $('nest_style').innerHTML;
		...
		changeLayout : function (newLayout) {
			...
			if (newLayoutLength == currentLayoutLength){
				...
				var _length = drag.data['nestpage'][0]['columns']['frame1_right']['children'].length;
				var tobj = $('frame1_center_temp');
				for (var i = 0; i < _length; i++) {
					var name = drag.data['nestpage'][0]['columns']['frame1_right']['children'][i].name;
		...
		getRandomBlockName : function () {
			var left = drag.data['nestpage'][0]['columns']['frame1_left']['children'];
			if (left.length > 2) {
				var block = left[0];
			} else {
				var block = drag.data['nestpage'][0]['columns']['frame1_center']['children'][0];
			}
			return block.name;
		},
		...
		setCurrentnest : function (type) {
			if (type) {
				$('nest_tag_'+this.currentnest).className = '';
				this.currentnest = type;
				$('nest_tag_'+this.currentnest).className = 'activity';
				this.initPalette(this.currentnest);
			}
		},
		hideBg : function () {
			var random = Math.random();
			this.setStyle(this.currentnest, 'background-image', '', random);
			this.setStyle(this.currentnest, 'background-color', '', random);
			if (this.currentnest == 'hd') this.setStyle(this.currentnest, 'height', '', random);
			this.initPalette(this.currentnest);
		},
		...
		setBgImage : function (value) {
			...
			var random = Math.random();
			this.setStyle(this.currentnest, 'background-image', Util.url(path), random);
			this.setStyle(this.currentnest, 'background-repeat', 'repeat', random);
			if (this.currentnest == 'hd') {
				var _method = this;
				var img = new Image();
				img.onload = function () {
					if (parseInt(img.height) < 140) {
						NEST.prototype.setStyle.call(_method, _method.currentnest, 'background-repeat', 'repeat', random);
						NEST.prototype.setStyle.call(_method, _method.currentnest, 'height', '', random);
					} else {
						NEST.prototype.setStyle.call(_method, _method.currentnest, 'height', img.height+"px", random);
					}
				};
				img.src = path;
			}
		...
		setBgRepeat : function (value) {
			...
			this.setStyle(this.currentnest, 'background-repeat', repeat[value]);
		},
		setBgAttachment : function (value) {
			var attachment = ['scroll','fixed'];
			if (typeof attachment[value] == 'undefined') {return false;}
			this.setStyle(this.currentnest, 'background-attachment', attachment[value]);
		},
		setBgColor : function (color) {
			this.setStyle(this.currentnest, 'background-color', color);
		},
		setTextColor : function (color) {
			this.setStyle(this.currentnest, 'color', color);
		},
		setLinkColor : function (color) {
			this.setStyle(this.currentnest + ' a', 'color', color);
		},
		setBgPosition : function (id) {
			...
			this.setStyle(this.currentnest, 'background-position', position[i]);
		},
		...
		cancel : function () {
			var flag = false;
			if (this.isChange) {
				flag = confirm(this.cancelConfirm ? this.cancelConfirm : '退出将不会保存你刚才的设置。是否确认退出？');
			}
			if (!this.isChange || flag) {
				location.href = location.href.replace(/[\?|\&]nest\=yes/g,'');
			}
		},
		extend : function (obj) {
			for (var i in obj) {
