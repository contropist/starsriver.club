var editorcurrentheight = 400, editorminheight = 400, savedataInterval = 30, editbox = null, editwin = null,
    editdoc = null, editcss = null, savedatat = null, savedatac = 0, autosave = 1, framemObj = null, cursor = -1,
    stack = [], initialized = false, postSubmited = false, editorcontroltop = false, editorcontrolwidth = false,
    editorcontrolheight = false, editorisfull = 0, fulloldheight = 0, savesimplodemode = null;
EXTRAFUNC['keydown'] = [];
EXTRAFUNC['keyup'] = [];
EXTRAFUNC['mouseup'] = [];
...
function savedataTime() {
    if (!autosave) {
        $(editorid + '_svdsecond').innerHTML = '<a title="点击开启自动缓存" onclick="setAutosave()">开启自动缓存</a> ';
        return;
    }
    if (!savedatac) {
        savedatac = savedataInterval;
        ...
        h = h < 10 ? '0' + h : h;
        m = m < 10 ? '0' + m : m;
        setEditorTip('内容在 ' + h + ':' + m + ' 进行了缓存');
    }
    $(editorid + '_svdsecond').innerHTML = '<a title="点击关闭自动保存" onclick="setAutosave()">' + savedatac + ' 秒后缓存</a> ';
    savedatac -= 1;
}
...
function editorcontrolpos() {
    ...
    var extra_top = $("nav").clientHeight;
    var scrollTop = Math.max(document.documentElement.scrollTop, document.body.scrollTop);
    if (scrollTop + extra_top > editorcontroltop && editorcurrentheight + extra_top > editorminheight) {
        $(editorid + '_controls').style.position = 'fixed';
        $(editorid + '_controls').style.top = extra_top + 'px';
    } else {
...
function editorfull(op) {
    var op = !op ? 0 : op, control = $(editorid + '_controls'), area = $(editorid + '_textarea').parentNode,
        bbar = $(editorid + '_bbar'), iswysiwyg = wysiwyg;
    if (op) {
...
function checklength(theform) {
    ...
    showDialog('当前长度: ' + mb_strlen(message) + ' 字节，' + (postmaxchars !== 0 ? '系统限制: ' + postminchars + ' 到 ' + postmaxchars + ' 字节。' : ''), 'info', '字数检查');
}
...
function buttonContext(obj, state) {
    if (state === 'mouseover') {
        ...
        var mode = obj.state ? 'down' : 'checked';
        if (obj.mode !== mode) {
            obj.mode = mode;
        ...
        if (obj.mode !== mode) {
            obj.mode = mode;
            obj.className = mode === 'selected' ? 'checked' : '';
        }
    }
}
...
function showEditorMenu(tag, params) {
    ...
    if (menu) {
        ...
        switch (tag) {
            case 'url':
                stitle = '插入链接';
                str = '<div class="input-block"><span>链接：</span><input type="text" id="' + ctrlid + '_param_1" /></div>' +
                    (selection ? '' : '<div class="input-block"><span>文字：</span><input type="text" id="' + ctrlid + '_param_2" /></div>');
                break;
            case 'tbl':
                stitle = '插入表格';
            ...
            case 'aud':
                stitle = '插入音乐';
                menuwidth = 300;
                str = '<div class="input-block"><span>地址：</span><input placeholder="支持 wma mp3 ra rm 等格式" type="text" id="' + ctrlid + '_param_1"/></div>'
                    + '示例：http://server/audio.mp3</p>';
                break;
            case 'vid':
                menuwidth = 360;
                stitle = '插入视频';
            ...
            case 'fls':
                stitle = '插入Flash';
                menuwidth = 300;
                str = '<div class="input-block"><span>地址：</span><input placeholder="支持 swf flv 等格式" type="text" id="' + ctrlid + '_param_1" /><input type="hidden" id="' + ctrlid + '_param_2"/><input type="hidden" id="' + ctrlid + '_param_3"/></div>'
                    + '示例: http://server/flash.swf';
                break;
            case 'password':
                stitle = '插入密码';
                str = '<div class="input-block wd"><span>回复密码：</span><input type="text" id="' + ctrlid + '_param_1" /></div>';
                break;
            case 'forecolor':
            ...
            case 'code':
                stitle = '插入代码';
                if (wysiwyg) {
                    opentag = '<div class="code"><pre class="prettyprint linenums">';
                    closetag = '</pre></div>';
                }
            case 'quote':
                stitle = '引用';
            ...
            case 'free':
                ...
                stitle = '插入' + lang[tag];
                str += (tag === 'code' ? '代码虽然现在没有颜色，但会在文章页面会变得很好看呢' : '') + '<textarea id="' + ctrlid + '_param_1" rows="5"></textarea>' +
                    (tag === 'hide' ?
                        '<div class="input-block wd"><span>有效天数:</span><input type="text" id="' + ctrlid + '_param_3" /></div>'
                        + '<div class="csbox">'
                        + '<input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_1" checked="checked"/><label for="' + ctrlid + '_radio_1">只有当浏览者回复本文章时才显示</label>'
                        + '<input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_2" /><label for="' + ctrlid + '_radio_2">只有当浏览者积分高于'
                        + '<input type="text" class="px4-radius" style="width: 50px; border:1px solid #ddd; height:20px; line-height:19px; padding: 0 5px; margin:2px; color: #888;" id="' + ctrlid + '_param_2" > 时才显示</label></div>'
                        : '');
                break;
            case 'beginning':
                stitle = '插入动画';
                str = '<p>请输入开头动画 Flash 或 图片 地址:</p><p><input type="text" id="' + ctrlid + '_param_1" value="" style="width: 220px;" /></p>';
                str += '<p>点击链接地址:</p><p><input type="text" id="' + ctrlid + '_param_2" value="" style="width: 220px;" /></p>';
            ...
            case 'pasteword':
                stitle = '从 Word 粘贴内容';
                str = '<p style="height:300px"><iframe id="' + ctrlid + '_param_1" frameborder="0" style="width:100%;height:100%" onload="this.contentWindow.document.body.style.width=\'550px\';this.contentWindow.document.body.contentEditable=true;this.contentWindow.document.body.focus();this.onload=null"></iframe></p><p>请通过快捷键(Ctrl+V)把 Word 文件中的内容粘贴到上方</p>';
                menuwidth = 600;
                menupos = '00';
                menutype = 'win';
                break;
        ...
        if (menupos === '00') {
            menu.className = 'fwinmask';
            s = '<div class="fwin">'
                + '<div class="header" onmousedown="dragMenu($(\'' + ctrlid + '_menu\'), event, 1)"><span class="title-tip">' + stitle + '</span><a onclick="hideMenu(\'\', \'win\');return false;" class="close" href="javascript:;">×</a></div>'
                + '<div class="body">' + str + '</div>'
                + '<div class="footer"><button class="btn r btn-small btn-success glass px4-radius" type="submit" id="' + ctrlid + '_submit">提交</button></div>'
                + '</div>';
        } else {
            menu.className = 'fwin-lite px4-radius';
            s = '<div class="header" onmousedown="dragMenu($(\'' + ctrlid + '_menu\'), event, 1)"><span class="title">' + stitle + '</span><a onclick="hideMenu();return false;" class="close" href="javascript:;">×</a></div>'
                + '<div class="body">' + str + '</div>'
                + '<div class="footer"><button class="btn btn-micro btn-success glass px4-radius" type="submit" id="' + ctrlid + '_submit">提交</button></div>';
        }
        menu.innerHTML = s;
