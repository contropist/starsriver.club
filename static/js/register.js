...
var lastusername = '', lastpassword = '', lastemail = '', lastinvitecode = '', stmp = [], modifypwd = false,
...
function emailMenu(e, id) {
    ...
    for (var i = 0; i < emaildomains.length; i++) {
        s += '<li><a onmouseover="emailMenuOp(5)" ' + (emailMenui === i ? 'class="active" ' : '') + 'onclick="$(stmp[\'email\']).value=this.innerHTML;display(\'emailmore_menu\');checkemail(stmp[\'email\']);">' + v + '@' + emaildomains[i] + '</a></li>';
    }
    s += '</ul>';
