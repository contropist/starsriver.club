...
function validate(theform) {
	...
	if(($('postsubmit').name != 'replysubmit' && !($('postsubmit').name == 'editsubmit' && !isfirstpost) && theform.subject.value == "") || !sortid && !special && trim(message) == "") {
		showError('抱歉，你尚未输入标题或内容');
		return false;
	} else if(mb_strlen(theform.subject.value) > 80) {
		showError('你的标题超过 80 个字符的限制');
		return false;
	}
	...
	if(!disablepostctrl && !sortid && !special && ((postminchars != 0 && mb_strlen(message) < postminchars) || (postmaxchars != 0 && mb_strlen(message) > postmaxchars))) {
		showError('你的回复长度不符合要求。\n\n当前长度: ' + mb_strlen(message) + ' 字节\n系统限制: ' + postminchars + ' 到 ' + postmaxchars + ' 字节');
		return false;
	}
	if(UPLOADSTATUS == 0) {
		if(!confirm('你有等待上传的附件，确认不上传这些附件吗？')) {
			return false;
		}
	} else if(UPLOADSTATUS == 1) {
		showDialog('你有正在上传的附件，请稍候，上传完成后回复将会自动发表...', 'notice');
		AUTOPOST = 1;
		return false;
	}
	if(isfirstpost && $('adddynamic') != null && $('adddynamic').checked && $('postsave') != null && isNaN(parseInt($('postsave').value)) && ($('readperm') != null && $('readperm').value || $('price') != null && $('price').value)) {
		if(confirm('由于你设置了阅读权限或出售帖，你确认还转播给你的粉丝看吗？') == false) {
			return false;
		}
	}
...
function addAttach(prefix) {
	...
	tags[0].id = prefix + 'attachform_' + id;
...
function insertAttach(prefix, id) {
	...
	$(prefix + 'cpdel_' + id).innerHTML = '<a class="ft-trash-2" onclick="reAddAttach(\'' + prefix + '\', ' + id + ')">删除</a>';
...
function updateAttach(aid) {
	...
	if(!objupdate.innerHTML) {
		...
		objupdate.innerHTML = '<input type="file" name="attachupdate[paid' + aid + ']"><a onclick="updateAttach(' + aid + ')">取消</a>';
	} else {
		obj.style.display = '';
		objupdate.innerHTML = '';
	}
...
function updatetradeattach(aid, url, attachurl) {
	$('tradeaid').value = aid;
	$('tradeattach_image').innerHTML = '<img src="' + attachurl + '/' + url + '"/>';
	ATTACHORIMAGE = 1;
}

function updateactivityattach(aid, url, attachurl) {
	$('activityaid').value = aid;
	$('activityattach_image').innerHTML = '<img src="' + attachurl + '/' + url + '"/>';
	ATTACHORIMAGE = 1;
}
...
function updatesortattach(aid, url, attachurl, identifier) {
	...
	$('sortattach_image_' + identifier).innerHTML = '<img src="' + attachurl + '/' + url + '"/>';
	ATTACHORIMAGE = 1;
}
...
function addpolloption() {
	if(curoptions < maxoptions) {
		...
		$('polloption_new').outerHTML = '<div class="poll-item">' + pollstr + '</div>' + $('polloption_new').outerHTML;
		curoptions++;
		curnumber++;
...
function showExtra(id) {
	if ($(id+'_c').style.display == 'inline-block') {
		$(id+'_b').className = 'checked';
		$(id+'_c').style.display = 'none';
	} else {
		var extraButton = $('post_ext_nav').getElementsByTagName('li');
		var extraForm = $('post_ext_c').getElementsByTagName('div');
		...
		for (i=0;i<extraForm.length;i++) {
			if(hasClass(extraForm[i],'tab-container')) {
		...
		$(id+'_b').className = 'checked';
		$(id+'_c').style.display = 'inline-block';
	}
}

function extraCheck(op) {
	if(!op && $('extra_replycredit_b')) {
		$('extra_replycredit_b').className = $('replycredit_extcredits').value > 0 && $('replycredit_times').value > 0 ? 'checked' : '';
	} else if(op == 1 && $('readperm')) {
		$('extra_readperm_b').className = $('readperm').value !== '' ? 'checked' : '';
	} else if(op == 2 && $('price')) {
		$('extra_price_b').className = $('price').value > 0 ? 'checked' : '';
	} else if(op == 3 && $('rushreply')) {
		$('extra_rushreplyset_b').className = $('rushreply').checked ? 'checked' : '';
	} else if(op == 4 && $('tags')) {
		$('extra_tag_b').className = $('tags').value !== '' ? 'checked' : '';
	} else if(op == 5 && $('cronpublish')) {
		$('extra_pubdate_b').className = $('cronpublish').checked ? 'checked' : '';
	}
}
...
function getreplycredit() {
	...
	if(real_reply_credit > userextcredit) {
		$('replycredit').innerHTML = '<b>回复奖励积分总额过大('+real_reply_credit+')</b>';
	} else {
