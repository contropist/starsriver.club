...
friendSelector.prototype = {
			...
			if(event.keyCode === 188 || event.keyCode === 13 || event.keyCode === 59) {
				...
				if(this.showType === 3) {
					...
					if(!result) {
						...
						$(this.handleObj.id+'_menu').style.display = '';
					}
				}
			}
	...
	selectUserName:function(userName) {
		...
		if(userName !== '') {
			...
			if(uid && typeof this.selectUser[uid] === 'undefined' || uid === 0 && typeof this.selectUser[userName] === 'undefined') {
				...
				this.selectNumber++;
				spanObj.innerHTML= '<span>'+ userName + '</span><a class="close" onclick="'+this.handleKey+'.delSelUser(\''+(spanObj.id)+'\');" title="删除">×</a><input type="hidden" name="users[]" value="'+userName+'" uid="uid'+uid+'" />';
				document.querySelector('#selectedlist').appendChild(spanObj);
				this.showObj.parentNode.style.display = 'none';
			}
		}
	},
	...
	append : function(uid, filtrate, form) {
		...
		if(this.showType === 1) {
			liObj.innerHTML = '<a id="' + liObj.userid + '" onclick="' + this.handleKey + '.select(this.id)" class="cl"><span class="avt brs" style="background-image: url(' + appendUserData['avatar'] + ');"><span></span></span><span class="d">' + username + '</span></a>';
		} else if(this.showType === 2) {
			if(appendUserData['new'] && typeof this.newPMUser[uid] === 'undefined') {
				this.newPMUser[uid] = uid;
			}
			liObj.className = this.interlaced ? 'alt' : '';
			liObj.innerHTML = '<div id="avt_' + liObj.userid + '" class="avt"><a href="home.php?mod=spacecp&ac=pm&op=showmsg&handlekey=showmsg_' + liObj.userid + '&touid=' + liObj.userid + '&pmid='+appendUserData['pmid']+'&daterange='+appendUserData['daterange']+'" title="'+username+'" id="avatarmsg_' + liObj.userid + '" onclick="'+this.handleKey+'.delNewFlag(' + liObj.userid + ');showWindow(\'showMsgBox\', this.href, \'get\', 0);"><img src="' + appendUserData['avatar'] + '" alt="'+username+'" /></a></div><p><a href="home.php?mod=spacecp&ac=pm&op=showmsg&handlekey=showmsg_' + liObj.userid + '&touid=' + liObj.userid + '&pmid='+appendUserData['pmid']+'&daterange='+appendUserData['daterange']+'" title="'+username+'" id="usernamemsg_' + liObj.userid + '" onclick="'+this.handleKey+'.delNewFlag(' + liObj.userid + ');showWindow(\'showMsgBox\', this.href, \'get\', 0);">'+username+'</a></p>';
		} else {
			if(form) {
				var checkstate = typeof this.selectUser[uid] === 'undefined' ? '' : ' checked="checked" ';
				liObj.innerHTML = '<input type="checkbox" name="selUsers[]" id="chk'+uid+'" value="'+ appendUserData['username'] +'" onclick="if(this.checked) {' + this.handleKey + '.selectUserName(this.value);} else {' + this.handleKey + '.delSelUser(\'uid'+uid+'\');}" '+checkstate+' /><label for="chk'+uid+'" >' + username + '</label>';
				this.selBoxObj.appendChild(liObj);
				return true;
			} else {
				liObj.innerHTML = '<a username="' + this.dataSource[uid]['username'] + '" onmouseover="' + this.handleKey + '.mouseOverPrompter(this);" onclick="' + this.handleKey + '.selectUserName(this.getAttribute(\'username\'));$(\'username\').focus();" class="cl" id="prompter_' + uid + '"><label>' + username + '</label></a>';
			}
