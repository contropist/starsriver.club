...
function emoticons(showid, target) {
    if($(showid + '_menu')) {
        ...
        var emoticon = [
            ...
            '◔ ‸◔？ ',       '（ง •̀_•́）ง',        '（*•̀ㅂ•́）و',      '╮(๑•́ ₃•̀๑)╭ ',

			'_(:3」∠)_',     '（/TДT)/',        '(ﾟДﾟ≡ﾟдﾟ)!?',  'Σ(ﾟдﾟ;)',

            '（ ͡° ͜ʖ ͡°）',   'ヘ(￣ω￣ヘ）',     '（,,#ﾟДﾟ）',     '（*′∇`*）',

            '≖‿≖✧ ',        '๑乛◡乛๑',          '（ ｡ớ ₃ờ）ھ ',    '（•‾̑⌣‾̑•）✧˖° ',

			' ٩(๑´0`๑)۶ ',   ' ٩(\//̀Д/́/)۶ ',    ' b(￣▽￣)d ',     ' \\(“▔□▔)/ ',

			'(╯°口°)╯(┴—┴',  '（#-_-)┯━┯', '(￣ε(#￣) Σ',
        ];
        var faceDiv = document.createElement("div");
        faceDiv.id = showid+'_menu';
        faceDiv.className = 'emoticons';
        faceDiv.style.position = 'absolute';
        faceDiv.style.zIndex = 1001;

        for(i=0; i<emoticon.length; i++) {
            faceDiv.innerHTML += '<i onclick="insertEmoticons(\''+ target +'\')">' + emoticon[i] + '</i>';
        }
        $('append_parent').appendChild(faceDiv)
    }
    setMenuPosition(showid, 0);
...
function insertEmoticons(target) {
	e = getEventobj();
	var faceText = e.innerHTML;
	if($(target) != null) {
...
function myfriend_post(uid) {
	if($('friend_'+uid)) {
		$('friend_'+uid).innerHTML = '<p>你们现在是好友了，接下来，你还可以：<a href="home.php?mod=space&do=wall&uid='+uid+'" target="_blank">给TA留言</a> ，或者 <a href="home.php?mod=spacecp&ac=poke&op=send&uid='+uid+'&handlekey=propokehk_'+uid+'" id="a_poke_'+uid+'" onclick="showWindow(this.id, this.href, \'get\', 0, {\'ctrlid\':this.id,\'pos\':\'13\'});">打个招呼</a></p>';
	}
	showCreditPrompt();
}
