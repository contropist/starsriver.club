/*
	...
	$Id: space_nest.js 23838 2011-08-11 06:51:58Z monkey $
*/
...
drag.extend({
	...
	removeBlock : function (e) {
		...
		if (!confirm('你确实要删除吗,删除以后将不可恢复')) return false;
		...
		if (el != null) {
			...
			x.get('home.php?mod=spacecp&ac=index&op=getblock&blockname='+blockname+'&inajax=1',function(s) {
				if (s) {
					...
					el.innerHTML = s;
					var id = drag.data['nestpage'][0]['columns']['frame1_left']['children'][0]['name'];
...
spacenest.extend({
	save:function () {
		drag.clearClose();
		document.nestform.spacecss.value = this.getSpacecssStr();
		document.nestform.style.value = this.style;
		document.nestform.layoutdata.value = drag.getPositionStr();
		document.nestform.currentlayout.value = this.currentLayout;
		document.nestform.submit();

	},
	getnest : function (type) {
		var type_ = type == 'image' ? 'nest' : type;
		if (type_) {
			...
			var ajaxtarget = type == 'image' ? 'nestimages' : '';
			var x = new Ajax();
			x.showId = ajaxtarget;
			x.get('home.php?mod=spacecp&ac=index'+para+'&inajax=1&ajaxtarget='+ajaxtarget,function(s) {
				if (s) {
					...
					if (type_ == 'block') {
						...
						$('layout'+spacenest.currentLayout).className = 'activity';
					} else if (type_ == 'nest' && type != 'image') {
	...
	initSpaceInfo : function () {
		...
		if ($('spaceinfoshow')) {
			...
			$('spaceinfoshow').onmousedown = function () {spacenest.showEditSpaceInfo();};
		}
		if ($('nv')) {
			if(!$('nv').getElementsByTagName('li').length) {
				$('nv').getElementsByTagName('ul')[0].className = 'mininv';
			}
			$('nv').onmouseover = function () {spacenest.showEditNvInfo();};
			$('nv').onmouseout = function () {spacenest.hideEditNvInfo();};
		}
	},
	showEditNvInfo : function () {
		...
		if(!nv) {
			...
			dom.innerHTML = '<span id="editnvinfo" class="edit" style="background-color:#336699;" onclick="spacenest.opNvEditInfo();">设置</span>';
