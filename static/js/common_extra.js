	...
	var showselect_row = function (inpid, s, v, notime, rettype) {
		if(v >= 0) {
			if(!rettype) {
				...
				return '<a onclick="$(\'' + inpid + '\').value = \'' + d.getFullYear() + '-' + month + '-' + day + (!notime ? ' ' + hour + ':' + minute: '') + '\'">' + s + '</a>';
			} else {
				return '<a onclick="$(\'' + inpid + '\').value = \'' + v + '\'">' + s + '</a>';
			}
		} else if(v == -1) {
			return '<a onclick="$(\'' + inpid + '\').focus()">' + s + '</a>';
		} else if(v == -2) {
			return '<a onclick="$(\'' + inpid + '\').onclick()">' + s + '</a>';
		}
	};
		...
		if(!t) {
			...
			if($(t)) {
				...
				for(i = 0;i < lis.length;i++) {
					s += '<a onclick="$(\'' + inpid + '\').value = this.innerHTML;$(\''+obj.id+'_menu\').style.display=\'none\'">' + lis[i].innerHTML + '</a>';
				}
				s += showselect_row(inpid, '自定义', -1);
			} else {
				s += '<a onclick="$(\'' + inpid + '\').value = \'0\'">永久</a>';
...
function _zoom(obj, zimg, nocover, pn, showexif) {
	...
	var imgtitle = !nocover && obj.alt ? '<div class="img-title">' + htmlspecialchars(obj.alt) + '</div>' +
	...
	var showimage = function (zimg, w, h, imgw, imgh) {
		...
		$(menuid + '_img').style.width = '100%';
		$(menuid + '_img').style.height = '100%';
		$(menuid).style.width = (w < 100 ? 100 : w - 40) + 'px';
		mheight = h - h/w*40 + 40;
		menu.style.height = mheight + 'px';
        menu.style.transitionDuration = 0;
        menu.style.transitionTimingFunction = '';
        menu.style.transitionDelay = 0;
		$(menuid + '_zoomlayer').style.height = (mheight < 120 ? 120 : mheight - h/w*40) + 'px';
		$(menuid + '_img').innerHTML = '<img id="' + zoomid + '" w="' + imgw + '" h="' + imgh + '">' + imgtitle;
		$(zoomid).src = zimg;
		$(zoomid).width = w - 40;
		$(zoomid).height = h - h/w*40;
		if($(menuid + '_imglink')) {
	...
	var adjust = function(e, a) {
		...
		menu.style.width = (parseInt(ele.width < 100 ? 100 : parseInt(ele.width))) + 'px';
		var mheight = (parseInt(ele.height) + 40);
		menu.style.height = mheight + 'px';
	...
	if(!menu && !pn) {
		...
		if(cover) {
			menu.innerHTML =
				'<div class="image-view" id="' + menuid + '_zoomlayer" style="display:none">' +
				'	<div class="imgsort" id="' + menuid + '_img"></div>' +
                '	<div class="thumb trans-ease-quick" id="' + menuid + '_picpage"></div>' +
                '	<a class="resize pxa-radius" id="' + menuid + '_adjust" href="javascipt:;" class="imgadjust" title="原尺寸"></a>' +
                '	<a class="close pxa-radius" onclick="hideMenu()"  title="关闭">×</a>' +
                '</div>';
		} else {
			menu.innerHTML =
				'<div class="popupmenu_popup" id="' + menuid + '_zoomlayer" style="width:auto">' +
				'	<div class="imgsort" id="' + menuid + '_img"></div>' +
                '	<div class="thumb" id="' + menuid + '_picpage"></div>' +
                '	<a class="close" onclick="hideMenu()">×</a>' +
				'</div>';
		}
	...
	if(typeof zoomgroup == 'object' && zoomgroup[obj.id] && typeof aimgcount == 'object' && aimgcount[zoomgroup[obj.id]]) {
		...
		if(authorlength > 1) {
			...
			if(authorcurrent !== '') {
				...
				picpage += ' <div class="prev" id="viewimg_prev" onmouseover="dragMenuDisabled=true;" onmouseout="dragMenuDisabled=false;" data-imgcode="'+paid+'" data-condition="' + (showexif ? 1 : 0) +'" onclick="page_switch(event)"></div> ';
				paid = authorcurrent < authorlength - 1 ? authorimgs[authorcurrent + 1] : authorimgs[0];
				picpage += ' <div class="next" id="viewimg_next" onmouseover="dragMenuDisabled=true;" onmouseout="dragMenuDisabled=false;" data-imgcode="'+paid+'" data-condition="'+ (showexif ? 1 : 0) +'" onclick="page_switch(event)"></div> ';
			}
			if(picpage) {
...
function page_switch(event) {
    var e = event || window.event;
    var ctl = e.srcElement || e.currentTarget;
    var paid = ctl.getAttribute('data-imgcode');
    var showexif = ctl.getAttribute('data-condition');
    var imagesrc = document.querySelector('#aimg_' + paid).getAttribute('zoomfile') ? document.querySelector('#aimg_' + paid).getAttribute('zoomfile') : document.querySelector('#aimg_' + paid).getAttribute('file');
    zoom(document.querySelector('#aimg_' + paid), imagesrc, 0, 1, showexif ? 1 : 0);
}
...
function _initTab(frameId, type) {
	if (typeof document['nestform'] == 'object' || $(frameId).className.indexOf('tab') < 0) return false;
	type = type || 'click';
...
function _showTip(ctrlobj) {
	...
	if(!$(menuid)) {
		...
		div.style.display = 'none';
		div.innerHTML = '<div class="content">' + ctrlobj.getAttribute('tip') + '</div>';
		$('append_parent').appendChild(div);
	}
...
function _showPrompt(ctrlid, evt, msg, timeout, classname,wl) {
	...
	$('append_parent').appendChild(div);
	if(ctrlid) {
		msg = '<div id="' + ctrlid + '_prompt"><div>' + msg + '</div>';
	} else {
		msg = '<div class="alert alert-flow-'+ wl +' pxa-radius">' + msg +'</div>';
	}
	div.innerHTML = msg;
	if(ctrlid) {
		if(!timeout) {
...
function creditShow(creditinfo, notice, basev, bk, first, creditrule) {
	...
	if(!$('creditpromptdiv')) {
		showPrompt(null, null, '<div id="creditpromptdiv">' + (creditrule ? '<i>' + creditrule + '</i> ' : '') + s + '</div>', 0 ,null,'success');
	} else {
		$('creditpromptdiv').innerHTML = s;
	}
	setTimeout(function () {hideMenu(1, 'prompt');$('append_parent').removeChild($('ntcwin'));}, 3500);
}
...
function _showCreditmenu() {
	if(!$('extcreditmenu_menu')) {
		...
		menu.innerHTML = '<img class="loading" src="'+ IMGDIR + '/loading.gif" /> 请稍候...';
		$('append_parent').appendChild(menu);
