...
function extrafunc_atListMenu(tag, op) {
	...
	if(!op) {
		...
		return '请输用户名:<br><input type="text" id="atkeyword" style="width:240px" value="" onkeydown="atEnter(event, \'atListSet\')" onkeyup="atFilter(this.value, \'at_list\',\'atListSet\',event, true);" /><div class="p_pop" id="at_list" style="width:250px;"><ul><li>@朋友账号，就能提醒他来看回复</li></ul></div>';
	} else {
		if($('atkeyword').value) {
...
function atFilter(kw, id, call, e, nae) {
	...
	if(nae || !atEnter(e, call)) {
		...
		if(atResult.length) {
			...
			for(i in atResult) {
				var atclass = i == curatli ? ' class="active"' : '';
				newlist += '<li><a id="atli_'+i+'"'+atclass+' onclick="'+call+'(this.innerText)">' + atResult[i] + '</a></li>';
			}
			$(id).innerHTML = '<ul>' + newlist + '<li>@朋友账号，就能提醒他来看回复</li></ul>';
		} else {
