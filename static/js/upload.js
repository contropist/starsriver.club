...
SWFUpload.prototype.initSettings = function (userSettings) {
	...
	this.ensureDefault("flash_url", IMGURL+"/common/program/swfupload.swf");
	this.ensureDefault("flash9_url", IMGURL+"/common/program/swfupload.swf");
...
function fileQueueError(file, errorCode, message) {
	try {
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			...
			showDialog("你选择的文件个数超过限制。\n"+(message === 0 ? "你已达到上传文件的上限了。" : "你还可以选择 " + message + " 个文件"), 'notice', null, null, 0, null, null, null, null, sdCloseTime);
			return;
		}
		...
		if(this.customSettings.uploadSource == 'forum') {
			if(this.customSettings.uploadType == 'poll') {
				...
				if(parseInt(data.aid)) {
					...
					var img = new Image();
					img.src = IMGURL + '/common/icon/image.svg';//data.smallimg;
			...
			if(parseInt(data.picid)) {
				...
				newTd = document.createElement("TD");
				newTd.className = 'd';
				newTd.innerHTML = '图片描述<br><textarea name="title['+data.picid+']" cols="40" rows="2" ></textarea>';
				newTr.appendChild(newTd);
