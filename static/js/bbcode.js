...
function bbcode2html(str) {
	...
	if(!fetchCheckbox('bbcodeoff') && allowbbcode) {
		...
		if(parsetype != 1) {
			str = str.replace(/\[quote]([\s\S]*?)\[\/quote\]\s?\s?/ig, '<blockquote>$1</blockquote>\n');
		}
		...
		str = preg_replace([
			...
			'</i>', '<u>', '</u>', '<strike>', '</strike>', '<hr/>', '<ul>', '<ul type=1 class="litype_1">', '<ul type=a class="litype_2">',
...
function html2bbcode(str) {
	...
	str = str.replace(/<div\sclass=["']?code["']?>[\s\S]*?<pre class="prettyprint linenums">([\s\S]+?)<\/pre>[\s\S]*?<\/div>/ig, function($1, $2) {return codetag($2);});
	...
	if(!fetchCheckbox('bbcodeoff') && allowbbcode) {
		...
		str = str.replace(/<blockquote>([\s\S]*?)<\/blockquote>([\s\S]*?)(<br[^>]*>)?/ig, "[quote]$1[/quote]");
		str = str.replace(/<div[^>]*code[^>]*><pre class="prettyprint linenums">([\s\S]*?)<\/pre><\/div>([\s\S]*?)(<br[^>]*>)?/ig, "[code]$1[/code]");
...
function parsecode(text) {
	DISCUZCODE['num']++;
	DISCUZCODE['html'][DISCUZCODE['num']] = '<div class="code"><pre class="prettyprint linenums">' + htmlspecialchars(text) + '</pre></div>';
